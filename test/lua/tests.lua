...
retest.describe('mupen64', function()
    ...
    retest.describe('memory', function()
        ...
        local rdram_size = 0x800000
        local wrapped_addr = rdram_size

        local function to_rdram_addr(addr, size)
            if size == 8 or size == 4 then
                return addr
            end
            ...
            error('unsupported access size')
        end

        local function wrapped_alias_addr(size)
            return to_rdram_addr(wrapped_addr, size) % rdram_size
        end

        local function oob_addr(size)
            return to_rdram_addr(rdram_size - (size - 1), size)
        end

        local wrapped_api_alias_addr = wrapped_addr % rdram_size
        local wrapped_byte_alias_addr = wrapped_alias_addr(1)
        local wrapped_word_alias_addr = wrapped_alias_addr(2)
        local wrapped_dword_alias_addr = wrapped_alias_addr(4)
        local wrapped_qword_alias_addr = wrapped_alias_addr(8)
        local wrapped_raw_alias_addr = wrapped_addr % rdram_size

        local last_oob_word_addr = oob_addr(2)
        local last_oob_dword_addr = oob_addr(4)
        local last_oob_qword_addr = oob_addr(8)
        local last_oob_raw_addr = rdram_size - 1

        retest.describe('readbyte', function()
            retest.it('reads_and_writes_byte', function()
                memory.writebyte(test_addr, 0x42)
                local value = memory.readbyte(test_addr)
                retest.expect(value).to.equal(0x42)
            end)
            retest.it('handles_zero', function()
                memory.writebyte(test_addr, 0x00)
                local value = memory.readbyte(test_addr)
                retest.expect(value).to.equal(0x00)
            end)
            retest.it('handles_max_value', function()
                memory.writebyte(test_addr, 0xFF)
                local value = memory.readbyte(test_addr)
                retest.expect(value).to.equal(0xFF)
            end)
            retest.it('wraps_address_past_rdram_end', function()
                memory.writebyte(wrapped_api_alias_addr, 0xA5)
                local value = memory.readbyte(wrapped_addr)
                retest.expect(value).to.equal(0xA5)
            end)
        end)

        retest.describe('readbytesigned', function()
            retest.it('reads_positive_value', function()
                memory.writebyte(test_addr, 0x7F) -- 127
                local value = memory.readbytesigned(test_addr)
                retest.expect(value).to.equal(127)
            end)
            retest.it('reads_negative_value', function()
                memory.writebyte(test_addr, 0xFF) -- -1
                local value = memory.readbytesigned(test_addr)
                retest.expect(value).to.equal(-1)
            end)
            retest.it('reads_zero', function()
                memory.writebyte(test_addr, 0x00)
                local value = memory.readbytesigned(test_addr)
                retest.expect(value).to.equal(0)
            end)
            retest.it('wraps_address_past_rdram_end', function()
                memory.writebyte(wrapped_api_alias_addr, 0xFF)
                local value = memory.readbytesigned(wrapped_addr)
                retest.expect(value).to.equal(-1)
            end)
        end)

        retest.describe('readword', function()
            retest.it('reads_and_writes_word', function()
                memory.writeword(test_addr, 0x1234)
                local value = memory.readword(test_addr)
                retest.expect(value).to.equal(0x1234)
            end)
            retest.it('handles_zero', function()
                memory.writeword(test_addr, 0x0000)
                local value = memory.readword(test_addr)
                retest.expect(value).to.equal(0x0000)
            end)
            retest.it('handles_max_value', function()
                memory.writeword(test_addr, 0xFFFF)
                local value = memory.readword(test_addr)
                retest.expect(value).to.equal(0xFFFF)
            end)
            retest.it('errors_when_read_crosses_rdram_end', function()
                local func = function()
                    memory.readword(last_oob_word_addr)
                end
                retest.expect(func).to.fail()
            end)
            retest.it('wraps_address_past_rdram_end', function()
                memory.writeword(wrapped_api_alias_addr, 0x4321)
                local value = memory.readword(wrapped_addr)
                retest.expect(value).to.equal(0x4321)
            end)
        end)

        retest.describe('readwordsigned', function()
            retest.it('reads_positive_value', function()
                memory.writeword(test_addr, 0x7FFF) -- 32767
                local value = memory.readwordsigned(test_addr)
                retest.expect(value).to.equal(32767)
            end)
            retest.it('reads_negative_value', function()
                memory.writeword(test_addr, 0xFFFF) -- -1
                local value = memory.readwordsigned(test_addr)
                retest.expect(value).to.equal(-1)
            end)
            retest.it('reads_zero', function()
            ...
            retest.it('errors_when_read_crosses_rdram_end', function()
                local func = function()
                    memory.readwordsigned(last_oob_word_addr)
                end
                retest.expect(func).to.fail()
            end)
            retest.it('wraps_address_past_rdram_end', function()
                memory.writeword(wrapped_api_alias_addr, 0xFFFF)
                local value = memory.readwordsigned(wrapped_addr)
                retest.expect(value).to.equal(-1)
            end)
        end)

        retest.describe('readdword', function()
            retest.it('reads_and_writes_dword', function()
                memory.writedword(test_addr, 0x12345678)
                local value = memory.readdword(test_addr)
                retest.expect(value).to.equal(0x12345678)
            end)
            retest.it('handles_zero', function()
                memory.writedword(test_addr, 0x00000000)
                local value = memory.readdword(test_addr)
                retest.expect(value).to.equal(0x00000000)
            end)
            retest.it('handles_large_value', function()
                memory.writedword(test_addr, 0xDEADBEEF)
                local value = memory.readdword(test_addr)
                retest.expect(value).to.equal(0xDEADBEEF)
            end)
            retest.it('errors_when_read_crosses_rdram_end', function()
                local func = function()
                    memory.readdword(last_oob_dword_addr)
                end
                retest.expect(func).to.fail()
            end)
            retest.it('wraps_address_past_rdram_end', function()
                memory.writedword(wrapped_dword_alias_addr, 0x89ABCDEF)
                local value = memory.readdword(wrapped_addr)
                retest.expect(value).to.equal(0x89ABCDEF)
            end)
        end)

        retest.describe('readdwordsigned', function()
            retest.it('reads_positive_value', function()
                memory.writedword(test_addr, 0x7FFFFFFF) -- 2147483647
                local value = memory.readdwordsigned(test_addr)
                retest.expect(value).to.equal(2147483647)
            end)
            retest.it('reads_negative_value', function()
                memory.writedword(test_addr, 0xFFFFFFFF) -- -1
                local value = memory.readdwordsigned(test_addr)
                retest.expect(value).to.equal(-1)
            end)
            retest.it('reads_zero', function()
            ...
            retest.it('errors_when_read_crosses_rdram_end', function()
                local func = function()
                    memory.readdwordsigned(last_oob_dword_addr)
                end
                retest.expect(func).to.fail()
            end)
            retest.it('wraps_address_past_rdram_end', function()
                memory.writedword(wrapped_dword_alias_addr, 0xFFFFFFFF)
                local value = memory.readdwordsigned(wrapped_addr)
                retest.expect(value).to.equal(-1)
            end)
        end)

        retest.describe('readqword', function()
            retest.it('reads_and_writes_qword', function()
                memory.writeqword(test_addr, { 0x12345678, 0x9ABCDEF0 })
                local value = memory.readqword(test_addr)
                retest.expect(value[1]).to.equal(0x12345678)
                retest.expect(value[2]).to.equal(0x9ABCDEF0)
            end)
            retest.it('handles_zero', function()
                memory.writeqword(test_addr, { 0x00000000, 0x00000000 })
                local value = memory.readqword(test_addr)
                retest.expect(value[1]).to.equal(0x00000000)
                retest.expect(value[2]).to.equal(0x00000000)
            end)
            retest.it('handles_max_value', function()
                memory.writeqword(test_addr, { 0xFFFFFFFF, 0xFFFFFFFF })
                local value = memory.readqword(test_addr)
                retest.expect(value[1]).to.equal(0xFFFFFFFF)
                retest.expect(value[2]).to.equal(0xFFFFFFFF)
            end)
            retest.it('errors_when_read_crosses_rdram_end', function()
                local func = function()
                    memory.readqword(last_oob_qword_addr)
                end
                retest.expect(func).to.fail()
            end)
            retest.it('wraps_address_past_rdram_end', function()
                memory.writeqword(wrapped_qword_alias_addr, { 0x01234567, 0x89ABCDEF })
                local value = memory.readqword(wrapped_addr)
                retest.expect(value[1]).to.equal(0x01234567)
                retest.expect(value[2]).to.equal(0x89ABCDEF)
            end)
        end)

        retest.describe('readqwordsigned', function()
            retest.it('reads_positive_value', function()
                memory.writeqword(test_addr, { 0x7FFFFFFF, 0xFFFFFFFF })
                local value = memory.readqwordsigned(test_addr)
                retest.expect(value[1]).to.equal(0x7FFFFFFF)
                retest.expect(value[2]).to.equal(0xFFFFFFFF)
            end)
            retest.it('reads_negative_value', function()
                memory.writeqword(test_addr, { 0xFFFFFFFF, 0xFFFFFFFF })
            ...
            retest.it('errors_when_read_crosses_rdram_end', function()
                local func = function()
                    memory.readqwordsigned(last_oob_qword_addr)
                end
                retest.expect(func).to.fail()
            end)
            retest.it('wraps_address_past_rdram_end', function()
                memory.writeqword(wrapped_qword_alias_addr, { 0xFFFFFFFF, 0xFFFFFFFF })
                local value = memory.readqwordsigned(wrapped_addr)
                retest.expect(value[1]).to.equal(0xFFFFFFFF)
                retest.expect(value[2]).to.equal(0xFFFFFFFF)
            end)
        end)

        retest.describe('readfloat', function()
            retest.it('reads_and_writes_float', function()
                local test_float = 3.14159
                memory.writefloat(test_addr, test_float)
                local value = memory.readfloat(test_addr)
                retest.expect(math.abs(value - test_float) < 0.0001).to.equal(true)
            end)
            retest.it('reads_zero', function()
                memory.writefloat(test_addr, 0.0)
                local value = memory.readfloat(test_addr)
                retest.expect(value).to.equal(0.0)
            end)
            retest.it('reads_negative_float', function()
                local test_float = -42.5
                memory.writefloat(test_addr, test_float)
                local value = memory.readfloat(test_addr)
                retest.expect(math.abs(value - test_float) < 0.0001).to.equal(true)
            end)
            retest.it('errors_when_read_crosses_rdram_end', function()
                local func = function()
                    memory.readfloat(last_oob_dword_addr)
                end
                retest.expect(func).to.fail()
            end)
            retest.it('wraps_address_past_rdram_end', function()
                local test_float = 42.25
                memory.writefloat(wrapped_dword_alias_addr, test_float)
                local value = memory.readfloat(wrapped_addr)
                retest.expect(math.abs(value - test_float) < 0.0001).to.equal(true)
            end)
        end)

        retest.describe('readdouble', function()
            retest.it('reads_and_writes_double', function()
                local test_double = 3.141592653589793
                memory.writedouble(test_addr, test_double)
                local value = memory.readdouble(test_addr)
                retest.expect(math.abs(value - test_double) < 0.00000001).to.equal(true)
            end)
            retest.it('reads_zero', function()
                memory.writedouble(test_addr, 0.0)
                local value = memory.readdouble(test_addr)
                retest.expect(value).to.equal(0.0)
            end)
            retest.it('reads_negative_double', function()
                local test_double = -123.456789
                memory.writedouble(test_addr, test_double)
                local value = memory.readdouble(test_addr)
                retest.expect(math.abs(value - test_double) < 0.00000001).to.equal(true)
            end)
            retest.it('errors_when_read_crosses_rdram_end', function()
                local func = function()
                    memory.readdouble(last_oob_qword_addr)
                end
                retest.expect(func).to.fail()
            end)
            retest.it('wraps_address_past_rdram_end', function()
                local test_double = -9876.54321
                memory.writedouble(wrapped_qword_alias_addr, test_double)
                local value = memory.readdouble(wrapped_addr)
                retest.expect(math.abs(value - test_double) < 0.00000001).to.equal(true)
            end)
        end)

        retest.describe('readsize', function()
            retest.it('reads_1_byte_unsigned', function()
                memory.writebyte(test_addr, 0x42)
                local value = memory.readsize(test_addr, 1)
                retest.expect(value).to.equal(0x42)
            end)
            retest.it('reads_1_byte_signed', function()
                memory.writebyte(test_addr, 0xFF)
                local value = memory.readsize(test_addr, -1)
                retest.expect(value).to.equal(-1)
            end)
            retest.it('reads_2_bytes_unsigned', function()
                memory.writeword(test_addr, 0x1234)
                local value = memory.readsize(test_addr, 2)
                retest.expect(value).to.equal(0x1234)
            end)
            retest.it('reads_2_bytes_signed', function()
                memory.writeword(test_addr, 0xFFFF)
                local value = memory.readsize(test_addr, -2)
                retest.expect(value).to.equal(-1)
            end)
            retest.it('reads_4_bytes_unsigned', function()
                memory.writedword(test_addr, 0x12345678)
                local value = memory.readsize(test_addr, 4)
                retest.expect(value).to.equal(0x12345678)
            end)
            retest.it('reads_4_bytes_signed', function()
                memory.writedword(test_addr, 0xFFFFFFFF)
                local value = memory.readsize(test_addr, -4)
                retest.expect(value).to.equal(-1)
            end)
            retest.it('reads_8_bytes_unsigned', function()
                memory.writeqword(test_addr, { 0x12345678, 0x9ABCDEF0 })
                local value = memory.readsize(test_addr, 8)
                retest.expect(value[1]).to.equal(0x12345678)
                retest.expect(value[2]).to.equal(0x9ABCDEF0)
            end)
            retest.it('reads_8_bytes_signed', function()
                memory.writeqword(test_addr, { 0xFFFFFFFF, 0xFFFFFFFF })
                local value = memory.readsize(test_addr, -8)
                retest.expect(value[1]).to.equal(0xFFFFFFFF)
                retest.expect(value[2]).to.equal(0xFFFFFFFF)
            end)
            retest.it('wraps_1_byte_unsigned_address_past_rdram_end', function()
                memory.writebyte(wrapped_api_alias_addr, 0x42)
                local value = memory.readsize(wrapped_addr, 1)
                retest.expect(value).to.equal(0x42)
            end)
            retest.it('wraps_1_byte_signed_address_past_rdram_end', function()
                memory.writebyte(wrapped_api_alias_addr, 0xFF)
                local value = memory.readsize(wrapped_addr, -1)
                retest.expect(value).to.equal(-1)
            end)
            retest.it('wraps_2_byte_unsigned_address_past_rdram_end', function()
                memory.writeword(wrapped_api_alias_addr, 0x1234)
                local value = memory.readsize(wrapped_addr, 2)
                retest.expect(value).to.equal(0x1234)
            end)
            retest.it('wraps_2_byte_signed_address_past_rdram_end', function()
                memory.writeword(wrapped_api_alias_addr, 0xFFFF)
                local value = memory.readsize(wrapped_addr, -2)
                retest.expect(value).to.equal(-1)
            end)
            retest.it('wraps_4_byte_unsigned_address_past_rdram_end', function()
                memory.writedword(wrapped_dword_alias_addr, 0x12345678)
                local value = memory.readsize(wrapped_addr, 4)
                retest.expect(value).to.equal(0x12345678)
            end)
            retest.it('wraps_4_byte_signed_address_past_rdram_end', function()
                memory.writedword(wrapped_dword_alias_addr, 0xFFFFFFFF)
                local value = memory.readsize(wrapped_addr, -4)
                retest.expect(value).to.equal(-1)
            end)
            retest.it('wraps_8_byte_unsigned_address_past_rdram_end', function()
                memory.writeqword(wrapped_qword_alias_addr, { 0x12345678, 0x9ABCDEF0 })
                local value = memory.readsize(wrapped_addr, 8)
                retest.expect(value[1]).to.equal(0x12345678)
                retest.expect(value[2]).to.equal(0x9ABCDEF0)
            end)
            retest.it('wraps_8_byte_signed_address_past_rdram_end', function()
                memory.writeqword(wrapped_qword_alias_addr, { 0xFFFFFFFF, 0xFFFFFFFF })
                local value = memory.readsize(wrapped_addr, -8)
                retest.expect(value[1]).to.equal(0xFFFFFFFF)
                retest.expect(value[2]).to.equal(0xFFFFFFFF)
            end)
            retest.it('errors_when_2_byte_unsigned_read_crosses_rdram_end', function()
                local func = function()
                    memory.readsize(last_oob_word_addr, 2)
                end
                retest.expect(func).to.fail()
            end)
            retest.it('errors_when_2_byte_signed_read_crosses_rdram_end', function()
                local func = function()
                    memory.readsize(last_oob_word_addr, -2)
                end
                retest.expect(func).to.fail()
            end)
            retest.it('errors_when_4_byte_unsigned_read_crosses_rdram_end', function()
                local func = function()
                    memory.readsize(last_oob_dword_addr, 4)
                end
                retest.expect(func).to.fail()
            end)
            retest.it('errors_when_4_byte_signed_read_crosses_rdram_end', function()
                local func = function()
                    memory.readsize(last_oob_dword_addr, -4)
                end
                retest.expect(func).to.fail()
            end)
            retest.it('errors_when_8_byte_unsigned_read_crosses_rdram_end', function()
                local func = function()
                    memory.readsize(last_oob_qword_addr, 8)
                end
                retest.expect(func).to.fail()
            end)
            retest.it('errors_when_8_byte_signed_read_crosses_rdram_end', function()
                local func = function()
                    memory.readsize(last_oob_qword_addr, -8)
                end
                retest.expect(func).to.fail()
        ...
        retest.describe('writebyte', function()
            retest.it('writes_byte', function()
                memory.writebyte(test_addr, 0xA5)
                retest.expect(memory.readbyte(test_addr)).to.equal(0xA5)
            end)
            retest.it('wraps_address_past_rdram_end', function()
                memory.writebyte(wrapped_addr, 0xA5)
                retest.expect(memory.readbyte(wrapped_api_alias_addr)).to.equal(0xA5)
            end)
        end)

        retest.describe('writeword', function()
            retest.it('writes_word', function()
                memory.writeword(test_addr, 0xABCD)
                retest.expect(memory.readword(test_addr)).to.equal(0xABCD)
            end)
            retest.it('errors_when_write_crosses_rdram_end', function()
                local func = function()
                    memory.writeword(last_oob_word_addr, 0xABCD)
                end
                retest.expect(func).to.fail()
            end)
        end)

        retest.describe('writedword', function()
            retest.it('writes_dword', function()
                memory.writedword(test_addr, 0x89ABCDEF)
                retest.expect(memory.readdword(test_addr)).to.equal(0x89ABCDEF)
            end)
            retest.it('errors_when_write_crosses_rdram_end', function()
                local func = function()
                    memory.writedword(last_oob_dword_addr, 0x89ABCDEF)
                end
                retest.expect(func).to.fail()
            end)
        end)

        retest.describe('writeqword', function()
            retest.it('writes_qword', function()
                memory.writeqword(test_addr, { 0x01234567, 0x89ABCDEF })
                local value = memory.readqword(test_addr)
                retest.expect(value[1]).to.equal(0x01234567)
                retest.expect(value[2]).to.equal(0x89ABCDEF)
            end)
            retest.it('errors_when_write_crosses_rdram_end', function()
                local func = function()
                    memory.writeqword(last_oob_qword_addr, { 0x01234567, 0x89ABCDEF })
                end
                retest.expect(func).to.fail()
            end)
        end)

        retest.describe('writefloat', function()
            retest.it('writes_float', function()
                local test_float = 42.25
                memory.writefloat(test_addr, test_float)
                local value = memory.readfloat(test_addr)
                retest.expect(math.abs(value - test_float) < 0.0001).to.equal(true)
            ...
            retest.it('errors_when_write_crosses_rdram_end', function()
                local func = function()
                    memory.writefloat(last_oob_dword_addr, 42.25)
                end
                retest.expect(func).to.fail()
            end)
        end)

        retest.describe('writedouble', function()
            retest.it('writes_double', function()
                local test_double = -9876.54321
                memory.writedouble(test_addr, test_double)
                local value = memory.readdouble(test_addr)
                retest.expect(math.abs(value - test_double) < 0.00000001).to.equal(true)
            ...
            retest.it('errors_when_write_crosses_rdram_end', function()
                local func = function()
                    memory.writedouble(last_oob_qword_addr, -9876.54321)
                end
                retest.expect(func).to.fail()
            end)
        end)

        retest.describe('writesize', function()
            retest.it('writes_1_byte', function()
                memory.writesize(test_addr, 1, 0x42)
                local value = memory.readbyte(test_addr)
                retest.expect(value).to.equal(0x42)
            end)
            retest.it('writes_2_bytes', function()
                memory.writesize(test_addr, 2, 0x1234)
                local value = memory.readword(test_addr)
                retest.expect(value).to.equal(0x1234)
            end)
            retest.it('writes_4_bytes', function()
                memory.writesize(test_addr, 4, 0x12345678)
                local value = memory.readdword(test_addr)
                retest.expect(value).to.equal(0x12345678)
            end)
            retest.it('writes_8_bytes', function()
                memory.writesize(test_addr, 8, { 0x12345678, 0x9ABCDEF0 })
                local value = memory.readqword(test_addr)
                retest.expect(value[1]).to.equal(0x12345678)
                retest.expect(value[2]).to.equal(0x9ABCDEF0)
            end)
            retest.it('wraps_1_byte_unsigned_address_past_rdram_end', function()
                memory.writesize(wrapped_addr, 1, 0x42)
                local value = memory.readbyte(wrapped_api_alias_addr)
                retest.expect(value).to.equal(0x42)
            end)
            retest.it('wraps_1_byte_signed_address_past_rdram_end', function()
                memory.writesize(wrapped_addr, -1, -1)
                local value = memory.readbytesigned(wrapped_api_alias_addr)
                retest.expect(value).to.equal(-1)
            end)
            retest.it('wraps_2_byte_unsigned_address_past_rdram_end', function()
                memory.writesize(wrapped_addr, 2, 0x1234)
                local value = memory.readword(wrapped_api_alias_addr)
                retest.expect(value).to.equal(0x1234)
            end)
            retest.it('wraps_2_byte_signed_address_past_rdram_end', function()
                memory.writesize(wrapped_addr, -2, -1)
                local value = memory.readwordsigned(wrapped_api_alias_addr)
                retest.expect(value).to.equal(-1)
            end)
            retest.it('wraps_4_byte_unsigned_address_past_rdram_end', function()
                memory.writesize(wrapped_addr, 4, 0x12345678)
                local value = memory.readdword(wrapped_dword_alias_addr)
                retest.expect(value).to.equal(0x12345678)
            end)
            retest.it('wraps_4_byte_signed_address_past_rdram_end', function()
                memory.writesize(wrapped_addr, -4, -1)
                local value = memory.readdwordsigned(wrapped_dword_alias_addr)
                retest.expect(value).to.equal(-1)
            end)
            retest.it('wraps_8_byte_unsigned_address_past_rdram_end', function()
                memory.writesize(wrapped_addr, 8, { 0x12345678, 0x9ABCDEF0 })
                local value = memory.readqword(wrapped_qword_alias_addr)
                retest.expect(value[1]).to.equal(0x12345678)
                retest.expect(value[2]).to.equal(0x9ABCDEF0)
            end)
            retest.it('wraps_8_byte_signed_address_past_rdram_end', function()
                memory.writesize(wrapped_addr, -8, { 0xFFFFFFFF, 0xFFFFFFFF })
                local value = memory.readqwordsigned(wrapped_qword_alias_addr)
                retest.expect(value[1]).to.equal(0xFFFFFFFF)
                retest.expect(value[2]).to.equal(0xFFFFFFFF)
            end)
            retest.it('errors_when_2_byte_unsigned_write_crosses_rdram_end', function()
                local func = function()
                    memory.writesize(last_oob_word_addr, 2, 0x1234)
                end
                retest.expect(func).to.fail()
            end)
            retest.it('errors_when_2_byte_signed_write_crosses_rdram_end', function()
                local func = function()
                    memory.writesize(last_oob_word_addr, -2, -1)
                end
                retest.expect(func).to.fail()
            end)
            retest.it('errors_when_4_byte_unsigned_write_crosses_rdram_end', function()
                local func = function()
                    memory.writesize(last_oob_dword_addr, 4, 0x12345678)
                end
                retest.expect(func).to.fail()
            end)
            retest.it('errors_when_4_byte_signed_write_crosses_rdram_end', function()
                local func = function()
                    memory.writesize(last_oob_dword_addr, -4, -1)
                end
                retest.expect(func).to.fail()
            end)
            retest.it('errors_when_8_byte_unsigned_write_crosses_rdram_end', function()
                local func = function()
                    memory.writesize(last_oob_qword_addr, 8, { 0x12345678, 0x9ABCDEF0 })
                end
                retest.expect(func).to.fail()
            end)
            retest.it('errors_when_8_byte_signed_write_crosses_rdram_end', function()
                local func = function()
                    memory.writesize(last_oob_qword_addr, -8, { 0xFFFFFFFF, 0xFFFFFFFF })
                end
                retest.expect(func).to.fail()
            end)
        end)

        retest.describe('write', function()
            retest.it('writes_buffer', function()
                local test_string = "abc123"
                memory.write(test_addr, test_string)
                local value = memory.read(test_addr, string.len(test_string))
                retest.expect(value).to.equal(test_string)
            end)
            retest.it('wraps_address_past_rdram_end', function()
                local test_string = "abc123"
                memory.write(wrapped_addr, test_string)
                local value = memory.read(wrapped_raw_alias_addr, string.len(test_string))
                retest.expect(value).to.equal(test_string)
            end)
            retest.it('errors_when_buffer_crosses_rdram_end', function()
                local func = function()
                    memory.write(last_oob_raw_addr, "ab")
                end
                retest.expect(func).to.fail()
            end)
        end)

        retest.describe('read', function()
            retest.it('reads_buffer', function()
                local test_string = "abc123"
                memory.write(test_addr, test_string)
                local value = memory.read(test_addr, string.len(test_string))
                retest.expect(value).to.equal(test_string)
            ...
            retest.it('wraps_address_past_rdram_end', function()
                local test_string = "xyz789"
                memory.write(wrapped_raw_alias_addr, test_string)
                local value = memory.read(wrapped_addr, string.len(test_string))
                retest.expect(value).to.equal(test_string)
            end)
            retest.it('errors_when_size_crosses_rdram_end', function()
                local func = function()
                    memory.read(last_oob_raw_addr, 2)
                end
                retest.expect(func).to.fail()
            end)
        end)

        retest.describe('inttofloat', function()
            retest.it('converts_int_bits_to_float', function()
                local float_val = memory.inttofloat(0x3F800000)
                retest.expect(math.abs(float_val - 1.0) < 0.0001).to.equal(true)
            end)
        end)

        retest.describe('floattoint', function()
            retest.it('converts_float_bits_to_int', function()
                local test_float = 1.0
                local int_val = memory.floattoint(test_float)
                local float_back = memory.inttofloat(int_val)
                retest.expect(math.abs(float_back - test_float) < 0.0001).to.equal(true)
            end)
        end)

        retest.describe('inttodouble', function()
            retest.it('converts_qword_bits_to_double', function()
                local double_val = memory.inttodouble({ 0x3FF00000, 0x00000000 })
                retest.expect(math.abs(double_val - 1.0) < 0.00000001).to.equal(true)
            end)
        end)

        retest.describe('doubletoint', function()
            retest.it('converts_double_bits_to_qword', function()
                local test_double = 1.0
                local qword_val = memory.doubletoint(test_double)
                local double_back = memory.inttodouble(qword_val)
                retest.expect(math.abs(double_back - test_double) < 0.00000001).to.equal(true)
            end)
        end)

        retest.describe('qwordtonumber', function()
            retest.it('converts_qword_to_number', function()
                local qword_val = { 0x00000000, 0x00000100 }
                local num = memory.qwordtonumber(qword_val)
                retest.expect(num).to.equal(256)
            end)
        end)
    end)
