...
void savestates_load_immediate_impl(const t_savestate_task& task)
{
    ...
    {
        auto result = g_core->show_ask_dialog("st_hash_mismatch", std::format(
    ...
    if (is_movie)
    {
        ...
        if (!task.ignore_warnings && code != Res_Ok && core_vcr_get_task() != task_idle)
        {
            ...
            const auto result = g_core->show_ask_dialog("st_unfreeze_warning", err_str.c_str(), L"Savestate", true);
            if (!result)
            {
                task.callback(Res_Cancelled, {});
                goto failedLoad;
    ...
    else
    {
        ...
        {
            const auto result = g_core->show_ask_dialog("st_not_from_movie",
