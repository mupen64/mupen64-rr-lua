...
core_result core_vcr_start_record(std::filesystem::path path, uint16_t flags, std::string author, std::string description)
{
    ...
    for (auto& [Present, RawData, Plugin] : g_core->controls)
    {
        if (Present && RawData)
        {
            bool proceed = g_core->show_ask_dialog("vcr_rawdata_warning", RAWDATA_WARNING_MESSAGE, L"VCR", true);
            if (!proceed)
            {
...
core_result core_vcr_start_playback(std::filesystem::path path)
{
    ...
    if (header.extended_version != 0)
    {
        ...
        {
            bool proceed = g_core->show_ask_dialog("vcr_wiivc_mode_warning", header.extended_flags.wii_vc ? WII_VC_MISMATCH_A_WARNING_MESSAGE : WII_VC_MISMATCH_B_WARNING_MESSAGE, L"VCR", true);
    ...
    {
        bool proceed = g_core->show_ask_dialog("vcr_rom_name_warning", std::format(ROM_NAME_WARNING_MESSAGE, string_to_wstring(header.rom_name), string_to_wstring((char*)ROM_HEADER.nom)).c_str(), L"VCR", true);
    ...
    else
    {
        ...
        {
            bool proceed = g_core->show_ask_dialog("vcr_rom_ccode_warning", std::format(ROM_COUNTRY_WARNING_MESSAGE, core_vr_country_code_to_country_name(header.rom_country), core_vr_country_code_to_country_name(ROM_HEADER.Country_code)).c_str(), L"VCR", true);
            if (!proceed)
            {
                return Res_Cancelled;
            }
        ...
        else if (header.rom_crc1 != ROM_HEADER.CRC1)
        {
            ...
            bool proceed = g_core->show_ask_dialog("vcr_rom_crc_warning", str, L"VCR", true);
            if (!proceed)
            {
                return Res_Cancelled;
            }
    ...
    if (!cht_path.empty())
    {
        ...
        {
            const auto proceed = g_core->show_ask_dialog("vcr_cheat_playback_ask", CHEAT_ERROR_ASK_MESSAGE, L"VCR", true);
