...
retest.describe('mupen64', function()
    ...
    retest.describe('memory', function()
        local RDRAM_SIZE <const> = 0x800000
        local TEST_ADDR_IB <const> = 0x100
        local TEST_ADDR_OOB <const> = RDRAM_SIZE + TEST_ADDR_IB

        retest.before(function()
            memory.writeqword(TEST_ADDR_IB, { 0xEEEEEEEE, 0xEEEEEEEE })
        end)

        retest.describe('readbyte', function()
            retest.it('roundtrip_works', function()
                memory.writebyte(TEST_ADDR_OOB, 0x80)
                retest.expect(memory.readbyte(TEST_ADDR_OOB)).to.equal(0x80)
                retest.expect(memory.readbyte(TEST_ADDR_IB)).to.equal(0x80)
            end)
            -- OOB case doesn't exist
        end)

        retest.describe('readbytesigned', function()
            retest.it('roundtrip_works', function()
                memory.writebyte(TEST_ADDR_OOB, 0x80)
                retest.expect(memory.readbytesigned(TEST_ADDR_OOB)).to.equal(-128)
                retest.expect(memory.readbytesigned(TEST_ADDR_IB)).to.equal(-128)
            end)
            -- OOB case doesn't exist
        end)
        ...
        retest.describe('readword', function()
            retest.it('roundtrip_works', function()
                memory.writeword(TEST_ADDR_OOB, 0xFFFF)
                retest.expect(memory.readword(TEST_ADDR_OOB)).to.equal(0xFFFF)
                retest.expect(memory.readword(TEST_ADDR_IB)).to.equal(0xFFFF)
            end)
            retest.it('errors_on_oob', function()
                local func = function()
                    memory.readword(0x007FFFFD)
                end
        ...
        retest.describe('readwordsigned', function()
            retest.it('roundtrip_works', function()
                memory.writeword(TEST_ADDR_OOB, 0xFFFF)
                retest.expect(memory.readwordsigned(TEST_ADDR_OOB)).to.equal(-1)
                retest.expect(memory.readwordsigned(TEST_ADDR_IB)).to.equal(-1)
            end)
            retest.it('errors_on_oob', function()
                local func = function()
                    memory.readwordsigned(0x007FFFFD)
                end
                retest.expect(func).to.fail()
            end)
        end)

        retest.describe('writewordsigned', function()
            retest.it('errors_on_oob', function()
                local func = function()
                    memory.writewordsigned(0x007FFFFD, 0)
                end
                retest.expect(func).to.fail()
            end)
        end)
        ...
        retest.describe('writedword', function()
            retest.it('errors_on_oob', function()
                local func = function()
                    memory.writedword(0x007FFFFD, 0)
                end
                retest.expect(func).to.fail()
            end)
        end)
        ...
        retest.describe('readdwordsigned', function()
            ...
            retest.it('errors_on_oob', function()
                local func = function()
                    memory.readdwordsigned(0x007FFFFD)
                end
                retest.expect(func).to.fail()
            end)
        end)

        retest.describe('writedwordsigned', function()
            retest.it('errors_on_oob', function()
                local func = function()
                    memory.writedwordsigned(0x007FFFFD, 0)
                end
                retest.expect(func).to.fail()
            end)
        end)
        ...
        retest.describe('readqword', function()
            retest.it('roundtrip_works', function()
                memory.writeqword(TEST_ADDR_OOB, { 0x80000000, 0x00000000 })
                retest.expect(memory.readqword(TEST_ADDR_OOB)).to.equal({ 0x80000000, 0 })
                retest.expect(memory.readqword(TEST_ADDR_IB)).to.equal({ 0x80000000, 0 })
            end)
            retest.it('errors_on_oob', function()
                local func = function()
                    memory.readqword(0x007FFFF9)
                end
                retest.expect(func).to.fail()
            end)
        end)
        ...
        retest.describe('writeqword', function()
            retest.it('errors_on_oob', function()
                local func = function()
                    memory.writeqword(0x007FFFF9, { 0, 0 })
                end
                retest.expect(func).to.fail()
            end)
        end)
        ...
        retest.describe('readqwordsigned', function()
            retest.it('roundtrip_works', function()
                memory.writeqword(TEST_ADDR_OOB, { 0x80000000, 0x00000000 })
                -- NOTE: This behavior is inconsistent with what you'd expect. We're intentionally testing for the compat-preserving case.
                local v = memory.readqwordsigned(TEST_ADDR_OOB)
                retest.expect(v[1]).to.equal(2147483648)
                retest.expect(v[2]).to.equal(0)
            end)
            retest.it('errors_on_oob', function()
                local func = function()
                    memory.readqwordsigned(0x007FFFF9)
                end
                retest.expect(func).to.fail()
            end)
        end)
        ...
        retest.describe('readsize', function()
            retest.it('roundtrip_works_1', function()
                memory.writesize(TEST_ADDR_OOB, 1, 0x80)
                retest.expect(memory.readsize(TEST_ADDR_OOB, 1)).to.equal(0x80)
                retest.expect(memory.readsize(TEST_ADDR_IB, 1)).to.equal(0x80)
            end)
            retest.it('roundtrip_works_-1', function()
                memory.writesize(TEST_ADDR_OOB, -1, 0x80)
                retest.expect(memory.readsize(TEST_ADDR_OOB, -1)).to.equal(-128)
                retest.expect(memory.readsize(TEST_ADDR_IB, -1)).to.equal(-128)
            end)
            retest.it('roundtrip_works_2', function()
                memory.writesize(TEST_ADDR_OOB, 2, 0xFFFF)
                retest.expect(memory.readsize(TEST_ADDR_OOB, 2)).to.equal(0xFFFF)
                retest.expect(memory.readsize(TEST_ADDR_IB, 2)).to.equal(0xFFFF)
            end)
            retest.it('roundtrip_works_-2', function()
                memory.writesize(TEST_ADDR_OOB, -2, 0xFFFF)
                retest.expect(memory.readsize(TEST_ADDR_OOB, -2)).to.equal(-1)
                retest.expect(memory.readsize(TEST_ADDR_IB, -2)).to.equal(-1)
            end)
            retest.it('roundtrip_works_4', function()
                memory.writesize(TEST_ADDR_OOB, 4, 0xFFFFFFFF)
                retest.expect(memory.readsize(TEST_ADDR_OOB, 4)).to.equal(0xFFFFFFFF)
                retest.expect(memory.readsize(TEST_ADDR_IB, 4)).to.equal(0xFFFFFFFF)
            end)
            retest.it('roundtrip_works_-4', function()
                memory.writesize(TEST_ADDR_OOB, -4, 0xFFFFFFFF)
                retest.expect(memory.readsize(TEST_ADDR_OOB, -4)).to.equal(-1)
                retest.expect(memory.readsize(TEST_ADDR_IB, -4)).to.equal(-1)
            end)
            retest.it('roundtrip_works_8', function()
                memory.writesize(TEST_ADDR_OOB, 8, { 0x80000000, 0x00000000 })
                local v = memory.readsize(TEST_ADDR_OOB, 8)
                retest.expect(v[1]).to.equal(0x80000000)
                retest.expect(v[2]).to.equal(0x00000000)
                v = memory.readsize(TEST_ADDR_IB, 8)
                retest.expect(v[1]).to.equal(0x80000000)
                retest.expect(v[2]).to.equal(0x00000000)
            end)
            retest.it('roundtrip_works_-8', function()
                memory.writesize(TEST_ADDR_OOB, -8, { 0x80000000, 0x00000000 })
                local v = memory.readsize(TEST_ADDR_OOB, -8)
                retest.expect(v[1]).to.equal(2147483648)
                retest.expect(v[2]).to.equal(0)
            ...
            retest.it('errors_on_oob_2', function()
                local func = function()
                    memory.readsize(0x007FFFFD, 2)
                end
                retest.expect(func).to.fail()
            end)
            retest.it('errors_on_oob_-2', function()
                local func = function()
                    memory.readsize(0x007FFFFD, -2)
                end
                retest.expect(func).to.fail()
            end)
            retest.it('errors_on_oob_4', function()
                local func = function()
                    memory.readsize(0x007FFFFD, 4)
                end
                retest.expect(func).to.fail()
            end)
            retest.it('errors_on_oob_-4', function()
                local func = function()
                    memory.readsize(0x007FFFFD, -4)
                end
                retest.expect(func).to.fail()
            end)
            retest.it('errors_on_oob_8', function()
                local func = function()
                    memory.readsize(0x007FFFF9, 8)
                end
                retest.expect(func).to.fail()
            end)
            retest.it('errors_on_oob_-8', function()
                local func = function()
                    memory.readsize(0x007FFFF9, -8)
                end
                retest.expect(func).to.fail()
