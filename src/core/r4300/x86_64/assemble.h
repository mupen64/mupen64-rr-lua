...
static inline void put8(unsigned char octet)
{
  ...
  {
    *inst_pointer = (uint8_t*)realloc_exec(*inst_pointer, max_code_length, max_code_length+8192);
...
static inline int rel_r15_offset(void *dest, const char *op_name)
{
    ...
    {
        g_core->logger->error("Error: destination %lx more than 2GB away from r15 base %lx in %s()", (long) dest, (long) reg, op_name);
        asm(" int $3; ");
    }
