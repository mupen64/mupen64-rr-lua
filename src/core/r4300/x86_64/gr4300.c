...
static void genupdate_count(unsigned int addr)
{
   ...
   add_m32rel_xreg32((unsigned int*)(&core_Count), EAX);
#else
...
{
   mov_xreg32_m32rel(EAX, &next_interrupt);
   cmp_xreg32_m32rel(EAX, &core_Count);
   ja_rj(0);
   jump_start_rel8();
...
void gendebug(void)
{
   ...
   mov_memoffs32_eax((unsigned int *) &vr_op);
   mov_reg64_imm64(RAX, (unsigned long long) Debugger::core_compare);
   call_reg64(RAX);
...
{
   mov_m32rel_imm32(&delay_slot, 1);
   recompile_opcode();
   
   free_all_registers();
   genupdate_count(dst->addr+4);
   
   mov_m32rel_imm32(&delay_slot, 0);
}
...
{
   mov_xreg32_m32rel(EBX, &next_interrupt);
   cmp_xreg32_m32rel(EBX, &core_Count);
   ja_rj(0);
   jump_start_rel8();
...
void genj(void)
{
   ...
   mov_m32rel_imm32(&last_addr, naddr);
...
void genj_idle(void)
{
   ...
   sub_xreg32_m32rel(EAX, (unsigned int *)(&core_Count));
   cmp_reg32_imm8(EAX, 3);
   jbe_rj(12);

   and_eax_imm32(0xFFFFFFFC);  // 5
   add_m32rel_xreg32((unsigned int *)(&core_Count), EAX); // 7
...
void gentest(void)
{
   ...
   mov_m32rel_imm32(&last_addr, dst->addr + (dst-1)->f.i.immediate*4);
   gencheck_interupt((unsigned long long) (dst + (dst-1)->f.i.immediate));
   jmp(dst->addr + (dst-1)->f.i.immediate*4);

   jump_end_rel32();

   mov_m32rel_imm32(&last_addr, dst->addr + 4);
...
void gentest_idle(void)
{
   ...
   sub_xreg32_m32rel(reg, (unsigned int *)(&core_Count));
   cmp_reg32_imm8(reg, 3);
   jbe_rj(0);
   jump_start_rel8();
   
   and_reg32_imm32(reg, 0xFFFFFFFC);
   add_m32rel_xreg32((unsigned int *)(&core_Count), reg);
...
void gensb(void)
{
   ...
   mov_m8rel_xreg8((unsigned char *)(&g_byte), CL); // 7
...
void gencheck_cop1_unusable(void)
{
   ...
   test_m32rel_imm32((unsigned int*)&core_Status, 0x20000000);
   jne_rj(0);
   jump_start_rel8();
