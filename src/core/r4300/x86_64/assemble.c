...
void init_assembler(void *block_jumps_table, int block_jumps_number, void *block_riprel_table, int block_riprel_number)
{
  ...
  {
    jumps_table = (jump_table*)malloc(512*sizeof(jump_table));
    jumps_number = 0;
    max_jumps_number = 512;
  }
  ...
  {
    riprel_table = (riprelative_table*)block_riprel_table;
  ...
  {
    riprel_table = (riprelative_table*)malloc(512 * sizeof(riprelative_table));
    riprel_number = 0;
    max_riprel_number = 512;
  }
}
...
void passe2(precomp_instr *dest, int start, int end, precomp_block *block)
{
  ...
  for (i = 0; i < jumps_number; i++)
  {
    ...
    else
    {
      ...
      *((int *) (block->code + jmp_offset_loc)) = (int) jump_rel_offset;
      ...
      {
        g_core->logger->error("assembler pass2 error: offset too big for relative jump from %lx to %lx", 
  ...
  for (i = 0; i < riprel_number; i++)
  {
    ...
    {
      g_core->logger->error("assembler pass2 error: offset too big between mem target: %lx and code position: %lx",
...
void jump_end_rel8(void)
{
  ...
  {
    g_core->logger->error("Error: 8-bit relative jump too long! From %x to %x", g_jump_start8, jump_end);
    asm(" int $3; ");
  }
