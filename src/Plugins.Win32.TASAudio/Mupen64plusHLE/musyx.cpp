...
static void update_base_vol(struct hle_t *hle, int32_t *base_vol, uint32_t voice_mask, uint32_t last_sample_ptr,
                            uint8_t mask_15, uint32_t ptr_24);

static void init_subframes_v1(musyx_t *musyx);
static void init_subframes_v2(musyx_t *musyx);

static uint32_t voice_stage(struct hle_t *hle, musyx_t *musyx, uint32_t voice_ptr, uint32_t last_sample_ptr);
...
static void load_samples_PCM16(struct hle_t *hle, uint32_t voice_ptr, int16_t *samples, unsigned *segbase,
                               unsigned *offset);
static void load_samples_ADPCM(struct hle_t *hle, uint32_t voice_ptr, int16_t *samples, unsigned *segbase,
                               unsigned *offset);

static void adpcm_decode_frames(struct hle_t *hle, int16_t *dst, const uint8_t *src, const int16_t *table,
...
static void mix_voice_samples(struct hle_t *hle, musyx_t *musyx, uint32_t voice_ptr, const int16_t *samples,
                              unsigned segbase, unsigned offset, uint32_t last_sample_ptr);

static void sfx_stage(struct hle_t *hle, mix_sfx_with_main_subframes_t mix_sfx_with_main_subframes, musyx_t *musyx,
                      uint32_t sfx_ptr, uint16_t idx);
...
static void interleave_stage_v2(struct hle_t *hle, musyx_t *musyx, uint16_t mask_16, uint32_t ptr_18, uint32_t ptr_1c,
                                uint32_t output_ptr);
...
static int32_t dot4(const int16_t *x, const int16_t *y)
{
    ...
    for (i = 0; i < 4; ++i) accu = clamp_s16(accu + (((int32_t)x[i] * (int32_t)y[i]) >> 15));
...
void musyx_v1_task(struct hle_t *hle)
{
    ...
    HleVerboseMessage(hle->user_defined, "musyx_v1_task: *data=%x, #SF=%d", sfd_ptr, sfd_count);
...
void musyx_v2_task(struct hle_t *hle)
{
    ...
    HleVerboseMessage(hle->user_defined, "musyx_v2_task: *data=%x, #SF=%d", sfd_ptr, sfd_count);
    ...
    for (;;)
    {
        ...
        {
            /* TODO */
            HleWarnMessage(hle->user_defined, "ptr_10=%08x mask_14=%02x ptr_24=%08x", ptr_10, mask_14, ptr_24);
        }
        ...
        if (mask_16) interleave_stage_v2(hle, &musyx, mask_16, ptr_18, ptr_1c, ptr_20);
                            ...
                            uint8_t mask_15, uint32_t ptr_24)
{
    ...
    HleVerboseMessage(hle->user_defined, "BEFORE: base_vol = %08x %08x %08x %08x", base_vol[0], base_vol[1],
                      base_vol[2], base_vol[3]);
    ...
    if (voice_mask != 0)
    {
        for (i = 0, mask = 1; i < MAX_VOICES; ++i, mask <<= 1, last_sample_ptr += 8)
        {
            ...
            for (k = 0; k < 4; ++k) base_vol[k] += (int16_t)*dram_u16(hle, last_sample_ptr + k * 2);
        }
    }
    ...
    if (mask_15 != 0)
    {
        for (i = 0, mask = 1; i < 4; ++i, mask <<= 1, ptr_24 += 8)
        {
            ...
            for (k = 0; k < 4; ++k) base_vol[k] += (int16_t)*dram_u16(hle, ptr_24 + k * 2);
        }
    }

    /* apply 3% decay */
    for (k = 0; k < 4; ++k) base_vol[k] = (base_vol[k] * 0x0000f850) >> 16;

    HleVerboseMessage(hle->user_defined, "AFTER: base_vol = %08x %08x %08x %08x", base_vol[0], base_vol[1], base_vol[2],
                      base_vol[3]);
}
...
static void init_subframes_v2(musyx_t *musyx)
{
    ...
    for (k = 0; k < 4; ++k) values[k] = clamp_s16(musyx->base_vol[k]);
    ...
    for (i = 0; i < SUBFRAME_SIZE; ++i)
    {
        ...
        for (k = 0; k < 4; ++k) *subframes[k]++ = values[k];
    }
}
                               ...
                               unsigned *offset)
{
    ...
    *offset = u8_3e;
    ...
    if (u16_42 != 0) dma_cat16(hle, (uint16_t *)samples, voice_ptr + VOICE_CATSRC_1);
}
                                ...
                                uint8_t count, uint8_t skip_samples)
{
    ...
    HleVerboseMessage(hle->user_defined, "ADPCM decode: count=%d, skip=%d", count, skip_samples);
                              ...
                              unsigned segbase, unsigned offset, uint32_t last_sample_ptr)
{
    ...
    const int16_t *const sample_restart =
        samples + (restart_point & 0x7fff) + ((restart_point & 0x8000) != 0 ? 0x000 : segbase);
    ...
    HleVerboseMessage(hle->user_defined,
                      ...
                      segbase, u16_4e, pitch_q16, pitch_shift, end_point, restart_point, v4_env[0], v4_env[1],
                      v4_env[2], v4_env[3], v4_env_step[0], v4_env_step[1], v4_env_step[2], v4_env_step[3]);
    ...
    HleVerboseMessage(hle->user_defined, "last_sample = %04x %04x %04x %04x", v4[0], v4[1], v4[2], v4[3]);
}
                      ...
                      uint32_t sfx_ptr, uint16_t idx)
{
    ...
    HleVerboseMessage(hle->user_defined, "cbuffer: ptr=%08x length=%x", cbuffer_ptr, cbuffer_length);

    HleVerboseMessage(hle->user_defined, "fir4: hgain=%04x hcoeff=%04x %04x %04x %04x", fir4_hgain, fir4_hcoeffs[0],
                      fir4_hcoeffs[1], fir4_hcoeffs[2], fir4_hcoeffs[3]);

    HleVerboseMessage(hle->user_defined,
                      "tap count=%d\n"
                      "delays: %08x %08x %08x %08x %08x %08x %08x %08x\n"
                      "gains:  %04x %04x %04x %04x %04x %04x %04x %04x",
                      tap_count, tap_delays[0], tap_delays[1], tap_delays[2], tap_delays[3], tap_delays[4],
                      tap_delays[5], tap_delays[6], tap_delays[7], tap_gains[0], tap_gains[1], tap_gains[2],
                      tap_gains[3], tap_gains[4], tap_gains[5], tap_gains[6], tap_gains[7]);
...
static void mix_subframes(int16_t *y, const int16_t *x, int16_t hgain)
{
    ...
    for (i = 0; i < SUBFRAME_SIZE; ++i) mix_samples(&y[i], x[i], hgain);
}
                                ...
                                uint32_t output_ptr)
{
    ...
    HleVerboseMessage(hle->user_defined, "mask_16=%04x ptr_18=%08x ptr_1c=%08x output_ptr=%08x", mask_16, ptr_18,
                      ptr_1c, output_ptr);
