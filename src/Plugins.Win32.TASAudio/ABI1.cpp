/*
 * Copyright (c) 2026, Mupen64 maintainers, contributors, and original authors (Azimer, Bobby Smiles).
 *
 * SPDX-License-Identifier: GPL-2.0-or-later
 */

#include "audiohle.h"

p_func ABI1[NUM_ABI_COMMANDS] = {
    SPNOOP,    ADPCM,  CLEARBUFF,  ENVMIXER, LOADBUFF, RESAMPLE, SAVEBUFF, UNKNOWN, SETBUFF, SETVOL, DMEMMOVE,
    LOADADPCM, MIXER,  INTERLEAVE, POLEF,    SETLOOP,  SPNOOP,   SPNOOP,   SPNOOP,  SPNOOP,  SPNOOP, SPNOOP,
    SPNOOP,    SPNOOP, SPNOOP,     SPNOOP,   SPNOOP,   SPNOOP,   SPNOOP,   SPNOOP,  SPNOOP,  SPNOOP,
};

p_func ABI1GE[NUM_ABI_COMMANDS] = {
    SPNOOP,    ADPCM,  CLEARBUFF,  ENVMIXER_GE, LOADBUFF, RESAMPLE, SAVEBUFF, UNKNOWN, SETBUFF, SETVOL, DMEMMOVE,
    LOADADPCM, MIXER,  INTERLEAVE, POLEF,       SETLOOP,  SPNOOP,   SPNOOP,   SPNOOP,  SPNOOP,  SPNOOP, SPNOOP,
    SPNOOP,    SPNOOP, SPNOOP,     SPNOOP,      SPNOOP,   SPNOOP,   SPNOOP,   SPNOOP,  SPNOOP,  SPNOOP,
};
