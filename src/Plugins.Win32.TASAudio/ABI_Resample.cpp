...
void RESAMPLE()
{
    ...
    */
    ...
    if ((Flags & 0x1) == 0)
    {
        ...
        for (int x = 0; x < 4; x++) src[MES(srcPtr + x)] = ((u16 *)DRAM)[MES(addy / 2 + x)];
        Accum = *(u16 *)(DRAM + addy + 10);
    }
    else
    {
        for (int x = 0; x < 4; x++) src[MES(srcPtr + x)] = 0; //*(u16 *)(rdram + HES(addy + x));
    }
    ...
    for (int x = 0; x < 4; x++) ((u16 *)DRAM)[MES(addy / 2 + x)] = src[MES(srcPtr + x)];
...
void RESAMPLE2()
{
    ...
    if (addy > 1024 * 1024 * 8) addy = t9 & 0xffffff;
