...
void on_emu_launched_changed(bool value)
{
    g_main_ctx.dispatcher->invoke([=] {
        ...
        {
            g_view_logger->info(
                "[View] Restoring window size to {}x{}...", g_config.window_width, g_config.window_height);
...
void on_vis_since_input_poll_exceeded()
{
    ...
    if (g_config.silent_mode || DialogService::show_ask_dialog(VIEW_DLG_LAG_EXCEEDED,
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT Message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_SYSKEYDOWN: {
        ...
        if (const auto keycode = HotkeyUtils::vk_to_keycode(wParam); keycode.has_value()) args.keycode = *keycode;
        args.pressed = true;
        args.repeat = repeat;
