...
#pragma clang diagnostic pop
    ...
    struct is_same_arithmetic_type : public std::integral_constant<bool, (std::is_integral<Rep1>::value && std::is_integral<Rep2>::value) || (std::is_floating_point<Rep1>::value && std::is_floating_point<Rep2>::value)> {};
...
namespace detail
{
    ...
    void write_fractional_seconds(OutputIt& out, Duration d, int precision = -1)
    {
        ...
        const auto subseconds = std::chrono::treat_as_floating_point<typename subsecond_precision::rep>::value ? fractional.count() : fmt_duration_cast<subsecond_precision>(fractional).count();
    ...
    void write_floating_seconds(memory_buffer& buf, Duration duration, int num_fractional_digits = -1)
    {
        ...
        fmt::format_to(std::back_inserter(buf), FMT_STRING("{:.{}f}"), std::fmod(val * static_cast<rep>(Duration::period::num) / static_cast<rep>(Duration::period::den), static_cast<rep>(60)), num_fractional_digits);
    }
...
struct formatter<std::chrono::time_point<std::chrono::system_clock, Duration>, Char> : formatter<std::tm, Char> {
    ...
    auto format(std::chrono::time_point<std::chrono::system_clock, Duration> val, FormatContext& ctx) const -> decltype(ctx.out())
    {
        ...
        if (detail::const_check(period::num != 1 || period::den != 1 || std::is_floating_point<typename Duration::rep>::value))
        {
            ...
            auto subsecs = detail::fmt_duration_cast<Duration>(epoch - detail::fmt_duration_cast<std::chrono::seconds>(epoch));
