...
namespace spdlog
{
    namespace details
    {
        ...
        std::future<void> SPDLOG_INLINE thread_pool::post_flush(async_logger_ptr&& worker_ptr, async_overflow_policy overflow_policy)
        {
            std::promise<void> promise;
            std::future<void> future = promise.get_future();
            post_async_msg_(async_msg(std::move(worker_ptr), async_msg_type::flush, std::move(promise)), overflow_policy);
            return future;
        }
