...
public:
  ...
  string_stream
  operator()(std::initializer_list<char const *const> init_list) const;
...
inline bool
parser::operator[](std::initializer_list<char const *const> init_list) const {
...
template <typename T>
string_stream
parser::operator()(std::initializer_list<char const *const> init_list,
                   T &&def_val) const {
  for (auto &name : init_list) {
    auto optIt = params_.find(trim_leading_dashes(name));
    if (params_.end() != optIt)
...
inline void
parser::add_param(std::initializer_list<const char *const> init_list) {
...
inline void
parser::add_params(std::initializer_list<char const *const> init_list) {
  for (auto &name : init_list)
    registeredParams_.insert(trim_leading_dashes(name));
}
...
inline multimap_iteration_wrapper
parser::params(std::string const &name) const {
