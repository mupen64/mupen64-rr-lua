...
### Action path normalization changed (breaking)

Normalized action paths have changed to:

- replace `.` with a special character
- normalize paths to `kebab-case.kebab-case.*`
- not have the `---` separator suffix
- not have the `#` menu-hidden prefix

Scripts aren't likely to be relying on this.
