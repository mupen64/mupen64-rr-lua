...
std::filesystem::path get_sram_path() { return std::format(L"{}{} {}.sra", get_saves_directory().wstring(), string_to_wstring((const char*)ROM_HEADER.nom), country_code_to_country_name(ROM_HEADER.Country_code)); }

std::filesystem::path get_eeprom_path() { return std::format(L"{}{} {}.eep", get_saves_directory().wstring(), string_to_wstring((const char*)ROM_HEADER.nom), country_code_to_country_name(ROM_HEADER.Country_code)); }

std::filesystem::path get_flashram_path() { return std::format(L"{}{} {}.fla", get_saves_directory().wstring(), string_to_wstring((const char*)ROM_HEADER.nom), country_code_to_country_name(ROM_HEADER.Country_code)); }

std::filesystem::path get_mempak_path() { return std::format(L"{}{} {}.mpk", get_saves_directory().wstring(), string_to_wstring((const char*)ROM_HEADER.nom), country_code_to_country_name(ROM_HEADER.Country_code)); }
    ...
    else
    {
        ...
        if (paddr)
        {
            if ((paddr & 0x1FFFFFFF) >= 0x10000000)
            {
                ...
                recompile_block((int32_t*)rom + ((((paddr - (PC->addr - blocks[PC->addr >> 12]->start)) & 0x1FFFFFFF) - 0x10000000) >> 2), blocks[PC->addr >> 12], PC->addr);
            }
            else
                recompile_block((int32_t*)(rdram + (((paddr - (PC->addr - blocks[PC->addr >> 12]->start)) & 0x1FFFFFFF) >> 2)), blocks[PC->addr >> 12], PC->addr);
        }
        else
...
inline void jump_to_func()
{
    ...
    if (invalid_code[addr >> 12])
    {
        ...
        init_block((int32_t*)(rdram + (((paddr - (addr - blocks[addr >> 12]->start)) & 0x1FFFFFFF) >> 2)), blocks[addr >> 12]);
    }
...
{
    g_core_logger->info("PC={:#08x}:{:#08x}", (uint32_t)(PC->addr), (uint32_t)(rdram[(PC->addr & 0xFFFFFF) / 4]));
    for (int32_t j = 0; j < 16; j++)
        g_core_logger->info("reg[{}]:{:#08x}{:#08x}        reg[{}]:{:#08x}{:#08x}", j, (uint32_t)(reg[j] >> 32), (uint32_t)reg[j], j + 16, (uint32_t)(reg[j + 16] >> 32), (uint32_t)reg[j + 16]);
    g_core_logger->info("hi:{:#08x}{:#08x}        lo:{:#08x}{:#08x}", (uint32_t)(hi >> 32), (uint32_t)hi, (uint32_t)(lo >> 32), (uint32_t)lo);
...
CoreResult vr_start_rom_impl(std::filesystem::path path)
{
    ...
    if (path.extension() == ".m64")
    {
        ...
        const auto matching_rom = FrontendService::find_available_rom(
        [&](auto header)
        {
    ...
    if (video_plugin.get() && audio_plugin.get() && input_plugin.get() && rsp_plugin.get() && video_plugin->path() == g_config.selected_video_plugin && audio_plugin->path() == g_config.selected_audio_plugin && input_plugin->path() == g_config.selected_input_plugin && rsp_plugin->path() == g_config.selected_rsp_plugin)
    {
