...
void print_queue() {
  ...
  aux = q;
  while (aux != NULL) {
    g_core_logger->info("Count:{:#06x}, {:#06x}", (uint32_t)aux->count,
                        aux->type);
    aux = aux->next;
  }
  g_core_logger->info("------------------");
...
/// <summary>
/// Adds interrupt to queue that will fire after certain amount of time (Count
/// register cycles)
/// </summary>
...
void add_interrupt_event(int32_t type, uint32_t delay) {
  ...
  while (aux->next != NULL &&
         (!before_event(count, aux->next->count, aux->next->type) || special))
    aux = aux->next;
...
void remove_interrupt_event() {
  ...
  q = aux;
  if (q != NULL &&
      (q->count > core_Count || (core_Count - q->count) < 0x80000000))
    next_interrupt = q->count;
  else
    next_interrupt = 0;
}
...
/// <summary>
/// Check if event of this type already exists in queue (for some reason this is
/// forbidden)
/// </summary>
...
void check_interrupt() {
  ...
  // same thing is done in gen_interrupt, but this is needed so that the bit is
  // cleared properly
  if (MI_register.mi_intr_reg & MI_register.mi_intr_mask_reg)
    core_Cause = (core_Cause | 0x400) &
                 0xFFFFFF83; // 0x400 is CAUSE_IP3 (rcp interrupt pending)
  else
    core_Cause &= ~0x400;
  if ((core_Status & 7) != 1)
    return;
...
void gen_interrupt() {
  ...
  case VI_INT: {
    ...
    // NOTE: It's ok to not update screen when lagging, doesn't cause any
    // obvious issues
    auto skip =
        (g_config.skip_rendering_lag && lag_count > 1) || g_vr_frame_skipped;
    auto update = FrontendService::get_prefers_no_render_skip()
                      ? true
                      : (screen_invalidated ? !skip : false);

    // NOTE: When frame advancing, screen_invalidated has a higher change of
    // being false despite the fact it should be true The update-limiting logic
    // doesn't apply in frameadvance because there are no high-frequency updates
    ...
    if (vi_register.vi_v_sync == 0)
      ...
      vi_register.vi_delay =
          ((vi_register.vi_v_sync + 1) * (1500 * g_config.counter_factor));
    // this is the place
  ...
  case COMPARE_INT: // game can set Compare register to some value, and make a
                    // timer like that
  ...
  case CHECK_INT: // fake interrupt used to trigger exception handler (when
  ...
  case PI_INT:
    ...
    pi_register.read_pi_status_reg &=
        ~3; // PI_STATUS_DMA_BUSY | PI_STATUS_IO_BUSY clear
    break;
  ...
  case AI_INT:
    ...
    if (ai_register.ai_status & 0x80000000) // full
    {
      ...
      // apparently this should be moved to when AIlen is changed (when sound
      // start splaying)
    } else {
      remove_interrupt_event();
  ...
  if (type == COMPARE_INT)
    core_Cause =
        (core_Cause | 0x8000) & 0xFFFFFF83; // COMPARE interrupt to be pending
  if (type != CHECK_INT) {
    if (MI_register.mi_intr_reg & MI_register.mi_intr_mask_reg)
      core_Cause = (core_Cause | 0x400) & 0xFFFFFF83; // RCP interrupt is
                                                      // pending
    else
      return;
    if ((core_Status & 7) != 1)
