...
void display_error(const char *txt) {
  ...
  if (interpcore) {
    ...
    if (!strcmp(txt, "PC"))
      g_core_logger->info("{:#06x} - {:#06x}", (int32_t)interp_addr,
                          *(int32_t *)&comp_reg[0]);
  } else {
  ...
  g_core_logger->info("{:#06x}, {:#06x}", (uint32_t)reg_cop0[9],
                      (uint32_t)comp_reg2[9]);
  ...
  if (!strcmp(txt, "gpr")) {
    for (i = 0; i < 32; i++) {
      if (reg[i] != comp_reg[i])
        g_core_logger->info("reg[{}]={:#08x} != reg[{}]={:#08x}", i, reg[i], i,
                            comp_reg[i]);
    }
  }
  if (!strcmp(txt, "cop0")) {
    for (i = 0; i < 32; i++) {
      if (reg_cop0[i] != comp_reg2[i])
        g_core_logger->info("reg_cop0[{}]={:#06x} != reg_cop0[{}]={:#06x}", i,
                            (uint32_t)reg_cop0[i], i, (uint32_t)comp_reg2[i]);
    }
  }
  /*for (i=0; i<32; i++)
    {
...
void check_input_sync(unsigned char *value) {
  ...
  if (compare_core_mode == 3 ? dynacore || interpcore
                             : compare_core_mode == 1) {
    fread(value, 4, 1, f);
  } else {
    fwrite(value, 4, 1, f);
...
void compare_core() {
  ...
  if (compare_core_mode == 3 ? dynacore || interpcore
    ...
    // if (reg_cop0[9] > 0x6800000) g_core_logger->info("PC={:#06x}",
    // (int32_t)PC->addr);
    if (pipe_opened != 2) {
      if (f)
        fclose(f);
