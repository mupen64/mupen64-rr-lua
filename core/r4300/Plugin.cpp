...
static int32_t __cdecl dummy_initiateAudio(AUDIO_INFO Audio_Info) { return 1; }
...
#define FUNC(target, type, fallback, name)                                     \
  target = (type)PlatformService::get_function_in_module(handle, name);        \
  if (!target)                                                                 \
  target = fallback
...
void load_gfx(void *handle) {
  ...
  FUNC(readScreen, READSCREEN,
       (READSCREEN)PlatformService::get_function_in_module(handle,
                                                           "ReadScreen2"),
       "ReadScreen");
  ...
  FUNC(fBGetFrameBufferInfo, FBGETFRAMEBUFFERINFO, dummy_fBGetFrameBufferInfo,
       "FBGetFrameBufferInfo");

  // ReadScreen returns a plugin-allocated buffer which must be freed by the
  // same CRT
  DllCrtFree = PlatformService::get_free_function_in_module(handle);

  gfx_info.hWnd = FrontendService::get_main_window_handle();
  gfx_info.hStatusBar = g_config.is_statusbar_enabled
                            ? FrontendService::get_statusbar_handle()
                            : nullptr;
...
void load_input(uint16_t version, void *handle) {
  ...
  FUNC(controllerCommand, CONTROLLERCOMMAND, dummy_controllerCommand,
       "ControllerCommand");
  FUNC(getKeys, GETKEYS, dummy_getKeys, "GetKeys");
  FUNC(setKeys, SETKEYS, dummy_setKeys, "SetKeys");
  if (version == 0x0101) {
    FUNC(initiateControllers, INITIATECONTROLLERS, dummy_initiateControllers,
         "InitiateControllers");
  } else {
    FUNC(old_initiateControllers, OLD_INITIATECONTROLLERS, nullptr,
         "InitiateControllers");
  }
  ...
  if (version == 0x0101) {
    ...
    old_initiateControllers(FrontendService::get_main_window_handle(),
                            Controls);
  }
}
...
void load_audio(void *handle) {
  ...
  FUNC(aiDacrateChanged, AIDACRATECHANGED, dummy_aiDacrateChanged,
       "AiDacrateChanged");
...
std::pair<std::wstring, std::unique_ptr<Plugin>>
Plugin::create(std::filesystem::path path) {
  uint64_t error = 0;
  void *module = PlatformService::load_library(path.wstring().c_str(), &error);

  if (module == nullptr) {
    return std::make_pair(std::format(L"LoadLibrary (code {})", error),
                          nullptr);
  }

  const auto get_dll_info =
      (GETDLLINFO)PlatformService::get_function_in_module(module, "GetDllInfo");
...
Plugin::~Plugin() { PlatformService::free_library((void *)m_module); }
...
void Plugin::config() {
  ...
  case PluginType::Video: {
    if (!emu_launched) {
      ...
      auto initiateGFX = (INITIATEGFX)PlatformService::get_function_in_module(
          (void *)m_module, "InitiateGFX");
      if (initiateGFX && !initiateGFX(dummy_gfx_info)) {
        FrontendService::show_dialog(L"Couldn't initialize video plugin.",
                                     L"Core",
                                     FrontendService::DialogType::Information);
      }
    }

    auto dllConfig = (DLLCONFIG)PlatformService::get_function_in_module(
        (void *)m_module, "DllConfig");
    if (dllConfig)
      dllConfig(FrontendService::get_plugin_config_parent_handle());

    if (!emu_launched) {
      auto closeDLL_gfx = (CLOSEDLL_GFX)PlatformService::get_function_in_module(
          (void *)m_module, "CloseDLL");
      if (closeDLL_gfx)
        closeDLL_gfx();
    }
    break;
  ...
  case PluginType::Audio: {
    if (!emu_launched) {
      auto initiateAudio =
          (INITIATEAUDIO)PlatformService::get_function_in_module(
              (void *)m_module, "InitiateAudio");
      if (initiateAudio && !initiateAudio(dummy_audio_info)) {
        FrontendService::show_dialog(L"Couldn't initialize audio plugin.",
                                     L"Core",
                                     FrontendService::DialogType::Information);
      }
    }
    ...
    if (!emu_launched) {
      auto closeDLL_audio =
          (CLOSEDLL_AUDIO)PlatformService::get_function_in_module(
              (void *)m_module, "CloseDLL");
      if (closeDLL_audio)
        closeDLL_audio();
    }
  ...
  case PluginType::Input: {
    if (!emu_launched) {
      if (m_version == 0x0101) {
        auto initiateControllers =
            (INITIATECONTROLLERS)PlatformService::get_function_in_module(
                (void *)m_module, "InitiateControllers");
        if (initiateControllers)
          initiateControllers(dummy_control_info);
      } else {
        auto old_initiateControllers =
            (OLD_INITIATECONTROLLERS)PlatformService::get_function_in_module(
                (void *)m_module, "InitiateControllers");
    ...
    if (!emu_launched) {
      auto closeDLL_input =
          (CLOSEDLL_INPUT)PlatformService::get_function_in_module(
              (void *)m_module, "CloseDLL");
      if (closeDLL_input)
        closeDLL_input();
    }
  ...
  case PluginType::RSP: {
    if (!emu_launched) {
      auto initiateRSP = (INITIATERSP)PlatformService::get_function_in_module(
          (void *)m_module, "InitiateRSP");
      uint32_t i = 0;
      if (initiateRSP)
    ...
    if (!emu_launched) {
      auto closeDLL_RSP = (CLOSEDLL_RSP)PlatformService::get_function_in_module(
          (void *)m_module, "CloseDLL");
      if (closeDLL_RSP)
        closeDLL_RSP();
    }
...
void Plugin::test() {
  dllTest = (DLLTEST)PlatformService::get_function_in_module((void *)m_module,
                                                             "DllTest");
  if (dllTest)
    dllTest(FrontendService::get_plugin_config_parent_handle());
}

void Plugin::about() {
  dllAbout = (DLLABOUT)PlatformService::get_function_in_module((void *)m_module,
                                                               "DllAbout");
  if (dllAbout)
    dllAbout(FrontendService::get_plugin_config_parent_handle());
}
