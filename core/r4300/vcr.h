...
enum { MOVIE_START_FROM_SNAPSHOT = (1 << 0), MOVIE_START_FROM_NOTHING = (1 << 1), MOVIE_START_FROM_EEPROM = (1 << 2), MOVIE_START_FROM_EXISTING_SNAPSHOT = (1 << 3) };
...
typedef struct ExtendedMovieData {
    /**
     * Special authorship information, such as the program which created the
     * movie.
     */
...
typedef struct MovieHeader {
     ...
     * \remarks This field makes no sense, as this value can already be figured
     * out by checking controller flags post-load
     */
    ...
    /**
     * \brief Amount of input samples in the movie, ideally equal to
     * <c>(file_length - 1024) / 4</c>
     */
    ...
    /**
     * \brief Bitfield of flags for each controller. Sequence of present, mempak
     * and rumble bits with per-controller values respectively
     * (c1,c2,c3,c4,r1,...)
     */
    ...
    /**
     * \brief The name of the video plugin the movie was recorded with, as
     * specified by the plugin
     */
    char video_plugin_name[64];

    /**
     * \brief The name of the audio plugin the movie was recorded with, as
     * specified by the plugin
     */
    char audio_plugin_name[64];

    /**
     * \brief The name of the input plugin the movie was recorded with, as
     * specified by the plugin
     */
    char input_plugin_name[64];

    /**
     * \brief The name of the RSP plugin the movie was recorded with, as specified
     * by the plugin
     */
...
enum class e_task { idle, start_recording_from_reset, start_recording_from_snapshot, start_recording_from_existing_snapshot, recording, start_playback_from_reset, start_playback_from_snapshot, playback };
...
/**
 * \brief The movie freeze buffer, which is used to store the movie (with only
 * essential data) associated with a savestate inside the savestate.
 */
...
namespace VCR
{
     ...
     * If no seek operation is active, the first value is the current sample and the
     * second one is SIZE_MAX.
     */
     ...
     * \param pause_at_end Whether the emu should be paused when the seek operation
     *ends \return The operation result \remarks When the seek operation completes,
     *the SeekCompleted message will be sent
     *
    ...
    /**
     * \brief Converts a freeze buffer into a movie, trying to reconstruct as much
     * as possible \param freeze The freeze buffer to convert \param header The
     * generated header \param inputs The generated inputs \return The operation
     * result
     */
    ...
    /**
     * Gets the sample length of the current movie. If no movie is active, the
     * function returns UINT32_MAX.
     */
    uint32_t get_length_samples();

    /**
     * Gets the VI length of the current movie. If no movie is active, the function
     * returns UINT32_MAX.
     */
    uint32_t get_length_vis();

    /**
     * Gets the current VI in the current movie. If no movie is active, the function
     * returns -1.
     */
    ...
    /**
     * Begins a warp modification operation. A "warp modification operation" is the
     * changing of sample data which is temporally behind the current sample.
     *
     * The VCR engine will find the last common sample between the current input
     * buffer and the provided one. Then, the closest savestate prior to that sample
     * will be loaded and recording will be resumed with the modified inputs up to
     * the sample the function was called at.
     *
     * This operation is long-running and status is reported via the
     * WarpModifyStatusChanged message. A successful warp modify operation can be
     * detected by the status changing from warping to none with no errors
     * inbetween.
     *
     * If the provided buffer is identical to the current input buffer (in both
     * content and size), the operation will succeed with no seek.
     *
     * If the provided buffer is larger than the current input buffer and the first
     * differing input is after the current sample, the operation will succeed with
     * no seek. The input buffer will be overwritten with the provided buffer and
     * when the modified frames are reached in the future, they will be "applied"
     * like in playback mode.
     *
     * If the provided buffer is smaller than the current input buffer, the VCR
     * engine will seek to the last frame and otherwise perform the warp
     * modification as normal.
     *
     ...
     * Keys are frame numbers, values are unimportant and abscence of a seek
     * savestate at a frame is marked by the respective key not existing.
     */
    std::unordered_map<size_t, bool> get_seek_savestate_frames();

    /**
     * Gets whether a seek savestate exists at the specified frame.
     * The returned state changes when the <c>SeekSavestatesChanged</c> message is
     * sent.
     */
    bool has_seek_savestate_at_frame(size_t frame);

    /**
     * HACK: The VCR engine can prevent the core from pausing. Gets whether the core
