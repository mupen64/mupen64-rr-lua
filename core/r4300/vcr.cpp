...
constexpr auto RAWDATA_WARNING_MESSAGE = L"Warning: One of the active controllers of your input plugin is set to "
...
constexpr auto ROM_NAME_WARNING_MESSAGE = L"The movie was recorded on the rom '{}', but is being played back on "
                                          ...
                                          L"continue?";
constexpr auto ROM_COUNTRY_WARNING_MESSAGE = L"The movie was recorded on a rom with country {}, but is being played "
                                             L"back on {}.\r\nPlayback might desynchronize. Are you sure you want to "
                                             L"continue?";
...
constexpr auto WII_VC_MISMATCH_A_WARNING_MESSAGE = L"The movie was recorded with WiiVC mode enabled, but is being played back "
                                                   L"with it disabled.\r\nPlayback might desynchronize. Are you sure you want "
                                                   L"to continue?";
constexpr auto WII_VC_MISMATCH_B_WARNING_MESSAGE = L"The movie was recorded with WiiVC mode disabled, but is being played "
                                                   L"back with it enabled.\r\nPlayback might desynchronize. Are you sure you "
                                                   L"want to continue?";
constexpr auto OLD_MOVIE_EXTENDED_SECTION_NONZERO_MESSAGE = L"The movie was recorded prior to the extended format being available, but "
...
bool is_task_playback(const e_task task) { return task == e_task::start_playback_from_reset || task == e_task::start_playback_from_snapshot || task == e_task::playback; }

uint64_t get_rerecord_count() { return static_cast<uint64_t>(g_header.extended_data.rerecord_count) << 32 | g_header.rerecord_count; }
...
void vcr_create_n_frame_savestate(size_t frame)
{
    ...
    Savestates::do_memory({}, Savestates::Job::Save,
                          [frame](CoreResult result, const auto& buf)
                          {
                              std::scoped_lock lock(vcr_mutex);
...
void vcr_handle_starting_tasks(int32_t index, BUTTONS* input)
{
    if (g_task == e_task::start_recording_from_reset)
    {
        ...
        [clear_eeprom]
        {
            ...
            if (result != CoreResult::Ok)
            {
                FrontendService::show_dialog(L"Failed to reset the rom when initiating a from-start "
    ...
    if (g_task == e_task::start_playback_from_reset)
    {
        ...
        [clear_eeprom]
        {
            ...
            if (result != CoreResult::Ok)
            {
                FrontendService::show_dialog(L"Failed to reset the rom when playing back a from-start "
    ...
    *input = g_movie_inputs[m_current_sample + index];
    ...
    if (input->Value == 0xC000)
    {
        ...
        []
        {
            ...
            if (result != CoreResult::Ok)
            {
                FrontendService::show_dialog(L"Failed to reset the rom following a movie-invoked "
                                             L"reset.\nRecording will be stopped.",
                                             L"VCR", FrontendService::DialogType::Error);
                VCR::stop_all();
                g_reset_pending = false;
...
CoreResult VCR::start_record(std::filesystem::path path, uint16_t flags, std::string author, std::string description)
{
    ...
    if (flags & MOVIE_START_FROM_SNAPSHOT)
    {
        ...
        [](CoreResult result, auto)
        {
            ...
            if (result != CoreResult::Ok)
            {
                FrontendService::show_dialog(L"Failed to save savestate while starting "
                                             L"recording.\nRecording will be stopped.",
                                             L"VCR", FrontendService::DialogType::Error);
                VCR::stop_all();
                return;
    ...
    else if (flags & MOVIE_START_FROM_EXISTING_SNAPSHOT)
    {
        ...
        [=]
        {
            ...
            [](CoreResult result, auto)
            {
                ...
                if (result != CoreResult::Ok)
                {
                    FrontendService::show_dialog(L"Failed to load savestate while starting "
                                                 L"recording.\nRecording will be stopped.",
                                                 L"VCR", FrontendService::DialogType::Error);
                    VCR::stop_all();
                    return;
...
CoreResult vcr_stop_record()
{
    ...
    if (g_task == e_task::recording)
    {
        ...
        g_core_logger->info("[VCR] Recording stopped. Recorded %ld input samples", g_header.length_samples);
    }
...
int32_t check_warn_controllers(wchar_t* warning_str)
{
    for (int32_t i = 0; i < 4; ++i)
    {
        if (!Controls[i].Present && (g_header.controller_flags & CONTROLLER_X_PRESENT(i)))
        {
        ...
        else
        {
            ...
            if (Controls[i].Present && (Controls[i].Plugin != (int32_t)ControllerExtension::Rumblepak) && (g_header.controller_flags & CONTROLLER_X_RUMBLE(i)))
                wsprintf(warning_str,
                         ...
                         L"for this movie to play back correctly.\n",
                         (i + 1));
            if (Controls[i].Present && (Controls[i].Plugin != (int32_t)ControllerExtension::None) && !(g_header.controller_flags & (CONTROLLER_X_MEMPAK(i) | CONTROLLER_X_RUMBLE(i))))
                wsprintf(warning_str,
...
bool can_seek_to(size_t frame) { return frame <= g_header.length_samples && frame > 0; }
...
CoreResult vcr_begin_seek_impl(std::wstring str, bool pause_at_end, bool resume, bool warp_modify)
{
    ...
    if (g_task == e_task::playback)
    {
        ...
        if (g_config.seek_savestate_interval != 0)
        {
            ...
            [=]
            {
                Savestates::do_memory(g_seek_savestates[closest_key], Savestates::Job::Load,
...
CoreResult VCR::begin_seek(std::wstring str, bool pause_at_end) { return vcr_begin_seek_impl(str, pause_at_end, true, false); }
...
bool task_is_playback(e_task task) { return task == e_task::playback || task == e_task::start_playback_from_reset || task == e_task::start_playback_from_snapshot; }

bool task_is_recording(e_task task) { return task == e_task::recording || task == e_task::start_recording_from_reset || task == e_task::start_recording_from_existing_snapshot || task == e_task::start_recording_from_snapshot; }
...
uint32_t VCR::get_length_samples() { return VCR::get_task() == e_task::idle ? UINT32_MAX : g_header.length_samples; }

uint32_t VCR::get_length_vis() { return VCR::get_task() == e_task::idle ? UINT32_MAX : g_header.length_vis; }

int32_t VCR::get_current_vi() { return VCR::get_task() == e_task::idle ? -1 : m_current_vi; }
...
CoreResult VCR::begin_warp_modify(const std::vector<BUTTONS>& inputs)
{
    ...
    {
        g_core_logger->info("[VCR] First different frame is in the future (current sample: {}, "
...
e_warp_modify_status VCR::get_warp_modify_status() { return g_warp_modify_status; }

size_t VCR::get_warp_modify_first_difference_frame() { return get_warp_modify_status() == e_warp_modify_status::warping ? g_warp_modify_first_difference_frame : SIZE_MAX; }
...
bool VCR::has_seek_savestate_at_frame(const size_t frame) { return g_seek_savestates.contains(frame); }
...
{
    Messenger::broadcast(Messenger::Message::TaskChanged, g_task);
    Messenger::subscribe(Messenger::Message::ResetRequested, [](std::any) { user_requested_reset = true; });
}
