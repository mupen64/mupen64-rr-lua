...
static void (*recomp_tlb[64])(void) = {
    RSV, RTLBR, RTLBWI, RSV, RSV, RSV, RTLBWR, RSV, RTLBP, RSV, RSV, RSV,   RSV,
...
static void (*recomp_cop0[32])(void) = {
    RMFC0, RSV, RSV, RSV, RMTC0, RSV,  RSV, RSV, RSV, RSV, RSV,
...
static void (*recomp_s[64])(void) = {
    RADD_S,     RSUB_S,     RMUL_S,     RDIV_S,     RSQRT_S,   RABS_S,
    RMOV_S,     RNEG_S,     RROUND_L_S, RTRUNC_L_S, RCEIL_L_S, RFLOOR_L_S,
    RROUND_W_S, RTRUNC_W_S, RCEIL_W_S,  RFLOOR_W_S, RSV,       RSV,
    RSV,        RSV,        RSV,        RSV,        RSV,       RSV,
    RSV,        RSV,        RSV,        RSV,        RSV,       RSV,
    RSV,        RSV,        RSV,        RCVT_D_S,   RSV,       RSV,
    RCVT_W_S,   RCVT_L_S,   RSV,        RSV,        RSV,       RSV,
    RSV,        RSV,        RSV,        RSV,        RSV,       RSV,
    RC_F_S,     RC_UN_S,    RC_EQ_S,    RC_UEQ_S,   RC_OLT_S,  RC_ULT_S,
    RC_OLE_S,   RC_ULE_S,   RC_SF_S,    RC_NGLE_S,  RC_SEQ_S,  RC_NGL_S,
...
static void (*recomp_d[64])(void) = {
    RADD_D,     RSUB_D,     RMUL_D,     RDIV_D,     RSQRT_D,   RABS_D,
    RMOV_D,     RNEG_D,     RROUND_L_D, RTRUNC_L_D, RCEIL_L_D, RFLOOR_L_D,
    RROUND_W_D, RTRUNC_W_D, RCEIL_W_D,  RFLOOR_W_D, RSV,       RSV,
    RSV,        RSV,        RSV,        RSV,        RSV,       RSV,
    RSV,        RSV,        RSV,        RSV,        RSV,       RSV,
    RSV,        RSV,        RCVT_S_D,   RSV,        RSV,       RSV,
    RCVT_W_D,   RCVT_L_D,   RSV,        RSV,        RSV,       RSV,
    RSV,        RSV,        RSV,        RSV,        RSV,       RSV,
    RC_F_D,     RC_UN_D,    RC_EQ_D,    RC_UEQ_D,   RC_OLT_D,  RC_ULT_D,
    RC_OLE_D,   RC_ULE_D,   RC_SF_D,    RC_NGLE_D,  RC_SEQ_D,  RC_NGL_D,
...
static void (*recomp_l[64])(void) = {
    RSV, RSV, RSV, RSV, RSV, RSV, RSV,      RSV,      RSV, RSV, RSV, RSV, RSV,
    RSV, RSV, RSV, RSV, RSV, RSV, RSV,      RSV,      RSV, RSV, RSV, RSV, RSV,
    RSV, RSV, RSV, RSV, RSV, RSV, RCVT_S_L, RCVT_D_L, RSV, RSV, RSV, RSV, RSV,
    RSV, RSV, RSV, RSV, RSV, RSV, RSV,      RSV,      RSV, RSV, RSV, RSV, RSV,
    RSV, RSV, RSV, RSV, RSV, RSV, RSV,      RSV,      RSV, RSV, RSV, RSV,
};
...
static void (*recomp_cop1[32])(void) = {
    RMFC1, RDMFC1, RCFC1, RSV, RMTC1, RDMTC1, RCTC1, RSV, RBC, RSV, RSV,
...
void init_block(int32_t *source, precomp_block *block) {
  ...
  if (!block->block) {
    block->block = (precomp_instr *)malloc(((length + 1) + (length >> 2)) *
    ...
    if (block->start >= 0x80000000 && block->end < 0xa0000000 &&
      ...
      if (!blocks[(block->start + 0x20000000) >> 12]) {
        blocks[(block->start + 0x20000000) >> 12] =
            (precomp_block *)malloc(sizeof(precomp_block));
        ...
        blocks[(block->start + 0x20000000) >> 12]->start =
            (block->start + 0x20000000) & ~0xFFF;
        blocks[(block->start + 0x20000000) >> 12]->end =
            ((block->start + 0x20000000) & ~0xFFF) + 0x1000;
      }
...
void recompile_block(int32_t *source, precomp_block *block, uint32_t func) {
  ...
  for (i = (func & 0xFFF) / 4; /*i<length &&*/ finished != 2; i++) {
    if (block->start < 0x80000000 || block->start >= 0xc0000000) {
      ...
      if (blocks[address2 >> 12]->block[(address2 & 0xFFF) / 4].ops ==
          NOTCOMPILED)
        blocks[address2 >> 12]->block[(address2 & 0xFFF) / 4].ops =
            NOTCOMPILED2;
    }
    ...
    if (i >= length - 2 + (length >> 2))
      finished = 2;
    if (i >= (length - 1) &&
        (block->start == 0xa4000000 || block->start >= 0xc0000000 ||
    ...
    if (/*i >= length &&*/
        ...
        !(i >= (length - 1) &&
          (block->start >= 0xc0000000 || block->end < 0x80000000)))
      finished = 1;
  }
  if (i >= length) {
    dst = block->block + i;
...
int32_t is_jump() {
  ...
  if (dyn)
    dynacore = 0;
  recomp_ops[((src >> 26) & 0x3F)]();
  if (dst->ops == J || dst->ops == J_OUT || dst->ops == J_IDLE ||
      dst->ops == JAL || dst->ops == JAL_OUT || dst->ops == JAL_IDLE ||
      dst->ops == BEQ || dst->ops == BEQ_OUT || dst->ops == BEQ_IDLE ||
      dst->ops == BNE || dst->ops == BNE_OUT || dst->ops == BNE_IDLE ||
      dst->ops == BLEZ || dst->ops == BLEZ_OUT || dst->ops == BLEZ_IDLE ||
      dst->ops == BGTZ || dst->ops == BGTZ_OUT || dst->ops == BGTZ_IDLE ||
      dst->ops == BEQL || dst->ops == BEQL_OUT || dst->ops == BEQL_IDLE ||
      dst->ops == BNEL || dst->ops == BNEL_OUT || dst->ops == BNEL_IDLE ||
      dst->ops == BLEZL || dst->ops == BLEZL_OUT || dst->ops == BLEZL_IDLE ||
      dst->ops == BGTZL || dst->ops == BGTZL_OUT || dst->ops == BGTZL_IDLE ||
      dst->ops == JR || dst->ops == JALR || dst->ops == BLTZ ||
      dst->ops == BLTZ_OUT || dst->ops == BLTZ_IDLE || dst->ops == BGEZ ||
      dst->ops == BGEZ_OUT || dst->ops == BGEZ_IDLE || dst->ops == BLTZL ||
      dst->ops == BLTZL_OUT || dst->ops == BLTZL_IDLE || dst->ops == BGEZL ||
      dst->ops == BGEZL_OUT || dst->ops == BGEZL_IDLE || dst->ops == BLTZAL ||
      dst->ops == BLTZAL_OUT || dst->ops == BLTZAL_IDLE || dst->ops == BGEZAL ||
      dst->ops == BGEZAL_OUT || dst->ops == BGEZAL_IDLE ||
      dst->ops == BLTZALL || dst->ops == BLTZALL_OUT ||
      dst->ops == BLTZALL_IDLE || dst->ops == BGEZALL ||
      dst->ops == BGEZALL_OUT || dst->ops == BGEZALL_IDLE || dst->ops == BC1F ||
      dst->ops == BC1F_OUT || dst->ops == BC1F_IDLE || dst->ops == BC1T ||
      dst->ops == BC1T_OUT || dst->ops == BC1T_IDLE || dst->ops == BC1FL ||
      dst->ops == BC1FL_OUT || dst->ops == BC1FL_IDLE || dst->ops == BC1TL ||
      dst->ops == BC1TL_OUT || dst->ops == BC1TL_IDLE)
    jump = 1;
  if (dyn)
    dynacore = 1;
  return jump;
  ...
  if ((addr >> 16) == 0xa400)
    ...
    if (paddr) {
      if ((paddr & 0x1FFFFFFF) >= 0x10000000) {
        recompile_block(
            (int32_t *)rom +
                ((((paddr - (addr - blocks[addr >> 12]->start)) & 0x1FFFFFFF) -
                  0x10000000) >>
                 2),
            blocks[addr >> 12], addr);
      } else {
