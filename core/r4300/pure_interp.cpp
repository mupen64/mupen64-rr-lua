...
static void (*interp_special[64])(void) = {SLL, NI, SRL, SRA, SLLV, NI, SRLV, SRAV, JR, JALR, NI, NI, SYSCALL, NI, NI, SYNC, MFHI, MTHI, MFLO, MTLO, DSLLV, NI, DSRLV, DSRAV, MULT, MULTU, DIV, DIVU, DMULT, DMULTU, DDIV, DDIVU, ADD, ADDU, SUB, SUBU, AND, OR, XOR, NOR, NI, NI, SLT, SLTU, DADD, DADDU, DSUB, DSUBU, NI, NI, NI, NI, TEQ, NI, NI, NI, DSLL, NI, DSRL, DSRA, DSLL32, NI, DSRL32, DSRA32};
...
static void BLTZAL()
{
    ...
    if ((&core_irs) != (reg + 31))
    {
        ...
        g_core_logger->error("erreur dans bltzal");
    last_addr = interp_addr;
    if (next_interrupt <= core_Count)
        gen_interrupt();
}
...
static void BGEZAL()
{
    ...
    if ((&core_irs) != (reg + 31))
    {
        ...
        g_core_logger->error("erreur dans bgezal");
    last_addr = interp_addr;
    if (next_interrupt <= core_Count)
        gen_interrupt();
}
...
static void BLTZALL()
{
    ...
    if ((&core_irs) != (reg + 31))
    {
        ...
        g_core_logger->error("erreur dans bltzall");
    last_addr = interp_addr;
    if (next_interrupt <= core_Count)
        gen_interrupt();
}
...
static void BGEZALL()
{
    ...
    if ((&core_irs) != (reg + 31))
    {
        ...
        g_core_logger->info("erreur dans bgezall");
    last_addr = interp_addr;
    if (next_interrupt <= core_Count)
        gen_interrupt();
}

static void (*interp_regimm[32])(void) = {BLTZ, BGEZ, BLTZL, BGEZL, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, BLTZAL, BGEZAL, BLTZALL, BGEZALL, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI};
...
static void TLBWI()
{
    ...
    tlb_e[core_Index & 0x3F].end_even = tlb_e[core_Index & 0x3F].start_even + (tlb_e[core_Index & 0x3F].mask << 12) + 0xFFF;
    tlb_e[core_Index & 0x3F].phys_even = tlb_e[core_Index & 0x3F].pfn_even << 12;

    if (tlb_e[core_Index & 0x3F].v_even)
    {
        if (tlb_e[core_Index & 0x3F].start_even < tlb_e[core_Index & 0x3F].end_even && !(tlb_e[core_Index & 0x3F].start_even >= 0x80000000 && tlb_e[core_Index & 0x3F].end_even < 0xC0000000) && tlb_e[core_Index & 0x3F].phys_even < 0x20000000)
        {
            for (i = tlb_e[core_Index & 0x3F].start_even; i < tlb_e[core_Index & 0x3F].end_even; i++)
                tlb_LUT_r[i >> 12] = 0x80000000 | (tlb_e[core_Index & 0x3F].phys_even + (i - tlb_e[core_Index & 0x3F].start_even));
            if (tlb_e[core_Index & 0x3F].d_even)
                for (i = tlb_e[core_Index & 0x3F].start_even; i < tlb_e[core_Index & 0x3F].end_even; i++)
                    tlb_LUT_w[i >> 12] = 0x80000000 | (tlb_e[core_Index & 0x3F].phys_even + (i - tlb_e[core_Index & 0x3F].start_even));
        }
    }

    tlb_e[core_Index & 0x3F].start_odd = tlb_e[core_Index & 0x3F].end_even + 1;
    tlb_e[core_Index & 0x3F].end_odd = tlb_e[core_Index & 0x3F].start_odd + (tlb_e[core_Index & 0x3F].mask << 12) + 0xFFF;
    tlb_e[core_Index & 0x3F].phys_odd = tlb_e[core_Index & 0x3F].pfn_odd << 12;

    if (tlb_e[core_Index & 0x3F].v_odd)
    {
        if (tlb_e[core_Index & 0x3F].start_odd < tlb_e[core_Index & 0x3F].end_odd && !(tlb_e[core_Index & 0x3F].start_odd >= 0x80000000 && tlb_e[core_Index & 0x3F].end_odd < 0xC0000000) && tlb_e[core_Index & 0x3F].phys_odd < 0x20000000)
        {
            for (i = tlb_e[core_Index & 0x3F].start_odd; i < tlb_e[core_Index & 0x3F].end_odd; i++)
                tlb_LUT_r[i >> 12] = 0x80000000 | (tlb_e[core_Index & 0x3F].phys_odd + (i - tlb_e[core_Index & 0x3F].start_odd));
            if (tlb_e[core_Index & 0x3F].d_odd)
                for (i = tlb_e[core_Index & 0x3F].start_odd; i < tlb_e[core_Index & 0x3F].end_odd; i++)
                    tlb_LUT_w[i >> 12] = 0x80000000 | (tlb_e[core_Index & 0x3F].phys_odd + (i - tlb_e[core_Index & 0x3F].start_odd));
        }
    }
    interp_addr += 4;
}
...
static void TLBWR()
{
    ...
    if (tlb_e[core_Random].v_even)
    {
        if (tlb_e[core_Random].start_even < tlb_e[core_Random].end_even && !(tlb_e[core_Random].start_even >= 0x80000000 && tlb_e[core_Random].end_even < 0xC0000000) && tlb_e[core_Random].phys_even < 0x20000000)
        {
            for (i = tlb_e[core_Random].start_even; i < tlb_e[core_Random].end_even; i++)
                tlb_LUT_r[i >> 12] = 0x80000000 | (tlb_e[core_Random].phys_even + (i - tlb_e[core_Random].start_even));
            if (tlb_e[core_Random].d_even)
                for (i = tlb_e[core_Random].start_even; i < tlb_e[core_Random].end_even; i++)
                    tlb_LUT_w[i >> 12] = 0x80000000 | (tlb_e[core_Random].phys_even + (i - tlb_e[core_Random].start_even));
        }
    }
    ...
    if (tlb_e[core_Random].v_odd)
    {
        if (tlb_e[core_Random].start_odd < tlb_e[core_Random].end_odd && !(tlb_e[core_Random].start_odd >= 0x80000000 && tlb_e[core_Random].end_odd < 0xC0000000) && tlb_e[core_Random].phys_odd < 0x20000000)
        {
            for (i = tlb_e[core_Random].start_odd; i < tlb_e[core_Random].end_odd; i++)
                tlb_LUT_r[i >> 12] = 0x80000000 | (tlb_e[core_Random].phys_odd + (i - tlb_e[core_Random].start_odd));
            if (tlb_e[core_Random].d_odd)
                for (i = tlb_e[core_Random].start_odd; i < tlb_e[core_Random].end_odd; i++)
                    tlb_LUT_w[i >> 12] = 0x80000000 | (tlb_e[core_Random].phys_odd + (i - tlb_e[core_Random].start_odd));
        }
    }
    interp_addr += 4;
}
...
static void TLBP()
{
    ...
    for (i = 0; i < 32; i++)
    {
        if (((tlb_e[i].vpn2 & (~tlb_e[i].mask)) == (((core_EntryHi & 0xFFFFE000) >> 13) & (~tlb_e[i].mask))) && ((tlb_e[i].g) || (tlb_e[i].asid == (core_EntryHi & 0xFF))))
        {
            core_Index = i;
            break;
        }
...
static void (*interp_tlb[64])(void) = {NI, TLBR, TLBWI, NI, NI, NI, TLBWR, NI, TLBP, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, ERET, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI};
...
static void TLB() { interp_tlb[(vr_op & 0x3F)](); }

static void (*interp_cop0[32])(void) = {MFC0, NI, NI, NI, MTC0, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, TLB, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI};
...
static void C_EQ_D()
{
    if (!isnan(*reg_cop1_double[core_cffs]) && !isnan(*reg_cop1_double[core_cfft]) && *reg_cop1_double[core_cffs] == *reg_cop1_double[core_cfft])
        FCR31 |= 0x800000;
    else
        FCR31 &= ~0x800000;
    interp_addr += 4;
...
static void (*interp_cop1_d[64])(void) = {ADD_D, SUB_D, MUL_D, DIV_D, SQRT_D, ABS_D, MOV_D, NEG_D, ROUND_L_D, TRUNC_L_D, CEIL_L_D, FLOOR_L_D, ROUND_W_D, TRUNC_W_D, CEIL_W_D, FLOOR_W_D, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, CVT_S_D, NI, NI, NI, CVT_W_D, CVT_L_D, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, C_F_D, C_UN_D, C_EQ_D, C_UEQ_D, C_OLT_D, C_ULT_D, C_OLE_D, C_ULE_D, C_SF_D, C_NGLE_D, C_SEQ_D, C_NGL_D, C_LT_D, C_NGE_D, C_LE_D, C_NGT_D};
...
static void (*interp_cop1_w[64])(void) = {NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, CVT_S_W, CVT_D_W, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI};
...
static void (*interp_cop1_l[64])(void) = {NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, CVT_S_L, CVT_D_L, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI};
...
static void CTC1()
{
    ...
    // if ((FCR31 >> 7) & 0x1F) g_core_logger->info("FPU Exception enabled :
    // {:#06x}\n", 				   (int32_t)((FCR31 >> 7) & 0x1F));
    set_rounding();
    interp_addr += 4;
}

static void BC() { interp_cop1_bc[(vr_op >> 16) & 3](); }

static void S() { interp_cop1_s[(vr_op & 0x3F)](); }

static void D() { interp_cop1_d[(vr_op & 0x3F)](); }

static void W() { interp_cop1_w[(vr_op & 0x3F)](); }

static void L() { interp_cop1_l[(vr_op & 0x3F)](); }

static void (*interp_cop1[32])(void) = {MFC1, DMFC1, CFC1, NI, MTC1, DMTC1, CTC1, NI, BC, NI, NI, NI, NI, NI, NI, NI, S, D, NI, NI, W, L, NI, NI, NI, NI, NI, NI, NI, NI, NI, NI};

static void SPECIAL() { interp_special[(vr_op & 0x3F)](); }

static void REGIMM() { interp_regimm[((vr_op >> 16) & 0x1F)](); }
...
static void COP0() { interp_cop0[((vr_op >> 21) & 0x1F)](); }
...
static void CACHE() { interp_addr += 4; }
...
void (*interp_ops[64])(void) = {SPECIAL, REGIMM, J, JAL, BEQ, BNE, BLEZ, BGTZ, ADDI, ADDIU, SLTI, SLTIU, ANDI, ORI, XORI, LUI, COP0, COP1, NI, NI, BEQL, BNEL, BLEZL, BGTZL, DADDI, DADDIU, LDL, LDR, NI, NI, NI, NI, LB, LH, LWL, LW, LBU, LHU, LWR, LWU, SB, SH, SWL, SW, SDL, SDR, SWR, CACHE, LL, LWC1, NI, NI, NI, LDC1, NI, LD, SC, SWC1, NI, NI, NI, SDC1, NI, SD};
...
void prefetch()
{
    ...
    /*if ((debug_count+Count) > 0xb70000)
      ...
      (int32_t)(Count+debug_count), interp_addr, op, line);*/
    //}
...
void pure_interpreter()
{
    ...
    while (!stop)
    {
        ...
        compare_core();
#endif
        // if (Count > 0x2000000) g_core_logger->info("inter:%x,%x",
        // interp_addr,op); if ((Count+debug_count) > 0xabaa2c) stop=1;
        interp_ops[((vr_op >> 26) & 0x3F)]();
