...
static void add_jump(uint32_t pc_addr, uint32_t mi_addr) {
  if (jumps_number == max_jumps_number) {
    max_jumps_number += 1000;
    jumps_table = (jump_table *)realloc(jumps_table,
                                        max_jumps_number * sizeof(jump_table));
  }
...
void passe2(precomp_instr *dest, int32_t start, int32_t end,
  ...
  for (i = 0; i < jumps_number; i++) {
    ...
    if (dest[(jumps_table[i].mi_addr - dest[0].addr) / 4]
            .reg_cache_infos.need_map) {
      addr_dest = (uint32_t)dest[(jumps_table[i].mi_addr - dest[0].addr) / 4]
                      .reg_cache_infos.jump_wrapper;
...
void push_reg32(int32_t reg32) { put8(0x50 + reg32); }

void pop_reg32(int32_t reg32) { put8(0x58 + reg32); }
...
void dec_reg32(int32_t reg32) { put8(0x48 + reg32); }

void inc_reg32(int32_t reg32) { put8(0x40 + reg32); }
