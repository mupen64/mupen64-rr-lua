...
void free_register(int32_t reg) {
  ...
  if (last_access[reg] != NULL && r64[reg] != -1 &&
      (int32_t)reg_content[reg] != (int32_t)reg_content[r64[reg]] - 4) {
  ...
  while (last <= dst) {
    ...
    if (last_access[reg] != NULL && r64[reg] != -1) {
      if (dirty[r64[reg]])
        last->reg_cache_infos.needed_registers[r64[reg]] =
            reg_content[r64[reg]];
      else
    ...
    for (i = 0; i < 8; i++) {
      if (last_access[i] != NULL && reg_content[i] == addr) {
        ...
        if (r64[i] != -1) {
          ...
          while (last <= dst) {
            last->reg_cache_infos.needed_registers[r64[i]] =
                reg_content[r64[i]];
            last++;
          }
...
void set_64_register_state(int32_t reg1, int32_t reg2, uint32_t *addr,
                           int32_t d) {
...
void unlock_register(int32_t reg) { last_access[reg] = NULL; }
...
// 0xC3 ret
// total : 62 bytes
void build_wrapper(precomp_instr *instr, unsigned char *code,
                   precomp_block *block) {
  int32_t i;
  int32_t j = 0;
...
void build_wrappers(precomp_instr *instr, int32_t start, int32_t end,
                    precomp_block *block) {
  int32_t i, reg;
  ;
  for (i = start; i < end; i++) {
