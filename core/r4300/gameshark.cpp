...
std::optional<std::shared_ptr<Gameshark::Script>>
Gameshark::Script::compile(const std::wstring &code) {
  ...
  for (const auto &line : lines) {
    ...
    if (serial) {
      g_core_logger->info("[GS] Compiling {} serial byte writes...",
                          serial_count);

      for (size_t i = 0; i < serial_count; ++i) {
        // Madghostek: warning, assumes that serial codes are writing bytes,
        // which seems to match pj64 Madghostek: if not, change WB to WW
        script->m_instructions.emplace_back(std::make_tuple(false, [=] {
          StoreRDRAMSafe<uint8_t>(address + serial_offset * i,
                                  val + serial_diff * i);
          return true;
        }));
      }
    ...
    if (opcode == L"80" || opcode == L"A0") {
      ...
      script->m_instructions.emplace_back(std::make_tuple(
          true, [=] { return LoadRDRAMSafe<uint16_t>(address) == val; }));
    } else if (opcode == L"D2") {
