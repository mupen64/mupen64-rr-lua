...
void print_pif()
{
    int32_t i;
    for (i = 0; i < (64 / 8); i++)
        g_core_logger->info("{:#06x} {:#06x} {:#06x} {:#06x} | {:#06x} {:#06x} {:#06x} {:#06x}", PIF_RAMb[i * 8 + 0], PIF_RAMb[i * 8 + 1], PIF_RAMb[i * 8 + 2], PIF_RAMb[i * 8 + 3], PIF_RAMb[i * 8 + 4], PIF_RAMb[i * 8 + 5], PIF_RAMb[i * 8 + 6], PIF_RAMb[i * 8 + 7]);
    // getchar();
}
#endif
...
{
    unsigned char init[] = {};
    int32_t i, j;
    for (i = 0; i < 4; i++)
    {
...
void update_pif_read()
{
    ...
    int32_t i = 0, channel = 0;
    bool once = emu_paused | frame_advancing | g_vr_wait_before_input_poll; // used to pause only once during
                                                                            // controller routine
    bool stAllowed = true; // used to disallow .st being loaded after any
                           // controller has already been read
#ifdef DEBUG_PIF
    ...
    while (i < 0x40)
    {
        ...
        default:
            ...
            if (!(PIF_RAMb[i] & 0xC0)) // mask error bits (isn't this wrong? error
                                       // bits are on i+1???)
            {
                if (channel < 4)
                {
                    ...
                    if (once && channel <= controllerRead && (&PIF_RAMb[i])[2] == 1)
                    {
                        ...
                        while (emu_paused)
                        {
                            ...
                            // COMPAT: Old input lua expects atvi to be called when paused
                            // (due to a bug in the invalidation)...
                            LuaService::call_vi();
                    ...
                    if (g_st_old)
                    {
                        // if old savestate, don't fetch controller (matches old behaviour),
                        // makes delay fix not work for that st but syncs all m64s
                    ...
                    if (Controls[channel].Present && Controls[channel].RawData && VCR::get_task() == e_task::idle)
                    {
