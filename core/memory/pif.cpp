...
void EepromCommand(uint8_t *Command) {
  ...
  default:
    g_core_logger->warn("unknown command in EepromCommand : {:#06x}",
                        Command[2]);
  }
}
...
void update_pif_write() {
  ...
  if (PIF_RAMb[0x3F] > 1) {
    ...
    case 0x08:
      ...
      break;
    default:
      g_core_logger->info("error in update_pif_write : {:#06x}",
                          PIF_RAMb[0x3F]);
    }
    return;
  }
...
void update_pif_read() {
  ...
  bool once = emu_paused | frame_advancing |
              g_vr_wait_before_input_poll; // used to pause only once during
                                           // controller routine
