...
void dma_pi_read()
{
    ...
    else if (g_config.use_summercart)
    {
        ...
        else if (pi_register.pi_cart_addr_reg >= 0x10000000 && pi_register.pi_cart_addr_reg < 0x14000000 && summercart.cfg_rom_write)
        {
            for (i = 0; i < longueur; i++)
            {
                uint32_t dram = (pi_register.pi_dram_addr_reg + i);
...
void dma_pi_write()
{
        ...
        if (pi_register.pi_cart_addr_reg >= 0x08000000 && pi_register.pi_cart_addr_reg < 0x08010000)
        {
            if (use_flashram != 1)
            {
                ...
                for (i = 0; i < (pi_register.pi_wr_len_reg & 0xFFFFFF) + 1; i++)
                    ((unsigned char*)rdram)[(pi_register.pi_dram_addr_reg + i) ^ S8] = sram[(((pi_register.pi_cart_addr_reg - 0x08000000) & 0xFFFF) + i) ^ S8];
                use_flashram = -1;
            }
            else
                dma_read_flashram();
    ...
    if (g_config.use_summercart && pi_register.pi_cart_addr_reg >= 0x1ffe0000 && pi_register.pi_cart_addr_reg < 0x1fff0000)
    {
        for (i = 0; i < longueur; i++)
        {
            uint32_t dram = (pi_register.pi_dram_addr_reg + i);
    ...
    longueur = (pi_register.pi_dram_addr_reg + longueur) > 0x7FFFFF ? (0x7FFFFF - pi_register.pi_dram_addr_reg) : longueur;
    ...
    if (!interpcore)
    {
        for (i = 0; i < longueur; i++)
        {
            ...
            ((unsigned char*)rdram)[(pi_register.pi_dram_addr_reg + i) ^ S8] = !Debugger::get_dma_read_enabled() ? 0xFF : rom[(((pi_register.pi_cart_addr_reg - 0x10000000) & 0x3FFFFFF) + i) ^ S8];
    ...
    /*for (i=0; i<=((longueur+0x800)>>12); i++)
      invalid_code[(((pi_register.pi_dram_addr_reg&0xFFFFFF)|0x80000000)>>12)+i] =
      1;*/
...
void dma_sp_write()
{
    ...
    if ((sp_register.sp_mem_addr_reg & 0x1000) > 0)
    {
        for (i = 0; i < ((sp_register.sp_rd_len_reg & 0xFFF) + 1); i++)
            ((unsigned char*)(SP_IMEM))[((sp_register.sp_mem_addr_reg & 0xFFF) + i) ^ S8] = ((unsigned char*)(rdram))[((sp_register.sp_dram_addr_reg & 0xFFFFFF) + i) ^ S8];
    }
    else
    {
        for (i = 0; i < ((sp_register.sp_rd_len_reg & 0xFFF) + 1); i++)
            ((unsigned char*)(SP_DMEM))[((sp_register.sp_mem_addr_reg & 0xFFF) + i) ^ S8] = ((unsigned char*)(rdram))[((sp_register.sp_dram_addr_reg & 0xFFFFFF) + i) ^ S8];
    }
}
...
void dma_sp_read()
{
    ...
    if ((sp_register.sp_mem_addr_reg & 0x1000) > 0)
    {
        for (i = 0; i < ((sp_register.sp_wr_len_reg & 0xFFF) + 1); i++)
            ((unsigned char*)(rdram))[((sp_register.sp_dram_addr_reg & 0xFFFFFF) + i) ^ S8] = ((unsigned char*)(SP_IMEM))[((sp_register.sp_mem_addr_reg & 0xFFF) + i) ^ S8];
    }
    else
    {
        for (i = 0; i < ((sp_register.sp_wr_len_reg & 0xFFF) + 1); i++)
            ((unsigned char*)(rdram))[((sp_register.sp_dram_addr_reg & 0xFFFFFF) + i) ^ S8] = ((unsigned char*)(SP_DMEM))[((sp_register.sp_mem_addr_reg & 0xFFF) + i) ^ S8];
    }
}
...
void dma_si_read()
{
    ...
    if (!g_st_skip_dma) // st already did this, see savestates.cpp, we still copy
                        // pif ram tho because it has new inputs
    {
        update_count();
        add_interrupt_event(SI_INT, /*0x100*/ 0x900);
    }
