...
static void update_MI_init_mode_reg() {
  ...
  MI_register.mi_init_mode_reg =
      ((MI_register.init_length) | (MI_register.init_mode << 7) |
...
static void update_MI_intr_mask_reg() {
  ...
  MI_register.mi_intr_mask_reg =
      ((MI_register.SP_intr_mask) | (MI_register.SI_intr_mask << 1) |
       (MI_register.AI_intr_mask << 2) | (MI_register.VI_intr_mask << 3) |
       (MI_register.PI_intr_mask << 4) | (MI_register.DP_intr_mask << 5));
}
...
void update_SP() {
  ...
  sp_register.sp_status_reg =
      ((sp_register.halt) | (sp_register.broke << 1) |
       (sp_register.dma_busy << 2) | (sp_register.dma_full << 3) |
       (sp_register.io_full << 4) | (sp_register.single_step << 5) |
       (sp_register.intr_break << 6) | (sp_register.signal0 << 7) |
       (sp_register.signal1 << 8) | (sp_register.signal2 << 9) |
       (sp_register.signal3 << 10) | (sp_register.signal4 << 11) |
       (sp_register.signal5 << 12) | (sp_register.signal6 << 13) |
      ...
      if (fBGetFrameBufferInfo && fBRead && fBWrite &&
        ...
        for (i = 0; i < 6; i++) {
          if (frameBufferInfos[i].addr) {
            ...
            int32_t end = start +
                          frameBufferInfos[i].width *
                              frameBufferInfos[i].height *
                              frameBufferInfos[i].size -
                          1;
            start = start >> 16;
            end = end >> 16;
            for (j = start; j <= end; j++) {
...
void update_DPC() {
  ...
  dpc_register.dpc_status =
      ((dpc_register.xbus_dmem_dma) | (dpc_register.freeze << 1) |
       (dpc_register.flush << 2) | (dpc_register.start_glck << 3) |
       (dpc_register.tmem_busy << 4) | (dpc_register.pipe_busy << 5) |
       (dpc_register.cmd_busy << 6) | (dpc_register.cbuf_busy << 7) |
       (dpc_register.dma_busy << 8) | (dpc_register.end_valid << 9) |
...
void read_rdramFB() {
  ...
  for (i = 0; i < 6; i++) {
    if (frameBufferInfos[i].addr) {
      int32_t start = frameBufferInfos[i].addr & 0x7FFFFF;
      int32_t end = start +
                    frameBufferInfos[i].width * frameBufferInfos[i].height *
                        frameBufferInfos[i].size -
                    1;
...
void read_rsp_memd() {
  if (*address_low < 0x1000) {
    ...
    *rdword =
        ((uint64_t)(*(uint32_t *)(SP_IMEMb + (*address_low & 0xFFF))) << 32) |
...
void read_rspd() {
  *rdword =
      ((uint64_t)(*readrsp[*address_low]) << 32) | *readrsp[*address_low + 4];
}
...
void read_dpd() {
  *rdword =
      ((uint64_t)(*readdp[*address_low]) << 32) | *readdp[*address_low + 4];
}
...
void write_dpb() {
  ...
  case 0xe:
  case 0xf:
    *((unsigned char *)&dpc_register.w_dpc_status + ((*address_low & 3) ^ S8)) =
        g_byte;
    update_DPC();
  case 0x8:
  case 0x9:
...
void read_dpsd() {
  *rdword =
      ((uint64_t)(*readdps[*address_low]) << 32) | *readdps[*address_low + 4];
}
...
void read_mid() {
  *rdword =
      ((uint64_t)(*readmi[*address_low]) << 32) | *readmi[*address_low + 4];
}
...
void read_vi() {
  ...
  case 0x10:
    ...
    vi_register.vi_current =
        (vi_register.vi_delay - (next_vi - core_Count)) / 1500;
...
void read_vid() {
  ...
  *rdword =
      ((uint64_t)(*readvi[*address_low]) << 32) | *readvi[*address_low + 4];
}
...
void write_vih() {
  ...
  case 0x2:
    temp = vi_register.vi_status;
    *((uint16_t *)((unsigned char *)&temp + ((*address_low & 3) ^ S16))) =
        hword;
    if (vi_register.vi_status != temp) {
      vi_register.vi_status = temp;
      viStatusChanged();
...
void read_ai() {
  ...
  case 0x4:
    ...
    if (ai_register.current_delay != 0 && get_event(AI_INT) != 0 &&
        (get_event(AI_INT) - core_Count) < 0x80000000)
      *rdword = ((get_event(AI_INT) - core_Count) *
                 (int64_t)ai_register.current_len) /
...
void read_aib() {
  ...
  case 0x7:
    update_count();
    if (ai_register.current_delay != 0 && get_event(AI_INT) != 0)
      len = ((get_event(AI_INT) - core_Count) *
             (int64_t)ai_register.current_len) /
            ai_register.current_delay;
    else
      len = 0;
...
void read_aih() {
  ...
  case 0x6:
    ...
    if (ai_register.current_delay != 0 && get_event(AI_INT) != 0)
      ...
      len = 0;
    *rdword =
        *((uint16_t *)((unsigned char *)&len + ((*address_low & 3) ^ S16)));
    return;
    break;
  }
...
void read_aid() {
  ...
  *rdword =
      ((uint64_t)(*readai[*address_low]) << 32) | *readai[*address_low + 4];
}
...
void write_ai() {
  ...
  case 0x4:
    ...
    case 0x59: {
      ...
      if (f)
        delay = ((uint64_t)ai_register.ai_len * vi_register.vi_delay * 50) /
                (f * 4);
    } break;
    case 0x37:
    case 0x41:
    ...
    case 0x4a: {
      ...
      if (f)
        delay = ((uint64_t)ai_register.ai_len * vi_register.vi_delay * 60) /
                (f * 4);
    } break;
    }
  ...
  case 0x10:
    if (ai_register.ai_dacrate != word) {
      ...
      case 0x59:
        ...
        Messenger::broadcast(Messenger::Message::DacrateChanged,
                             SystemType::PAL);
        break;
      case 0x37:
      case 0x41:
...
void read_pid() {
  *rdword =
      ((uint64_t)(*readpi[*address_low]) << 32) | *readpi[*address_low + 4];
}
...
void write_pib() {
  ...
  case 0xa:
  case 0xb:
    *((unsigned char *)&pi_register.pi_rd_len_reg + ((*address_low & 3) ^ S8)) =
        g_byte;
    dma_pi_read();
    return;
    break;
  ...
  case 0xe:
  case 0xf:
    *((unsigned char *)&pi_register.pi_wr_len_reg + ((*address_low & 3) ^ S8)) =
        g_byte;
    dma_pi_write();
    return;
    break;
...
void read_rid() {
  *rdword =
      ((uint64_t)(*readri[*address_low]) << 32) | *readri[*address_low + 4];
}
...
void read_sid() {
  *rdword =
      ((uint64_t)(*readsi[*address_low]) << 32) | *readsi[*address_low + 4];
}
...
void write_sib() {
  ...
  case 0x2:
  case 0x3:
    *((unsigned char *)&si_register.si_dram_addr + ((*address_low & 3) ^ S8)) =
        g_byte;
    return;
    break;
  case 0x4:
...
#endif
  *rdword = ((uint64_t)sl(*((uint32_t *)(PIF_RAMb + (address & 0x7FF) - 0x7C0)))
             << 32) |
...
void read_sc_regd() {
  FrontendService::show_dialog(L"read_sc_regd not supported by RCP", L"Core",
