...
static void update_MI_init_mode_reg()
{
    ...
    MI_register.mi_init_mode_reg = ((MI_register.init_length) | (MI_register.init_mode << 7) | (MI_register.ebus_test_mode << 8) | (MI_register.RDRAM_reg_mode << 9));
}
...
static void update_MI_intr_mask_reg()
{
    ...
    MI_register.mi_intr_mask_reg = ((MI_register.SP_intr_mask) | (MI_register.SI_intr_mask << 1) | (MI_register.AI_intr_mask << 2) | (MI_register.VI_intr_mask << 3) | (MI_register.PI_intr_mask << 4) | (MI_register.DP_intr_mask << 5));
}
...
void update_SP()
{
    ...
    sp_register.sp_status_reg = ((sp_register.halt) | (sp_register.broke << 1) | (sp_register.dma_busy << 2) | (sp_register.dma_full << 3) | (sp_register.io_full << 4) | (sp_register.single_step << 5) | (sp_register.intr_break << 6) | (sp_register.signal0 << 7) | (sp_register.signal1 << 8) | (sp_register.signal2 << 9) | (sp_register.signal3 << 10) | (sp_register.signal4 << 11) | (sp_register.signal5 << 12) | (sp_register.signal6 << 13) | (sp_register.signal7 << 14));
    ...
    if (!sp_register.halt && !sp_register.broke)
    {
        ...
        if (SP_DMEM[0xFC0 / 4] == 1)
        {
            ...
            // NOTE: we increment this here, and not in vcr_updatescreen, since
            // invocation of vcr_updatescreen depends on vi interrupts, which dont get
            // generated if we skip rsp if that happens, then we never increment
...
void update_DPC()
{
    ...
    dpc_register.dpc_status = ((dpc_register.xbus_dmem_dma) | (dpc_register.freeze << 1) | (dpc_register.flush << 2) | (dpc_register.start_glck << 3) | (dpc_register.tmem_busy << 4) | (dpc_register.pipe_busy << 5) | (dpc_register.cmd_busy << 6) | (dpc_register.cbuf_busy << 7) | (dpc_register.dma_busy << 8) | (dpc_register.end_valid << 9) | (dpc_register.start_valid << 10));
}
...
void read_nothingb() { *rdword = 0; }

void read_nothingh() { *rdword = 0; }

void read_nothingd() { *rdword = 0; }
...
void read_rdramb() { *rdword = *(rdramb + ((address & 0xFFFFFF) ^ S8)); }

void read_rdramh() { *rdword = *((uint16_t*)(rdramb + ((address & 0xFFFFFF) ^ S16))); }

void read_rdramd() { *rdword = ((uint64_t)(*(uint32_t*)(rdramb + (address & 0xFFFFFF))) << 32) | ((*(uint32_t*)(rdramb + (address & 0xFFFFFF) + 4))); }
...
void read_rdramFB()
{
    ...
    for (i = 0; i < 6; i++)
    {
        if (frameBufferInfos[i].addr)
        {
            int32_t start = frameBufferInfos[i].addr & 0x7FFFFF;
            int32_t end = start + frameBufferInfos[i].width * frameBufferInfos[i].height * frameBufferInfos[i].size - 1;
            if ((address & 0x7FFFFF) >= start && (address & 0x7FFFFF) <= end && framebufferRead[(address & 0x7FFFFF) >> 12])
            {
                fBRead(address);
...
void write_rdram() { *((uint32_t*)(rdramb + (address & 0xFFFFFF))) = word; }

void write_rdramb() { *((rdramb + ((address & 0xFFFFFF) ^ S8))) = g_byte; }

void write_rdramh() { *(uint16_t*)((rdramb + ((address & 0xFFFFFF) ^ S16))) = hword; }
...
void read_rdramreg() { *rdword = *(readrdramreg[*address_low]); }

void read_rdramregb() { *rdword = *((unsigned char*)readrdramreg[*address_low & 0xfffc] + ((*address_low & 3) ^ S8)); }

void read_rdramregh() { *rdword = *((uint16_t*)((unsigned char*)readrdramreg[*address_low & 0xfffc] + ((*address_low & 3) ^ S16))); }

void read_rdramregd() { *rdword = ((uint64_t)(*readrdramreg[*address_low]) << 32) | *readrdramreg[*address_low + 4]; }

void write_rdramreg() { *readrdramreg[*address_low] = word; }

void write_rdramregb() { *((unsigned char*)readrdramreg[*address_low & 0xfffc] + ((*address_low & 3) ^ S8)) = g_byte; }

void write_rdramregh() { *((uint16_t*)((unsigned char*)readrdramreg[*address_low & 0xfffc] + ((*address_low & 3) ^ S16))) = hword; }
...
void read_rsp_memd()
{
    if (*address_low < 0x1000)
    {
        *rdword = ((uint64_t)(*(uint32_t*)(SP_DMEMb + (*address_low))) << 32) | ((*(uint32_t*)(SP_DMEMb + (*address_low) + 4)));
    }
    else if (*address_low < 0x2000)
    {
        *rdword = ((uint64_t)(*(uint32_t*)(SP_IMEMb + (*address_low & 0xFFF))) << 32) | ((*(uint32_t*)(SP_IMEMb + (*address_low & 0xFFF) + 4)));
    }
    else
        read_nomemd();
}
...
void read_rsp() { *rdword = *(readrsp[*address_low]); }

void read_rspb() { *rdword = *((unsigned char*)readrsp[*address_low & 0xfffc] + ((*address_low & 3) ^ S8)); }

void read_rsph() { *rdword = *((uint16_t*)((unsigned char*)readrsp[*address_low & 0xfffc] + ((*address_low & 3) ^ S16))); }

void read_rspd() { *rdword = ((uint64_t)(*readrsp[*address_low]) << 32) | *readrsp[*address_low + 4]; }

void write_rsp() { *readrsp[*address_low] = word; }

void write_rspb() { *((unsigned char*)readrsp[*address_low & 0xfffc] + ((*address_low & 3) ^ S8)) = g_byte; }

void write_rsph() { *((uint16_t*)((unsigned char*)readrsp[*address_low & 0xfffc] + ((*address_low & 3) ^ S16))) = hword; }
...
void read_dp() { *rdword = *(readdp[*address_low]); }

void read_dpb() { *rdword = *((unsigned char*)readdp[*address_low & 0xfffc] + ((*address_low & 3) ^ S8)); }

void read_dph() { *rdword = *((uint16_t*)((unsigned char*)readdp[*address_low & 0xfffc] + ((*address_low & 3) ^ S16))); }

void read_dpd() { *rdword = ((uint64_t)(*readdp[*address_low]) << 32) | *readdp[*address_low + 4]; }
...
void read_dps() { *rdword = *(readdps[*address_low]); }

void read_dpsb() { *rdword = *((unsigned char*)readdps[*address_low & 0xfffc] + ((*address_low & 3) ^ S8)); }

void read_dpsh() { *rdword = *((uint16_t*)((unsigned char*)readdps[*address_low & 0xfffc] + ((*address_low & 3) ^ S16))); }

void read_dpsd() { *rdword = ((uint64_t)(*readdps[*address_low]) << 32) | *readdps[*address_low + 4]; }

void write_dps() { *readdps[*address_low] = word; }

void write_dpsb() { *((unsigned char*)readdps[*address_low & 0xfffc] + ((*address_low & 3) ^ S8)) = g_byte; }

void write_dpsh() { *((uint16_t*)((unsigned char*)readdps[*address_low & 0xfffc] + ((*address_low & 3) ^ S16))) = hword; }
...
void read_mi() { *rdword = *(readmi[*address_low]); }

void read_mib() { *rdword = *((unsigned char*)readmi[*address_low & 0xfffc] + ((*address_low & 3) ^ S8)); }

void read_mih() { *rdword = *((uint16_t*)((unsigned char*)readmi[*address_low & 0xfffc] + ((*address_low & 3) ^ S16))); }

void read_mid() { *rdword = ((uint64_t)(*readmi[*address_low]) << 32) | *readmi[*address_low + 4]; }
        ...
        case 0x59:
            {
                ...
                if (f)
                    delay = ((uint64_t)ai_register.ai_len * vi_register.vi_delay * 50) / (f * 4);
            }
            break;
        case 0x37:
        case 0x41:
        ...
        case 0x4a:
            {
                ...
                if (f)
                    delay = ((uint64_t)ai_register.ai_len * vi_register.vi_delay * 60) / (f * 4);
            }
            break;
        }
        if (!g_config.is_audio_delay_enabled)
            delay = 0;
...
void write_aib()
{
    ...
    case 0x7:
        ...
        *((unsigned char*)&temp + ((*address_low & 3) ^ S8)) = g_byte;
        ...
        case 0x58:
        case 0x59:
            delay = ((uint64_t)ai_register.ai_len * (ai_register.ai_dacrate + 1) * vi_register.vi_delay * 50) / 49656530;
            break;
        case 0x37:
        case 0x41:
        case 0x45:
        case 0x4a:
            delay = ((uint64_t)ai_register.ai_len * (ai_register.ai_dacrate + 1) * vi_register.vi_delay * 60) / 48681812;
            break;
        }
        // delay = 0;
        if (ai_register.ai_status & 0x40000000) // busy
...
void read_pi() { *rdword = *(readpi[*address_low]); }

void read_pib() { *rdword = *((unsigned char*)readpi[*address_low & 0xfffc] + ((*address_low & 3) ^ S8)); }

void read_pih() { *rdword = *((uint16_t*)((unsigned char*)readpi[*address_low & 0xfffc] + ((*address_low & 3) ^ S16))); }

void read_pid() { *rdword = ((uint64_t)(*readpi[*address_low]) << 32) | *readpi[*address_low + 4]; }
...
void read_ri() { *rdword = *(readri[*address_low]); }

void read_rib() { *rdword = *((unsigned char*)readri[*address_low & 0xfffc] + ((*address_low & 3) ^ S8)); }

void read_rih() { *rdword = *((uint16_t*)((unsigned char*)readri[*address_low & 0xfffc] + ((*address_low & 3) ^ S16))); }

void read_rid() { *rdword = ((uint64_t)(*readri[*address_low]) << 32) | *readri[*address_low + 4]; }

void write_ri() { *readri[*address_low] = word; }

void write_rib() { *((unsigned char*)readri[*address_low & 0xfffc] + ((*address_low & 3) ^ S8)) = g_byte; }

void write_rih() { *((uint16_t*)((unsigned char*)readri[*address_low & 0xfffc] + ((*address_low & 3) ^ S16))) = hword; }
...
void read_si() { *rdword = *(readsi[*address_low]); }

void read_sib() { *rdword = *((unsigned char*)readsi[*address_low & 0xfffc] + ((*address_low & 3) ^ S8)); }

void read_sih() { *rdword = *((uint16_t*)((unsigned char*)readsi[*address_low & 0xfffc] + ((*address_low & 3) ^ S16))); }

void read_sid() { *rdword = ((uint64_t)(*readsi[*address_low]) << 32) | *readsi[*address_low + 4]; }
...
void read_flashram_statusb() { g_core_logger->info("read_flashram_statusb"); }

void read_flashram_statush() { g_core_logger->info("read_flashram_statush"); }

void read_flashram_statusd() { g_core_logger->info("read_flashram_statusd"); }
...
void write_flashram_commandb() { g_core_logger->info("write_flashram_commandb"); }

void write_flashram_commandh() { g_core_logger->info("write_flashram_commandh"); }

void write_flashram_commandd() { g_core_logger->info("write_flashram_commandd"); }
...
void read_romb() { *rdword = *(rom + ((address ^ S8) & 0x03FFFFFF)); }

void read_romh() { *rdword = *((uint16_t*)(rom + ((address ^ S16) & 0x03FFFFFF))); }

void read_romd() { *rdword = ((uint64_t)(*((uint32_t*)(rom + (address & 0x03FFFFFF)))) << 32) | *((uint32_t*)(rom + ((address + 4) & 0x03FFFFFF))); }

void write_rom() { lastwrite = word; }
...
#endif
    *rdword = ((uint64_t)sl(*((uint32_t*)(PIF_RAMb + (address & 0x7FF) - 0x7C0))) << 32) | sl(*((uint32_t*)(PIF_RAMb + ((address + 4) & 0x7FF) - 0x7C0)));
}
...
#endif
    *((uint32_t*)(PIF_RAMb + (address & 0x7FF) - 0x7C0)) = sl((uint32_t)(dword >> 32));
    *((uint32_t*)(PIF_RAMb + (address & 0x7FF) - 0x7C0)) = sl((uint32_t)(dword & 0xFFFFFFFF));
...
void read_sc_reg() { *rdword = read_summercart(address); }

void read_sc_regb() { *rdword = read_summercart(address) >> ((address & 3) << 3) & 0xFF; }

void read_sc_regh() { *rdword = read_summercart(address) >> ((address & 2) << 3) & 0xFFFF; }
...
void write_sc_reg() { write_summercart(address, word); }
...
void write_sc_regh() { write_summercart(address, hword << ((~address & 2) << 3)); }

void write_sc_regd() { write_summercart(address, dword >> 32); }
