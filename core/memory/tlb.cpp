...
uint32_t virtual_to_physical_address(uint32_t addresse, int32_t w) {
  if (addresse >= 0x7f000000 &&
      addresse < 0x80000000) // golden eye hack (it uses TLB a lot)
  {
...
void TLBWI() {
  ...
  if (tlb_e[core_Index & 0x3F].v_even) {
    for (i = tlb_e[core_Index & 0x3F].start_even >> 12;
         i <= tlb_e[core_Index & 0x3F].end_even >> 12; i++) {
      ...
      if (!invalid_code[i]) {
        ...
        blocks[i]->adler32 = adler32(
            0, (const Bytef *)&rdram[(tlb_LUT_r[i] & 0x7FF000) / 4], 0x1000);
  ...
  if (tlb_e[core_Index & 0x3F].v_odd) {
    for (i = tlb_e[core_Index & 0x3F].start_odd >> 12;
         i <= tlb_e[core_Index & 0x3F].end_odd >> 12; i++) {
      if (!invalid_code[i] && (invalid_code[tlb_LUT_r[i] >> 12] ||
                               invalid_code[(tlb_LUT_r[i] >> 12) + 0x20000]))
        invalid_code[i] = 1;
      if (!invalid_code[i]) {
  ...
  if (tlb_e[core_Index & 0x3F].v_even) {
    ...
    for (i = tlb_e[core_Index & 0x3F].start_even >> 12;
      ...
      if (blocks[i] && blocks[i]->adler32) {
        if (blocks[i]->adler32 ==
            adler32(0, (const Bytef *)&rdram[(tlb_LUT_r[i] & 0x7FF000) / 4],
                    0x1000))
          invalid_code[i] = 0;
      }
    }
...
void TLBWR() {
  ...
  if (tlb_e[core_Random].v_even) {
    for (i = tlb_e[core_Random].start_even >> 12;
         i <= tlb_e[core_Random].end_even >> 12; i++) {
      if (!invalid_code[i] && (invalid_code[tlb_LUT_r[i] >> 12] ||
                               invalid_code[(tlb_LUT_r[i] >> 12) + 0x20000]))
        invalid_code[i] = 1;
      if (!invalid_code[i]) {
  ...
  if (tlb_e[core_Random].v_odd) {
    for (i = tlb_e[core_Random].start_odd >> 12;
         i <= tlb_e[core_Random].end_odd >> 12; i++) {
      if (!invalid_code[i] && (invalid_code[tlb_LUT_r[i] >> 12] ||
                               invalid_code[(tlb_LUT_r[i] >> 12) + 0x20000]))
        invalid_code[i] = 1;
      if (!invalid_code[i]) {
