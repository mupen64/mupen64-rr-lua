...
static void vhd_copy(struct vhd *vhd, FILE *dst, FILE *src, void *buf,
                     uint32_t n) {
  uint64_t len;
...
static void sd_read() {
  ...
  if ((path = get_sd_path())) {
    ...
    sd_error(L"Could not generate SD image path.", L"SD read error");
}
...
static void sd_write() {
  ...
  if ((path = get_sd_path())) {
    ...
    sd_error(L"Could not generate SD image path.", L"SD write error");
}
...
void save_summercart(const char *filename) {
  ...
  if ((buf = malloc(512 * (n = 128)))) {
    if ((sdp = get_sd_path())) {
      ...
      sd_error(L"Could not generate SD image path.", L"Save error");
    free(buf);
  } else
  ...
  if ((buf = malloc(512 * (n = 128)))) {
    if ((stp = get_st_path(filename))) {
      if ((sdp = get_sd_path())) {
        ...
        sd_error(L"Could not generate SD image path.", L"Load error");
      free(stp);
    } else
...
void init_summercart() { memset(&summercart, 0, sizeof(struct summercart)); }
