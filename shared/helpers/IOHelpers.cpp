...
std::vector<uint8_t> auto_decompress(std::vector<uint8_t> &vec,
  ...
  // we dont know what the decompressed size is, so we reallocate a buffer until
  // we find the right size lol
  ...
  while (true) {
    ...
    auto result = libdeflate_gzip_decompress(
        decompressor, vec.data(), vec.size(), out_buf, buf_size, &actual_size);
