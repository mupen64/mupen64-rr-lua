...
static int get_keys(lua_State *L) {
  ...
  lua_pushinteger(
      L, g_last_wheel_delta > 0 ? 1 : (g_last_wheel_delta < 0 ? -1 : 0));
...
static INT_PTR CALLBACK InputPromptProc(HWND wnd, UINT msg, WPARAM wParam,
  ...
  case WM_INITDIALOG: {
    ...
    SetWindowText(wnd,
                  string_to_wstring(luaL_optstring(L, 1, "input:")).c_str());
...
static int InputPrompt(lua_State *L) {
  DialogBoxParam(g_app_instance, MAKEINTRESOURCE(IDD_LUAINPUTPROMPT),
                 g_main_hwnd, InputPromptProc, (LPARAM)L);
  return 1;
}
} // namespace LuaCore::Input
