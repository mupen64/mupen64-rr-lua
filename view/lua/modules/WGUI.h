...
const std::unordered_map<std::wstring, COLORREF> color_map = {
    ...
    {L"clear", 0x00000000},  {L"gray", 0xFF808080},
    {L"red", 0xFF0000FF},    {L"orange", 0xFF0080FF},
    {L"yellow", 0xFF00FFFF}, {L"chartreuse", 0xFF00FF80},
    {L"green", 0xFF00FF00},  {L"teal", 0xFF80FF00},
    {L"cyan", 0xFFFFFF00},   {L"blue", 0xFFFF0000},
    {L"purple", 0xFFFF0080},
};
...
static int ResizeWindow(lua_State *L) {
  ...
  SetWindowPos(g_main_hwnd, 0, 0, 0, w + (wndRect.right - clientRect.right),
...
static COLORREF StrToColorA(const std::wstring &s, bool alpha = false,
                            COLORREF def = 0) {
  if (s[0] == '#') {
    int l = s.size();
    if (l == 4) {
...
static COLORREF StrToColor(const std::wstring &s, bool alpha = false,
                           COLORREF def = 0) {
...
static int SetFont(lua_State *L) {
  ...
  font.lfHeight =
      -MulDiv(font_size, GetDeviceCaps(lua->gdi_back_dc, LOGPIXELSY), 72);
...
static int LoadScreen(lua_State *L) {
  ...
  BitBlt(lua->loadscreen_dc, 0, 0, lua->dc_size.width, lua->dc_size.height, dc,
         0, 0, SRCCOPY);
...
static int FillPolygonAlpha(lua_State *L) {
  ...
  Gdiplus::SolidBrush brush(
      Gdiplus::Color(luaL_checkinteger(L, 2), luaL_checkinteger(L, 3),
...
static int FillRect(lua_State *L) {
  ...
  COLORREF color = RGB(luaL_checknumber(L, 5), luaL_checknumber(L, 6),
