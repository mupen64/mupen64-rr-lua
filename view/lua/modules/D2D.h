...
#define D2D_GET_RECT(L, idx) D2D1::RectF(luaL_checknumber(L, idx), luaL_checknumber(L, idx + 1), luaL_checknumber(L, idx + 2), luaL_checknumber(L, idx + 3))

#define D2D_GET_COLOR(L, idx) D2D1::ColorF(luaL_checknumber(L, idx), luaL_checknumber(L, idx + 1), luaL_checknumber(L, idx + 2), luaL_checknumber(L, idx + 3))

#define D2D_GET_POINT(L, idx)                                                                                                                                                                                                                                                                                                                                                                                                                                                                  \
    D2D1_POINT_2F { .x = (float)luaL_checknumber(L, idx), .y = (float)luaL_checknumber(L, idx + 1) }

#define D2D_GET_ELLIPSE(L, idx)                                                                                                                                                                                                                                                                                                                                                                                                                                                                \
    D2D1_ELLIPSE { .point = D2D_GET_POINT(L, idx), .radiusX = (float)luaL_checknumber(L, idx + 2), .radiusY = (float)luaL_checknumber(L, idx + 3) }

#define D2D_GET_ROUNDED_RECT(L, idx) D2D1_ROUNDED_RECT(D2D_GET_RECT(L, idx), luaL_checknumber(L, idx + 5), luaL_checknumber(L, idx + 6))
    ...
    static int create_brush(lua_State* L)
    {
        ...
        lua->d2d_render_target_stack.top()->CreateSolidColorBrush(color, &brush);
    ...
    static int draw_text(lua_State* L)
    {
        ...
        if (!lua->dw_text_layouts.contains(params_hash))
        {
            ...
            lua->dw_factory->CreateTextFormat(string_to_wstring(font_name).c_str(), nullptr, static_cast<DWRITE_FONT_WEIGHT>(font_weight), static_cast<DWRITE_FONT_STYLE>(font_style), DWRITE_FONT_STRETCH_NORMAL, font_size, L"", &text_format);
    ...
    static int set_text_antialias_mode(lua_State* L)
    {
        ...
        lua->d2d_render_target_stack.top()->SetTextAntialiasMode((D2D1_TEXT_ANTIALIAS_MODE)mode);
        return 0;
    }
    ...
    static int measure_text(lua_State* L)
    {
        ...
        lua->dw_factory->CreateTextFormat(string_to_wstring(font_name).c_str(), NULL, DWRITE_FONT_WEIGHT_NORMAL, DWRITE_FONT_STYLE_NORMAL, DWRITE_FONT_STRETCH_NORMAL, font_size, L"", &text_format);
    ...
    static int push_clip(lua_State* L)
    {
        ...
        lua->d2d_render_target_stack.top()->PushAxisAlignedClip(rectangle, D2D1_ANTIALIAS_MODE_PER_PRIMITIVE);
    ...
    static int draw_rounded_rectangle(lua_State* L)
    {
        ...
        lua->d2d_render_target_stack.top()->DrawRoundedRectangle(&rounded_rectangle, brush, thickness);
    ...
    static int load_image(lua_State* L)
    {
        ...
        HRESULT hr = pIWICFactory->CreateDecoderFromFilename(string_to_wstring(path).c_str(), NULL, GENERIC_READ, WICDecodeMetadataCacheOnLoad, &pDecoder);
        ...
        lua->d2d_render_target_stack.top()->CreateBitmapFromWicBitmap(pConverter, NULL, &bmp);
    ...
    static int draw_image(lua_State* L)
    {
        ...
        lua->d2d_render_target_stack.top()->DrawBitmap(bmp, destination_rectangle, opacity, (D2D1_BITMAP_INTERPOLATION_MODE)interpolation, source_rectangle);
    ...
    static int draw_to_image(lua_State* L)
    {
        ...
        // With render target at top of stack, we hand control back to script and let
        // it run its callback with rt-scoped drawing
