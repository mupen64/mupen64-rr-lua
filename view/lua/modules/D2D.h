...
#define D2D_GET_RECT(L, idx)                                                   \
  D2D1::RectF(luaL_checknumber(L, idx), luaL_checknumber(L, idx + 1),          \
              luaL_checknumber(L, idx + 2), luaL_checknumber(L, idx + 3))

#define D2D_GET_COLOR(L, idx)                                                  \
  D2D1::ColorF(luaL_checknumber(L, idx), luaL_checknumber(L, idx + 1),         \
               luaL_checknumber(L, idx + 2), luaL_checknumber(L, idx + 3))
...
#define D2D_GET_ROUNDED_RECT(L, idx)                                           \
  D2D1_ROUNDED_RECT(D2D_GET_RECT(L, idx), luaL_checknumber(L, idx + 5),        \
...
static int clear(lua_State *L) {
  ...
  lua->d2d_render_target_stack.top()->Clear(
      lua->presenter->adjust_clear_color(color));
...
static int draw_rectangle(lua_State *L) {
  ...
  lua->d2d_render_target_stack.top()->DrawRectangle(&rectangle, brush,
                                                    thickness);
...
static int draw_line(lua_State *L) {
  ...
  lua->d2d_render_target_stack.top()->DrawLine(point_a, point_b, brush,
                                               thickness);
...
static int draw_text(lua_State *L) {
  ...
  if (!lua->dw_text_layouts.contains(params_hash)) {
    ...
    lua->dw_factory->CreateTextFormat(
        string_to_wstring(font_name).c_str(), nullptr,
    ...
    lua->dw_factory->CreateTextLayout(
        wtext.c_str(), wtext.length(), text_format,
...
static int measure_text(lua_State *L) {
  ...
  lua->dw_factory->CreateTextFormat(
      string_to_wstring(font_name).c_str(), NULL, DWRITE_FONT_WEIGHT_NORMAL,
      DWRITE_FONT_STYLE_NORMAL, DWRITE_FONT_STRETCH_NORMAL, font_size, L"",
      &text_format);

  IDWriteTextLayout *text_layout;

  lua->dw_factory->CreateTextLayout(text.c_str(), text.length(), text_format,
...
static int push_clip(lua_State *L) {
  ...
  lua->d2d_render_target_stack.top()->PushAxisAlignedClip(
      rectangle, D2D1_ANTIALIAS_MODE_PER_PRIMITIVE);
...
static int fill_rounded_rectangle(lua_State *L) {
  ...
  lua->d2d_render_target_stack.top()->FillRoundedRectangle(&rounded_rectangle,
                                                           brush);
...
static int draw_rounded_rectangle(lua_State *L) {
  ...
  lua->d2d_render_target_stack.top()->DrawRoundedRectangle(&rounded_rectangle,
                                                           brush, thickness);
...
static int load_image(lua_State *L) {
  ...
  HRESULT hr = pIWICFactory->CreateDecoderFromFilename(
      string_to_wstring(path).c_str(), NULL, GENERIC_READ,
  ...
  lua->d2d_render_target_stack.top()->CreateBitmapFromWicBitmap(pConverter,
                                                                NULL, &bmp);
...
static int draw_image(lua_State *L) {
  ...
  lua->d2d_render_target_stack.top()->DrawBitmap(
      bmp, destination_rectangle, opacity,
      (D2D1_BITMAP_INTERPOLATION_MODE)interpolation, source_rectangle);
