...
static int IsMainWindowInForeground(lua_State *L) {
  ...
  lua_pushboolean(L, GetForegroundWindow() == g_main_hwnd ||
                         GetActiveWindow() == g_main_hwnd);
  return 1;
}

static int LuaPlaySound(lua_State *L) {
  PlaySound(string_to_wstring(luaL_checkstring(L, 1)).c_str(), NULL,
...
#define B(n)                                                                   \
  ...
  const NameAndVariable list[] = {
      A(rdram),       A(rdram_register), A(MI_register),  A(pi_register),
      A(sp_register), A(rsp_register),   A(si_register),  A(vi_register),
      A(ri_register), A(ai_register),    A(dpc_register), A(dps_register),
      B(SP_DMEM),     B(PIF_RAM),        {NULL, NULL}};
#undef A
#undef B
