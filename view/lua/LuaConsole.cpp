...
int at_panic(lua_State *L) {
  ...
  FrontendService::show_dialog(message.c_str(), L"Lua",
...
void invoke_callbacks_with_key_on_all_instances(
  ...
  // OPTIMIZATION: Store destruction-queued scripts in queue and destroy them
  // after iteration to avoid having to clone the queue This is somehow faster
  // lol
...
INT_PTR CALLBACK DialogProc(HWND wnd, UINT msg, WPARAM wParam, LPARAM lParam) {
  ...
  case WM_COMMAND: {
    ...
    case IDC_BUTTON_LUASTATE: {
      ...
      GetWindowText(GetDlgItem(wnd, IDC_TEXTBOX_LUASCRIPTPATH), path,
                    std::size(path));

      // if already running, delete and erase it (we dont want to overwrite the
      // environment without properly disposing it)
      if (g_hwnd_lua_map.contains(wnd)) {
        LuaEnvironment::destroy(g_hwnd_lua_map[wnd]);
      }
      ...
      Messenger::broadcast(Messenger::Message::ScriptStarted,
    ...
    case IDC_BUTTON_LUAEDIT: {
      ...
      GetWindowText(GetDlgItem(wnd, IDC_TEXTBOX_LUASCRIPTPATH), buf,
                    std::size(buf));
  ...
  case WM_SIZE: {
    ...
    SetWindowPos(console_hwnd, nullptr, 0, 0,
                 window_rect.right - console_rect.left * 2,
                 window_rect.bottom - console_rect.top, SWP_NOMOVE);

    HWND path_hwnd = GetDlgItem(wnd, IDC_TEXTBOX_LUASCRIPTPATH);
    RECT path_rect = get_window_rect_client_space(wnd, path_hwnd);
    SetWindowPos(path_hwnd, nullptr, 0, 0,
                 window_rect.right - console_rect.left * 2,
                 path_rect.bottom - path_rect.top, SWP_NOMOVE);
...
HWND lua_create() {
  HWND hwnd = CreateDialogParam(g_app_instance, MAKEINTRESOURCE(IDD_LUAWINDOW),
                                g_main_hwnd, DialogProc, NULL);
...
void lua_create_and_run(const std::wstring &path) {
  ...
  // click run button
  SendMessage(hwnd, WM_COMMAND, MAKEWPARAM(IDC_BUTTON_LUASTATE, BN_CLICKED),
...
// begin lua funcs
const luaL_Reg globalFuncs[] = {{"print", LuaCore::Global::Print},
...
const luaL_Reg emuFuncs[] = {
    ...
    {"atwarpmodifystatuschanged",
     LuaCore::Emu::RegisterWarpModifyStatusChanged},
...
const luaL_Reg iohelperFuncs[] = {
    {"filediag", LuaCore::IOHelper::LuaFileDialog}, {NULL, NULL}};
const luaL_Reg aviFuncs[] = {{"startcapture", LuaCore::Avi::StartCapture},
...
void stop_all_scripts() {
  ...
  for (const auto [key, _] : copy) {
    SendMessage(key, WM_COMMAND, MAKEWPARAM(IDC_BUTTON_LUASTOP, BN_CLICKED),
                (LPARAM)GetDlgItem(key, IDC_BUTTON_LUASTOP));
  }
  assert(g_hwnd_lua_map.empty());
}

LRESULT CALLBACK d2d_overlay_wndproc(HWND hwnd, UINT msg, WPARAM wparam,
                                     LPARAM lparam) {
  switch (msg) {
  case WM_PAINT: {
    // NOTE: Sometimes, this control receives a WM_PAINT message while execution
    // is already in WM_PAINT, causing us to call begin_present twice in a
    // row... Usually this shouldn't happen, but the shell file dialog API
    // causes this by messing with the parent window's message loop.
    ...
    bool failed = lua->invoke_callbacks_with_key(LuaService::pcall_no_params,
                                                 REG_ATDRAWD2D);
...
LRESULT CALLBACK gdi_overlay_wndproc(HWND hwnd, UINT msg, WPARAM wparam,
                                     LPARAM lparam) {
  switch (msg) {
  case WM_PAINT: {
    auto lua = (LuaEnvironment *)GetWindowLongPtr(hwnd, GWLP_USERDATA);
    ...
    BitBlt(dc, 0, 0, lua->dc_size.width, lua->dc_size.height, lua->gdi_back_dc,
           0, 0, SRCCOPY);
...
void LuaEnvironment::create_loadscreen() {
  ...
  loadscreen_bmp =
      CreateCompatibleBitmap(gdi_dc, dc_size.width, dc_size.height);
...
void LuaEnvironment::create_renderer() {
  ...
  if (hr != S_OK && hr != S_FALSE && hr != RPC_E_CHANGED_MODE) {
    FrontendService::show_dialog(
        ...
        L"Lua", FrontendService::DialogType::Error);
    return;
  }
  ...
  // NOTE: We don't want negative or zero size on any axis, as that messes up
  // comp surface creation
  dc_size = {(UINT32)max(1, window_rect.right),
             (UINT32)max(1, window_rect.bottom)};
  g_view_logger->info("Lua dc size: {} {}", dc_size.width, dc_size.height);

  d2d_overlay_hwnd =
      CreateWindowEx(WS_EX_LAYERED, D2D_OVERLAY_CLASS, L"",
                     WS_CHILD | WS_VISIBLE, 0, 0, dc_size.width, dc_size.height,
                     g_main_hwnd, nullptr, g_app_instance, nullptr);
  SetWindowLongPtr(d2d_overlay_hwnd, GWLP_USERDATA, (LONG_PTR)this);

  DWriteCreateFactory(DWRITE_FACTORY_TYPE_SHARED, __uuidof(dw_factory),
  ...
  if (!presenter->init(d2d_overlay_hwnd)) {
    FrontendService::show_dialog(
        L"Failed to initialize presenter.\r\nVerify that your system supports "
        L"the selected presenter.",
        L"Lua", FrontendService::DialogType::Error);
    return;
  }
  ...
  dw_text_layouts =
      MicroLRU::Cache<uint64_t, IDWriteTextLayout *>(128, [&](auto value) {
  ...
  gdi_overlay_hwnd =
      CreateWindowEx(WS_EX_LAYERED, GDI_OVERLAY_CLASS, L"",
                     WS_CHILD | WS_VISIBLE, 0, 0, dc_size.width, dc_size.height,
                     g_main_hwnd, nullptr, g_app_instance, nullptr);
  SetWindowLongPtr(gdi_overlay_hwnd, GWLP_USERDATA, (LONG_PTR)this);
  SetLayeredWindowAttributes(gdi_overlay_hwnd, lua_gdi_color_mask, 0,
                             LWA_COLORKEY);

  // If we don't fill up the DC with the key first, it never becomes
  // "transparent"
...
std::string LuaEnvironment::create(const std::filesystem::path &path,
                                   HWND wnd) {
  assert(is_on_gui_thread());
  ...
  // NOTE: We need to add the lua to the global map already since it may receive
  // callbacks while its executing the global code
  g_hwnd_lua_map[lua_environment->hwnd] = lua_environment;
  rebuild_lua_env_map();

  bool has_error =
      luaL_dofile(lua_environment->L, lua_environment->path.string().c_str());
...
// calls all functions that lua script has defined as callbacks, reads them from
// registry returns true at fail
bool LuaEnvironment::invoke_callbacks_with_key(
    const std::function<int(lua_State *)> &function, const char *key) {
  assert(is_on_gui_thread());
...
void LuaEnvironment::repaint_visuals() {
  RedrawWindow(this->d2d_overlay_hwnd, nullptr, nullptr,
               RDW_INVALIDATE | RDW_UPDATENOW);
  RedrawWindow(this->gdi_overlay_hwnd, nullptr, nullptr,
               RDW_INVALIDATE | RDW_UPDATENOW);
}

void register_as_package(lua_State *lua_state, const char *name,
                         const luaL_Reg regs[]) {
  if (name == nullptr) {
...
void LuaEnvironment::register_functions() {
  ...
  luaL_dostring(L, "input.map_virtual_key_ex = function() "
                   "print('input.map_virtual_key_ex has been deprecated') end");

  // DEPRECATED: emu.getsystemmetrics couples to WinAPI
  luaL_dostring(L, "emu.getsystemmetrics = function() "
                   "print('emu.getsystemmetrics has been deprecated') end");

  // DEPRECATED: movie.begin_seek_to doesn't exist anymore
  luaL_dostring(L,
                "movie.begin_seek_to = function() print('movie.begin_seek_to "
                "has been deprecated, use movie.begin_seek instead') end");

  // DEPRECATED: movie.get_seek_info doesn't exist anymore
  luaL_dostring(L,
                "movie.get_seek_info = function() print('movie.get_seek_info "
                "has been deprecated, use movie.begin_seek instead') end");
