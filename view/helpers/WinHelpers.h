...
static int32_t get_checkbox_state(const HWND hwnd, const int32_t id) {
  return SendDlgItemMessage(hwnd, id, BM_GETCHECK, 0, 0) == BST_CHECKED ? TRUE
                                                                        : FALSE;
}
...
static void read_combo_box_value(const HWND hwnd, const int resource_id,
                                 char *ret) {
...
static RECT get_window_rect_client_space(HWND parent, HWND child) {
  ...
  return {offset_client.left, offset_client.top,
          offset_client.left + (client.right - client.left),
          offset_client.top + (client.bottom - client.top)};
}

static bool create_composition_surface(
    HWND hwnd, D2D1_SIZE_U size, IDXGIFactory2 **factory,
    IDXGIAdapter1 **dxgiadapter, ID3D11Device **d3device,
    IDXGIDevice1 **dxdevice, ID2D1Bitmap1 **bitmap,
    IDCompositionVisual **comp_visual, IDCompositionDevice **comp_device,
    IDCompositionTarget **comp_target, IDXGISwapChain1 **swapchain,
    ID2D1Factory3 **d2d_factory, ID2D1Device2 **d2d_device,
    ID3D11DeviceContext **d3d_dc, ID2D1DeviceContext2 **d2d_dc,
    IDXGISurface1 **dxgi_surface, ID3D11Resource **dxgi_surface_resource,
    ID3D11Resource **front_buffer, ID3D11Texture2D **d3d_gdi_tex) {
  ...
  D3D11CreateDevice(*dxgiadapter, D3D_DRIVER_TYPE_UNKNOWN, nullptr,
                    ...
                    nullptr, 0, D3D11_SDK_VERSION, d3device, nullptr, d3d_dc);
...

  {
    ...
    (*factory)->CreateSwapChainForComposition(*d3device, &swapdesc, nullptr,
                                              swapchain);
  ...
  {
    (*d2d_device)
        ->CreateDeviceContext(D2D1_DEVICE_CONTEXT_OPTIONS_NONE, d2d_dc);
  ...
  // Since the swapchain can't be gdi-compatible, we need a gdi-compatible
  // texture which we blit the swapbuffer onto
...
static HWND create_tooltip(HWND hwnd, int id, std::wstring text) {
  HWND hwnd_tip =
      CreateWindowEx(NULL, TOOLTIPS_CLASS, NULL, WS_POPUP | TTS_NOPREFIX,
  ...
  // FIXME: Why does ms want mutable string for this, isn't this getting copied
  // when the tool is added???
  info.lpszText = const_cast<LPWSTR>(text.c_str());

  SendMessage(hwnd_tip, TTM_ADDTOOL, 0, (LPARAM)&info);
  // Multiline tooltips only work if you specify a max width because of course
  // they do
...
/**
 * Shifts the listview selection by the specified amount. Retains sparse
 * selections. Selections which fall outside the bounds of the item range after
 * shifting are dropped. \param hwnd Handle to a listview. \param offset The
 * shift amount.
 */
 ...
 */
static void set_listview_selection(const HWND hwnd,
