...
{
    auto result = FrontendService::show_multiple_choice_dialog({L"Choose Default Plugins", L"Change Plugins", L"Cancel"},
    ...
    if (result == 0)
    {
        ...
        auto first_video_plugin = std::ranges::find_if(plugin_discovery_result.plugins, [](const auto& plugin) { return plugin->type() == PluginType::Video; });

        auto first_audio_plugin = std::ranges::find_if(plugin_discovery_result.plugins, [](const auto& plugin) { return plugin->type() == PluginType::Audio; });

        auto first_input_plugin = std::ranges::find_if(plugin_discovery_result.plugins, [](const auto& plugin) { return plugin->type() == PluginType::Input; });

        auto first_rsp_plugin = std::ranges::find_if(plugin_discovery_result.plugins, [](const auto& plugin) { return plugin->type() == PluginType::RSP; });
...
{
    if (result == CoreResult::Ok || result == CoreResult::ST_Cancelled || result == CoreResult::VCR_Cancelled)
    {
        return false;
    }
...
void SetDlgItemHotkeyAndMenu(HWND hwnd, int idc, t_hotkey* hotkey, int menuItemID)
{
    ...
    set_menu_accelerator(menuItemID, hotkey_str == L"(nothing)" ? L"" : hotkey_str.c_str());
}
...
const wchar_t* get_status_text()
{
    ...
    if (VCR::get_task() == e_task::playback)
    {
        ...
        {
            wsprintfW(text, L"%d / %d (%d / %d) ", current_vi, VCR::get_length_vis(), current_sample - index_adjustment, VCR::get_length_samples());
        }
    }
...
void on_vis_since_input_poll_exceeded(std::any)
{
    ...
    if (g_config.silent_mode ||
        FrontendService::show_ask_dialog(L"An unusual execution pattern was detected. Continuing might leave "
...
void update_core_fast_forward(std::any) { g_vr_fast_forward = g_fast_forward || VCR::is_seeking() || g_vr_benchmark_enabled || Cli::wants_fast_forward(); }
...
bool confirm_user_exit()
{
    ...
    if (warnings > 0)
        res = MessageBox(g_main_hwnd, final_message.c_str(), L"Stop emulation?", MB_YESNO | MB_ICONWARNING);
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT Message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_FOCUS_MAIN_WINDOW:
        ...
        break;
    case WM_CREATE:
        g_main_window_dispatcher = std::make_unique<Dispatcher>(g_ui_thread_id, [] { SendMessage(g_main_hwnd, WM_EXECUTE_DISPATCHER, 0, 0); });
    ...
    case WM_COMMAND:
        {
            ...
            case IDM_BENCHMARK_LUA_CALLBACK:
                {
                    FrontendService::show_dialog(L"Make sure the Lua script is running and "
                                                 ...
                                                 L"Benchmark Lua Callback", FrontendService::DialogType::Information);
            ...
            case IDM_RAMSTART:
                {
                    ...
                    const auto result = FrontendService::show_multiple_choice_dialog({L"Copy STROOP config line", L"Close"}, str.c_str(), L"Show RAM Start", FrontendService::DialogType::Information);
            ...
            case IDM_STATS:
                {
                    ...
                    auto str = std::format(L"Total playtime: {}\r\nTotal rerecords: {}", format_duration(g_config.total_frames / 30), g_config.total_rerecords);
            ...
            case IDM_START_MOVIE_RECORDING:
                {
                    ...
                    auto vcr_result = VCR::start_record(movie_dialog_result.path, movie_dialog_result.start_flag, wstring_to_string(movie_dialog_result.author), wstring_to_string(movie_dialog_result.description));
            ...
            case IDM_START_CAPTURE:
                if (emu_launched)
                {
                    ...
                    if (EncodingManager::start_capture(path, static_cast<EncoderType>(g_config.encoder_type), ask_preset))
                    {
            ...
            default:
                ...
                else if (LOWORD(wParam) >= ID_RECENTROMS_FIRST && LOWORD(wParam) < (ID_RECENTROMS_FIRST + g_config.recent_rom_paths.size()))
                {
                ...
                else if (LOWORD(wParam) >= ID_RECENTMOVIES_FIRST && LOWORD(wParam) < (ID_RECENTMOVIES_FIRST + g_config.recent_movie_paths.size()))
                {
...
LONG WINAPI ExceptionReleaseTarget(_EXCEPTION_POINTERS* ExceptionInfo)
{
    ...
    bool is_continuable = !(ExceptionInfo->ExceptionRecord->ExceptionFlags & EXCEPTION_NONCONTINUABLE);
...
int CALLBACK WinMain(_In_ HINSTANCE hInstance, _In_opt_ HINSTANCE hPrevInstance, _In_ LPSTR lpCmdLine, _In_ int nCmdShow)
{
    ...
    g_main_hwnd = CreateWindowEx(0, WND_CLASS, MUPEN_VERSION, WS_OVERLAPPEDWINDOW | WS_EX_COMPOSITED, g_config.window_x, g_config.window_y, g_config.window_width, g_config.window_height, NULL, NULL, hInstance, NULL);
    ...
    Messenger::subscribe(Messenger::Message::SeekStatusChanged, [](std::any) { update_core_fast_forward(nullptr); });
