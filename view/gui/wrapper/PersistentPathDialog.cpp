...
std::wstring show_persistent_open_dialog(const std::wstring& id, HWND hwnd, const std::wstring& filter)
{
    ...
    {
        std::wstring restored_path = g_config.persistent_folder_paths.contains(id) ? g_config.persistent_folder_paths[id] : get_desktop_path();
        ...
        if (SHCreateItemFromParsingName(restored_path.c_str(), nullptr, IID_PPV_ARGS(&shlPtr)) != S_OK)
            g_view_logger->info("Unable to create IShellItem from parsing lastPath name");
    }
...
std::wstring show_persistent_save_dialog(const std::wstring& id, HWND hwnd, const std::wstring& filter)
{
    ...
    FAILSAFE(CoCreateInstance(CLSID_FileSaveDialog, nullptr, CLSCTX_INPROC_SERVER, IID_PPV_ARGS(&pFileDialog)));
    ...
    // we are saving a file, use the filter list's first extension as the default
    // suffix because otherwise we get an extension-less file...
    {
