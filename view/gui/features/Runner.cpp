...
void run_auto(int id, std::filesystem::path path) {
  ...
  case IDC_LIST_MOVIES:
    ...
    AsyncExecutor::invoke_async([=] { VCR::start_playback(path); });
    break;
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT Message, WPARAM wParam,
  ...
  case WM_INITDIALOG: {
    last_selected_id = -1;
    auto populate_with_paths = [&](const int id,
                                   std::vector<std::wstring> paths) {
      auto ctl = GetDlgItem(hwnd, id);
      for (auto path : paths) {
        auto index = ListBox_AddString(
            ctl, std::filesystem::path(path).filename().wstring().c_str());
  ...
  case WM_COMMAND:
    ...
    case IDC_LIST_SCRIPTS:
      if (HIWORD(wParam) == LBN_DBLCLK) {
        ...
        auto buffer = (char *)ListBox_GetItemData(
            GetDlgItem(hwnd, LOWORD(wParam)), index);
    ...
    case IDOK: {
      ...
      SendMessage(hwnd, WM_COMMAND, MAKEWPARAM(last_selected_id, LBN_DBLCLK),
                  0);
      break;
    }
    case IDCANCEL:
...
void show() {
  DialogBox(g_app_instance, MAKEINTRESOURCE(IDD_RUNNER), g_main_hwnd,
            (DLGPROC)WndProc);
}
} // namespace Runner
