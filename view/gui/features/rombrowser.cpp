...
namespace Rombrowser
{
    ...
    std::vector<std::wstring> find_available_roms()
    {
        ...
        std::ranges::copy_if(rom_paths, std::back_inserter(filtered_rom_paths),
                             [](std::wstring val)
                             {
    ...
    void rombrowser_update_sort() { ListView_SortItems(rombrowser_hwnd, rombrowser_compare, nullptr); }
...
#define ADD_ICON(id)                                                                                                                                                                                                                                                                                                                                                                                                                                                                           \
    ...
    void build_impl()
    {
        ...
        g_view_logger->info("Rombrowser loading took {}ms", static_cast<int>((std::chrono::high_resolution_clock::now() - start_time).count() / 1'000'000));
    }
