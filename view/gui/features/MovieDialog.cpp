...
size_t count_button_presses(const std::vector<BUTTONS> &buttons,
                            const int mask) {
...
LRESULT CALLBACK MovieInspectorProc(HWND hwnd, UINT Message, WPARAM wParam,
                                    LPARAM lParam) {
  // List of dialog item IDs that shouldn't be interactable when in a specific
  // mode
  const std::vector disabled_on_play = {IDC_RADIO_FROM_START, IDC_RADIO_FROM_ST,
  ...
  case WM_INITDIALOG: {
    ...
    grid_hwnd = CreateWindowEx(
        ...
        grid_rect.bottom - grid_rect.top, hwnd, nullptr, g_app_instance, NULL);

    ListView_SetExtendedListViewStyle(grid_hwnd, LVS_EX_GRIDLINES |
    ...
    SendMessage(GetDlgItem(hwnd, IDC_INI_DESCRIPTION), EM_SETLIMITTEXT,
                sizeof(t_movie_header::description), 0);
    SendMessage(GetDlgItem(hwnd, IDC_INI_AUTHOR), EM_SETLIMITTEXT,
                sizeof(t_movie_header::author), 0);
    ...
    SetDlgItemText(hwnd, IDC_INI_MOVIEFILE,
                   record_params.path.wstring().c_str());
  ...
  case WM_DESTROY: {
    ...
    GetDlgItemText(hwnd, IDC_INI_DESCRIPTION, description,
  ...
  case WM_COMMAND:
    ...
    case IDOK: {
      ...
      if (record_params.start_flag == MOVIE_START_FROM_EXISTING_SNAPSHOT) {
        ...
        std::filesystem::path path = show_persistent_open_dialog(
            L"o_movie_existing_snapshot", hwnd, L"*.st;*.savestate");
        ...
        if (exists(path)) {
          const auto str = std::format(
              ...
              path.wstring());
          if (!FrontendService::show_ask_dialog(str.c_str(), L"VCR", true,
                                                hwnd)) {
            break;
          }
        }
    ...
    case IDC_INI_MOVIEFILE: {
      if (g_is_closing) {
        g_view_logger->warn("[MovieDialog] Tried to update movie file path "
                            "while closing dialog");
        break;
      }
    ...
    case IDC_MOVIE_BROWSE: {
      std::wstring path;
      if (is_readonly) {
        path = show_persistent_open_dialog(L"o_movie", hwnd, L"*.m64;*.rec");
      } else {
        path = show_persistent_save_dialog(L"s_movie", hwnd, L"*.m64;*.rec");
      }
...
refresh:
  ...
  metadata.emplace_back(std::make_pair(
      L"ROM",
      std::format(L"{} ({}, {})", string_to_wstring((char *)header.rom_name),
                  country_code_to_country_name(header.rom_country),
                  std::format(L"{:#08x}", header.rom_crc1))));

  metadata.emplace_back(
      std::make_pair(L"Length", std::format(L"{} ({} input)", header.length_vis,
                                            header.length_samples)));
  metadata.emplace_back(std::make_pair(
      L"Duration", format_duration((double)header.length_vis /
                                   (double)header.vis_per_second)));
  metadata.emplace_back(std::make_pair(
      L"Rerecords",
      std::to_wstring(static_cast<uint64_t>(header.extended_data.rerecord_count)
                          << 32 |
                      header.rerecord_count)));

  metadata.emplace_back(std::make_pair(
      L"Video Plugin", string_to_wstring(header.video_plugin_name)));
  metadata.emplace_back(std::make_pair(
      L"Input Plugin", string_to_wstring(header.input_plugin_name)));
  metadata.emplace_back(std::make_pair(
      L"Sound Plugin", string_to_wstring(header.audio_plugin_name)));
  metadata.emplace_back(
  ...
  for (int i = 0; i < 4; ++i) {
    ...
    metadata.emplace_back(
        std::make_pair(std::format(L"Controller {}", i + 1), tempbuf));
  }
  ...
  char authorship[5] = {0};
  memcpy(authorship, header.extended_data.authorship_tag,
         sizeof(header.extended_data.authorship_tag));

  metadata.emplace_back(
      std::make_pair(L"Authorship", header.extended_version == 0
                                        ? L"Unknown"
                                        : string_to_wstring(authorship)));

  metadata.emplace_back(std::make_pair(
      L"A Presses", std::to_wstring(count_button_presses(inputs, 7))));
  metadata.emplace_back(std::make_pair(
      L"B Presses", std::to_wstring(count_button_presses(inputs, 6))));
  metadata.emplace_back(std::make_pair(
      L"Z Presses", std::to_wstring(count_button_presses(inputs, 5))));
  metadata.emplace_back(std::make_pair(
      L"S Presses", std::to_wstring(count_button_presses(inputs, 4))));
  metadata.emplace_back(std::make_pair(
      L"R Presses", std::to_wstring(count_button_presses(inputs, 12))));

  metadata.emplace_back(std::make_pair(
      L"C^ Presses", std::to_wstring(count_button_presses(inputs, 11))));
  metadata.emplace_back(std::make_pair(
      L"Cv Presses", std::to_wstring(count_button_presses(inputs, 10))));
  metadata.emplace_back(std::make_pair(
      L"C< Presses", std::to_wstring(count_button_presses(inputs, 9))));
  metadata.emplace_back(std::make_pair(
      L"C> Presses", std::to_wstring(count_button_presses(inputs, 8))));

  const auto lag_frames =
      max(0, (int64_t)header.length_vis - 2 * (int64_t)header.length_samples);
  metadata.emplace_back(std::make_pair(L"Lag Frames (approximation)",
                                       std::to_wstring(lag_frames)));
  metadata.emplace_back(std::make_pair(
      L"Unused Inputs", std::to_wstring(count_unused_inputs(inputs))));
  metadata.emplace_back(std::make_pair(
      L"Joystick Frames", std::to_wstring(count_joystick_frames(inputs))));
  metadata.emplace_back(std::make_pair(
      L"Input Changes", std::to_wstring(count_input_changes(inputs))));

  SetDlgItemText(hwnd, IDC_INI_AUTHOR,
                 string_to_wstring(header.author).c_str());
  SetDlgItemText(hwnd, IDC_INI_DESCRIPTION,
                 string_to_wstring(header.description).c_str());
  ...
  CheckDlgButton(hwnd, IDC_RADIO_FROM_EXISTING_ST,
                 header.startFlags == MOVIE_START_FROM_EXISTING_SNAPSHOT);
  ...
  CheckDlgButton(hwnd, IDC_RADIO_FROM_EEPROM,
                 header.startFlags == MOVIE_START_FROM_EEPROM);
...
t_record_params show(bool readonly) {
  ...
  record_params.path =
      std::format(L"{} ({}).m64", string_to_wstring((char *)ROM_HEADER.nom),
  ...
  DialogBox(g_app_instance, MAKEINTRESOURCE(IDD_MOVIE_PLAYBACK_DIALOG),
            g_main_hwnd, (DLGPROC)MovieInspectorProc);
