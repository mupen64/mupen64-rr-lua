...
namespace MovieDialog
{
    ...
    LRESULT CALLBACK MovieInspectorProc(HWND hwnd, UINT Message, WPARAM wParam, LPARAM lParam)
    {
        ...
        // mode
        const std::vector disabled_on_play = {IDC_RADIO_FROM_START, IDC_RADIO_FROM_ST, IDC_RADIO_FROM_EEPROM, IDC_RADIO_FROM_EXISTING_ST};
        ...
        case WM_INITDIALOG:
            {
                ...
                SendMessage(GetDlgItem(hwnd, IDC_INI_DESCRIPTION), EM_SETLIMITTEXT, sizeof(t_movie_header::description), 0);
                SendMessage(GetDlgItem(hwnd, IDC_INI_AUTHOR), EM_SETLIMITTEXT, sizeof(t_movie_header::author), 0);
    ...
    refresh:
        ...
        metadata.emplace_back(std::make_pair(L"ROM", std::format(L"{} ({}, {})", string_to_wstring((char*)header.rom_name), country_code_to_country_name(header.rom_country), std::format(L"{:#08x}", header.rom_crc1))));

        metadata.emplace_back(std::make_pair(L"Length", std::format(L"{} ({} input)", header.length_vis, header.length_samples)));
        metadata.emplace_back(std::make_pair(L"Duration", format_duration((double)header.length_vis / (double)header.vis_per_second)));
        metadata.emplace_back(std::make_pair(L"Rerecords", std::to_wstring(static_cast<uint64_t>(header.extended_data.rerecord_count) << 32 | header.rerecord_count)));

        metadata.emplace_back(std::make_pair(L"Video Plugin", string_to_wstring(header.video_plugin_name)));
        metadata.emplace_back(std::make_pair(L"Input Plugin", string_to_wstring(header.input_plugin_name)));
        metadata.emplace_back(std::make_pair(L"Sound Plugin", string_to_wstring(header.audio_plugin_name)));
        metadata.emplace_back(std::make_pair(L"RSP Plugin", string_to_wstring(header.rsp_plugin_name)));
        ...
        metadata.emplace_back(std::make_pair(L"WiiVC", header.extended_version == 0 ? L"Unknown" : (header.extended_flags.wii_vc ? L"Enabled" : L"Disabled")));

        char authorship[5] = {0};
        memcpy(authorship, header.extended_data.authorship_tag, sizeof(header.extended_data.authorship_tag));

        metadata.emplace_back(std::make_pair(L"Authorship", header.extended_version == 0 ? L"Unknown" : string_to_wstring(authorship)));
    ...
    t_record_params show(bool readonly)
    {
        ...
        DialogBox(g_app_instance, MAKEINTRESOURCE(IDD_MOVIE_PLAYBACK_DIALOG), g_main_hwnd, (DLGPROC)MovieInspectorProc);
