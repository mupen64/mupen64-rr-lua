...
INT_PTR CALLBACK coredbg_dlgproc(HWND hwnd, UINT msg, WPARAM w_param,
  ...
  case WM_COMMAND:
    ...
    case IDC_COREDBG_CART_TILT:
      Debugger::set_dma_read_enabled(
          !IsDlgButtonChecked(hwnd, IDC_COREDBG_CART_TILT));
      break;
    case IDC_COREDBG_RSP_TOGGLE:
      Debugger::set_rsp_enabled(
          IsDlgButtonChecked(hwnd, IDC_COREDBG_RSP_TOGGLE));
      break;
  ...
  case WM_DEBUGGER_CPU_STATE_UPDATED: {
    ...
    char disasm[255] = {0};
    DisassembleInstruction(disasm, g_cpu_state.opcode, g_cpu_state.address);

    auto str = std::format("{} ({:#08x}, {:#08x})", disasm, g_cpu_state.opcode,
                           g_cpu_state.address);
  ...
  case WM_DEBUGGER_RESUMED_UPDATED:
    Button_SetText(GetDlgItem(g_hwnd, IDC_COREDBG_TOGGLEPAUSE),
                   Debugger::get_resumed() ? L"Pause" : L"Resume");
    break;
  default:
    return FALSE;
  }
...
void show() {
  std::thread([] {
    DialogBox(g_app_instance, MAKEINTRESOURCE(IDD_COREDBG), 0, coredbg_dlgproc);
  }).detach();
}

void init() {
  Messenger::subscribe(
      Messenger::Message::DebuggerCpuStateChanged, [](std::any data) {
  ...
  Messenger::subscribe(
      Messenger::Message::DebuggerResumedChanged, [](std::any data) {
        if (g_hwnd) {
