...
typedef struct OptionsItem {
  ...
  /**
   * Possible predefined values (e.g.: enum values) for an option along with a
   * name.
   *
  ...
  /**
   * Function which returns whether the option can be changed. Useful for values
   * which shouldn't be changed during emulation.
   */
  std::function<bool()> is_readonly = [] { return false; };

  /**
   * Gets the value name for the current backing data, or a fallback name if no
   * match is found.
   */
  ...
  void *get_default_value_ptr(const t_config *config) const {
    // Find the field offset for the option relative to g_config and grab the
    // equivalent from the default config
    size_t field_offset;
    if (type == Type::String) {
...
int32_t get_user_hotkey(t_hotkey *hotkey) {
  ...
  for (i = 0; i < 500; i++) {
    SleepEx(10, TRUE);
    for (j = 8; j < 254; j++) {
      if (j == VK_LCONTROL || j == VK_RCONTROL || j == VK_LMENU ||
          j == VK_RMENU || j == VK_LSHIFT || j == VK_RSHIFT)
        continue;
      ...
      if (GetAsyncKeyState(j) & 0x8000) {
        // HACK to avoid exiting all the way out of the dialog on pressing
        // escape to clear a hotkeys or continually re-activating the button on
        // trying to assign space as a hotkeys
        if (j == VK_ESCAPE)
          return 0;
...
INT_PTR CALLBACK plugin_discovery_dlgproc(HWND hwnd, UINT msg, WPARAM w_param,
                                          LPARAM l_param) {
  ...
  case WM_INITDIALOG: {
    ...
    g_pldlv_hwnd = CreateWindowEx(
        ...
        hwnd, nullptr, g_app_instance, NULL);

    ListView_SetExtendedListViewStyle(g_pldlv_hwnd, LVS_EX_GRIDLINES |
  ...
  case WM_NOTIFY: {
    ...
    case LVN_GETDISPINFO: {
      ...
      case 0:
        StrNCpy(plvdi->item.pszText, pair.first.filename().c_str(),
                plvdi->item.cchTextMax);
        break;
      case 1: {
        StrNCpy(plvdi->item.pszText, pair.second.c_str(),
                plvdi->item.cchTextMax);
        break;
      }
      default:
...
INT_PTR CALLBACK about_dlg_proc(const HWND hwnd, const UINT message,
                                const WPARAM w_param, LPARAM) {
  switch (message) {
  case WM_INITDIALOG:
  ...
  case WM_COMMAND:
    ...
    case IDC_WEBSITE:
      ShellExecute(nullptr, nullptr, L"http://mupen64.emulation64.com", nullptr,
                   nullptr, SW_SHOW);
      break;
    case IDC_GITREPO:
      ShellExecute(nullptr, nullptr,
                   L"https://github.com/mkdasher/mupen64-rr-lua-/", nullptr,
                   nullptr, SW_SHOW);
      break;
    default:
      break;
...
void configdialog_about() {
  DialogBox(g_app_instance, MAKEINTRESOURCE(IDD_ABOUT), g_main_hwnd,
            about_dlg_proc);
}
...
INT_PTR CALLBACK directories_cfg(const HWND hwnd, const UINT message,
                                 const WPARAM w_param, LPARAM l_param) {
  ...
  case WM_INITDIALOG:
    ...
    SendMessage(GetDlgItem(hwnd, IDC_RECURSION), BM_SETCHECK,
                g_config.is_rombrowser_recursion_enabled ? BST_CHECKED
                                                         : BST_UNCHECKED,
                0);

    if (g_config.is_default_plugins_directory_used) {
      SendMessage(GetDlgItem(hwnd, IDC_DEFAULT_PLUGINS_CHECK), BM_SETCHECK,
                  BST_CHECKED, 0);
    ...
    if (g_config.is_default_saves_directory_used) {
      SendMessage(GetDlgItem(hwnd, IDC_DEFAULT_SAVES_CHECK), BM_SETCHECK,
                  BST_CHECKED, 0);
    ...
    if (g_config.is_default_screenshots_directory_used) {
      SendMessage(GetDlgItem(hwnd, IDC_DEFAULT_SCREENSHOTS_CHECK), BM_SETCHECK,
                  BST_CHECKED, 0);
    ...
    if (g_config.is_default_backups_directory_used) {
      SendMessage(GetDlgItem(hwnd, IDC_DEFAULT_BACKUPS_CHECK), BM_SETCHECK,
                  BST_CHECKED, 0);
    ...
    SetDlgItemText(hwnd, IDC_SCREENSHOTS_DIR,
                   g_config.screenshots_directory.c_str());
  ...
  case WM_COMMAND:
    ...
    case IDC_BACKUPS_DIR:
      ...
      break;
    case IDC_RECURSION:
      g_config.is_rombrowser_recursion_enabled =
          IsDlgButtonChecked(hwnd, IDC_RECURSION) == BST_CHECKED;
      break;
    ...
    case IDC_REMOVE_BROWSER_DIR: {
      const int32_t selected_index =
          ListBox_GetCurSel(GetDlgItem(hwnd, IDC_ROMBROWSER_DIR_LIST));
      if (selected_index == -1) {
        break;
      }
      g_config.rombrowser_rom_paths.erase(
          g_config.rombrowser_rom_paths.begin() + selected_index);
      build_rom_browser_path_list(hwnd);
      break;
    }
    ...
    case IDC_DEFAULT_PLUGINS_CHECK: {
      ...
      EnableWindow(GetDlgItem(hwnd, IDC_PLUGINS_DIR),
                   !g_config.is_default_plugins_directory_used);
      EnableWindow(GetDlgItem(hwnd, IDC_CHOOSE_PLUGINS_DIR),
                   !g_config.is_default_plugins_directory_used);
    ...
    case IDC_DEFAULT_BACKUPS_CHECK: {
      ...
      EnableWindow(GetDlgItem(hwnd, IDC_BACKUPS_DIR),
                   !g_config.is_default_backups_directory_used);
      EnableWindow(GetDlgItem(hwnd, IDC_CHOOSE_BACKUPS_DIR),
                   !g_config.is_default_backups_directory_used);
    } break;
    case IDC_PLUGIN_DIRECTORY_HELP: {
      MessageBox(
          hwnd,
          L"Changing the plugin directory may introduce bugs to some plugins.",
    ...
    case IDC_DEFAULT_SAVES_CHECK: {
      g_config.is_default_saves_directory_used =
          IsDlgButtonChecked(hwnd, IDC_DEFAULT_SAVES_CHECK) == BST_CHECKED;
      EnableWindow(GetDlgItem(hwnd, IDC_SAVES_DIR),
                   !g_config.is_default_saves_directory_used);
      EnableWindow(GetDlgItem(hwnd, IDC_CHOOSE_SAVES_DIR),
                   !g_config.is_default_saves_directory_used);
    } break;
    ...
    case IDC_DEFAULT_SCREENSHOTS_CHECK: {
      g_config.is_default_screenshots_directory_used =
          IsDlgButtonChecked(hwnd, IDC_DEFAULT_SCREENSHOTS_CHECK) ==
          BST_CHECKED;
      EnableWindow(GetDlgItem(hwnd, IDC_SCREENSHOTS_DIR),
                   !g_config.is_default_screenshots_directory_used);
...
void update_plugin_selection(const HWND hwnd, const int32_t id,
                             const std::filesystem::path &path) {
  for (int i = 0; i < SendDlgItemMessage(hwnd, id, CB_GETCOUNT, 0, 0); ++i) {
    if (const auto plugin =
            (Plugin *)SendDlgItemMessage(hwnd, id, CB_GETITEMDATA, i, 0);
        plugin->path() == path) {
...
static void refresh_plugins_page(const HWND hwnd) {
  ...
  if (g_config.plugin_discovery_async) {
    ...
    g_plugin_discovery_thread =
        std::thread([=] { start_plugin_discovery(hwnd); });
  } else {
    start_plugin_discovery(hwnd);
  }
}

INT_PTR CALLBACK plugins_cfg(const HWND hwnd, const UINT message,
  ...
  case WM_INITDIALOG: {
    SendDlgItemMessage(hwnd, IDB_DISPLAY, STM_SETIMAGE, IMAGE_BITMAP,
                       (LPARAM)LoadImage(g_app_instance,
                                         MAKEINTRESOURCE(IDB_DISPLAY),
                                         IMAGE_BITMAP, 0, 0, 0));
    SendDlgItemMessage(hwnd, IDB_CONTROL, STM_SETIMAGE, IMAGE_BITMAP,
                       (LPARAM)LoadImage(g_app_instance,
                                         MAKEINTRESOURCE(IDB_CONTROL),
                                         IMAGE_BITMAP, 0, 0, 0));
    ...
    SendDlgItemMessage(hwnd, IDB_RSP, STM_SETIMAGE, IMAGE_BITMAP,
                       (LPARAM)LoadImage(g_app_instance,
                                         MAKEINTRESOURCE(IDB_RSP), IMAGE_BITMAP,
                                         0, 0, 0));
  ...
  case WM_PLUGIN_DISCOVERY_FINISHED: {
    ...
    std::ranges::copy_if(plugin_discovery_result.results,
                         std::back_inserter(broken_plugins),
    ...
    if (broken_plugins.empty()) {
      ...
      SetDlgItemText(
          ...
          std::format(
              L"Not all discovered plugins shown. {} plugin(s) failed to load.",
              broken_plugins.size())
              .c_str());
    }

    EnableWindow(GetDlgItem(hwnd, IDC_PLUGIN_DISCOVERY_INFO),
                 !broken_plugins.empty());
    ...
    for (const auto &plugin : plugin_discovery_result.plugins) {
      ...
      SendDlgItemMessage(
          ...
          reinterpret_cast<LPARAM>(string_to_wstring(plugin->name()).c_str()));
      SendDlgItemMessage(hwnd, id, CB_SETITEMDATA, i, (LPARAM)plugin.get());
    }

    update_plugin_selection(hwnd, IDC_COMBO_GFX,
                            g_config.selected_video_plugin);
    update_plugin_selection(hwnd, IDC_COMBO_SOUND,
                            g_config.selected_audio_plugin);
    update_plugin_selection(hwnd, IDC_COMBO_INPUT,
                            g_config.selected_input_plugin);
    update_plugin_selection(hwnd, IDC_COMBO_RSP, g_config.selected_rsp_plugin);

    const auto ids_to_enable = {
        IDM_VIDEO_SETTINGS, IDM_AUDIO_SETTINGS, IDM_INPUT_SETTINGS,
        IDM_RSP_SETTINGS,   IDGFXTEST,          IDSOUNDTEST,
        IDINPUTTEST,        IDRSPTEST,          IDGFXABOUT,
        IDSOUNDABOUT,       IDINPUTABOUT,       IDRSPABOUT,
    };
  ...
  case WM_COMMAND:
    ...
    case IDC_COMBO_RSP: {
      auto has_plugin_selected =
          ComboBox_GetItemData(
              GetDlgItem(hwnd, LOWORD(w_param)),
              ComboBox_GetCurSel(GetDlgItem(hwnd, LOWORD(w_param)))) &&
    ...
    case IDC_PLUGIN_DISCOVERY_INFO:
      DialogBox(g_app_instance, MAKEINTRESOURCE(IDD_PLUGIN_DISCOVERY_RESULTS),
                hwnd, plugin_discovery_dlgproc);
      break;
    default:
      break;
    }
  ...
  case WM_NOTIFY:
    ...
    if (lpnmhdr->code == PSN_APPLY) {
      if (const auto plugin = get_selected_plugin(hwnd, IDC_COMBO_GFX);
          plugin != nullptr) {
        g_config.selected_video_plugin = plugin->path().wstring();
      }
      if (const auto plugin = get_selected_plugin(hwnd, IDC_COMBO_SOUND);
          plugin != nullptr) {
        g_config.selected_audio_plugin = plugin->path().wstring();
      }
      if (const auto plugin = get_selected_plugin(hwnd, IDC_COMBO_INPUT);
          plugin != nullptr) {
        g_config.selected_input_plugin = plugin->path().wstring();
      }
      if (const auto plugin = get_selected_plugin(hwnd, IDC_COMBO_RSP);
          plugin != nullptr) {
...
void get_config_listview_items(std::vector<t_options_group> &groups,
                               std::vector<t_options_item> &options) {
  size_t id = 1;
  ...
  t_options_group seek_piano_roll_group = {.id = ++id,
  ...
  t_options_group hotkey_group = {.id = ++id, .name = L"Hotkeys"};

  groups = {interface_group, statusbar_group, seek_piano_roll_group,
            flow_group,      capture_group,   core_group,
            lua_group,       debug_group,     hotkey_group};
  ...
  options = {
      ...
      t_options_item{
          ...
          .name = L"Silent Mode",
          .tooltip =
              L"Suppresses all dialogs and chooses reasonable defaults for "
              L"multiple-choice dialogs.\nCan cause data loss during normal "
              L"usage; only enable in automation scenarios!",
      ...
      t_options_item{
          ...
          .tooltip = L"Keep the working directory specified by the caller "
                     L"program at startup.\nWhen disabled, mupen changes the "
                     L"working directory to its current path.",
      ...
      t_options_item{
          ...
          .tooltip =
              L"Whether the new async executor is used for async "
              L"calls.\nLowers interaction latency in general usecases.\nOn - "
              L"Each call is put on the async execution queue\nOff - Each call "
              L"runs on a newly created thread (legacy behaviour)",
      ...
      t_options_item{
          ...
          .tooltip = L"Whether plugins discovery is performed asynchronously. "
                     L"Removes potential waiting times in the config dialog.",
      ...
      t_options_item{
          ...
          .tooltip = L"The statusbar layout preset.\nClassic - The legacy "
                     L"layout\nModern - The new layout containing additional "
                     L"information\nModern+ - The new layout, but with a "
                     L"section for read-only status",
          ...
          .possible_values =
              {
                  ...
                  std::make_pair(L"Modern+",
                                 (int32_t)StatusbarLayout::ModernWithReadonly),
              },
      },
      t_options_item{
          .group_id = statusbar_group.id,
          .name = L"Zero-index",
          .tooltip = L"Show indicies in the statusbar, such as VCR frame "
                     L"counts, relative to 0 instead of 1.",
      ...
      t_options_item{
          ...
          .tooltip =
              L"Whether the statusbar is allowed to scale its segments down.",
      ...
      t_options_item{
          ...
          .tooltip =
              L"Whether the statusbar is allowed to scale its segments up.",
      ...
      t_options_item{
          ...
          .tooltip =
              L"The interval at which to create savestates for seeking. Piano "
              L"Roll is exclusively read-only if this value is 0.\nHigher "
              L"numbers will reduce the seek duration at cost of emulator "
              L"performance, a value of 1 is not allowed.\n0 - Seek savestate "
              L"generation disabled\nRecommended: 100",
          .data = &g_config.seek_savestate_interval,
          .type = t_options_item::Type::Number,
          .is_readonly = [] { return VCR::get_task() != e_task::idle; },
      },
      t_options_item{
          .group_id = seek_piano_roll_group.id,
          .name = L"Savestate Max Count",
          .tooltip = L"The maximum amount of savestates to keep in memory for "
                     L"seeking.\nHigher numbers might cause an out of memory "
                     L"exception.",
      ...
      t_options_item{
          ...
          .tooltip = L"Whether piano roll edits are constrained to the column "
                     L"they started on.",
      ...
      t_options_item{
          ...
          .tooltip =
              L"Whether the piano roll will try to keep the selection visible.",
      ...
      t_options_item{
          ...
          .tooltip =
              L"Whether the piano roll will try to keep the playhead visible.",
      ...
      t_options_item{
          ...
          .name = L"Delay",
          .tooltip = L"Miliseconds to wait before capturing a frame. Useful "
                     L"for syncing with external programs.",
      ...
      t_options_item{
          ...
          .name = L"Encoder",
          .tooltip =
              L"The encoder to use when generating an output file.\nVFW - Slow "
              L"but stable (recommended)\nFFmpeg - Fast but less stable",
          .data = &g_config.encoder_type,
          .type = t_options_item::Type::Enum,
          .possible_values =
              {
                  std::make_pair(L"VFW", (int32_t)EncoderType::VFW),
                  std::make_pair(L"FFmpeg (experimental)",
                                 (int32_t)EncoderType::FFmpeg),
              },
      ...
      t_options_item{
          ...
          .name = L"Mode",
          .tooltip =
              L"The video source to use for capturing video frames.\nPlugin - "
              L"Captures frames solely from the video plugin\nWindow - "
              L"Captures frames from the main window\nScreen - Captures "
              L"screenshots of the current display and crops them to "
      ...
      t_options_item{
          ...
          .name = L"Sync",
          .tooltip =
              L"The strategy to use for synchronizing video and audio during "
              L"capture.\nNone - No synchronization\nAudio - Audio is "
              L"synchronized to video\nVideo - Video is synchronized to audio",
      ...
      t_options_item{
          ...
          .tooltip = L"The argument format string to be passed to FFmpeg when "
                     L"capturing.",
      ...
      t_options_item{
          ...
          .tooltip = L"The core type to utilize for emulation.\nInterpreter - "
                     L"Slow and relatively accurate\nDynamic Recompiler - "
                     L"Fast, possibly less accurate, and only for x86 "
          ...
          .is_readonly = [] { return emu_launched; },
      },
      t_options_item{
          .group_id = core_group.id,
      ...
      t_options_item{
          ...
          .tooltip = L"The maximum amount of lag frames before the core emits "
                     L"a warning\n0 - Disabled",
      ...
      t_options_item{
          ...
          .name = L"Audio Delay",
          .tooltip =
              L"Whether to delay audio interrupts\nEnabled - More stability",
      ...
      t_options_item{
          ...
          .tooltip =
              L"Automatically increment the save slot upon saving a state.",
      ...
      t_options_item{
          ...
          .tooltip = L"Emulate float operation-related crashes which would "
                     L"also crash on real hardware",
      ...
      t_options_item{
          ...
          .tooltip =
              L"Generate a backup of the currently recorded movie when loading "
      ...
      t_options_item{
          ...
          .tooltip = L"Skip rendering every nth frame when in fast-forward "
                     L"mode.\n0 - Render nothing\n1 - Render every frame\nn - "
                     L"Render every nth frame",
      ...
      t_options_item{
          ...
          .tooltip =
              L"Record manually performed resets to the current movie.\nThese "
              L"resets will be repeated when the movie is played back.",
      ...
      t_options_item{
          ...
          .tooltip = L"Enable SD card emulation.\nRequires a VHD-formatted SD "
                     L"card file named card.vhd in the same folder as Mupen.",
          .data = &g_config.is_reset_recording_enabled,
          .type = t_options_item::Type::Bool,
      },
      t_options_item{
          ...
          .tooltip =
              L"Saves and loads game graphics to savestates to allow instant "
              L"graphics updates when loading savestates.\nGreatly increases "
              L"savestate saving and loading time.",
      ...
      t_options_item{
          ...
          .tooltip =
              L"Prevents calls to updateScreen during lag.\nMight improve "
              L"performance on some video plugins at the cost of stability.",
      ...
      t_options_item{
          ...
          .tooltip =
              L"Size of the ROM cache.\nImproves ROM loading performance at "
              L"the cost of data staleness and high memory usage.\n0 - "
      ...
      t_options_item{
          ...
          .name = L"Presenter",
          .tooltip = L"The presenter type to use for displaying and capturing "
                     L"Lua graphics.\nRecommended: DirectComposition",
          .data = &g_config.presenter_type,
          .type = t_options_item::Type::Enum,
          .possible_values =
              {
                  std::make_pair(L"DirectComposition",
                                 (int32_t)PresenterType::DirectComposition),
                  std::make_pair(L"GDI", (int32_t)PresenterType::GDI),
              },
          .is_readonly = [] { return !g_hwnd_lua_map.empty(); },
      },
      ...
      t_options_item{
          ...
          .tooltip =
              L"Whether the async executor will apply concurrency fuzzing. "
              L"When enabled, task execution will be delayed to expose delayed "
              L"task execution handling deficiencies at the callsite.\nDo not "
              L"enable unless you are debugging the async executor.",
  ...
  for (const auto hotkey : g_config_hotkeys) {
    options.push_back(t_options_item{
        ...
        .tooltip = std::format(L"{} hotkey.\nAction down: #{}\nAction up: #{}",
                               hotkey->identifier, (int32_t)hotkey->down_cmd,
                               (int32_t)hotkey->up_cmd),
...
LRESULT CALLBACK InlineEditBoxProc(HWND hwnd, UINT msg, WPARAM wParam,
                                   LPARAM lParam, UINT_PTR sId,
                                   DWORD_PTR dwRefData) {
  switch (msg) {
  case WM_GETDLGCODE: {
    if (wParam == VK_RETURN) {
...
INT_PTR CALLBACK EditStringDialogProc(HWND wnd, UINT msg, WPARAM wParam,
                                      LPARAM lParam) {
  switch (msg) {
  case WM_INITDIALOG: {
...
void advance_listview_selection() {
  ...
  ListView_SetItemState(g_lv_hwnd, i + 1, LVIS_SELECTED | LVIS_FOCUSED,
...
bool begin_listview_edit(HWND hwnd) {
  ...
  if (option_item.type == OptionsItem::Type::Enum) {
    // 1. Find the index of the currently selected item, while falling back to
    // the first possible value if there's no match
  ...
  // For strings, allow editing in a dialog (since it might be a multiline
  // string and we can't really handle that below)
  if (option_item.type == OptionsItem::Type::String) {
    g_edit_option_item_index = item.lParam;
    DialogBoxParam(g_app_instance, MAKEINTRESOURCE(IDD_LUAINPUTPROMPT), hwnd,
                   EditStringDialogProc, 0);
  }
  ...
  if (option_item.type == OptionsItem::Type::Number) {
    ...
    g_edit_hwnd = CreateWindowEx(
        WS_EX_CLIENTEDGE, L"EDIT", L"", WS_CHILD | WS_VISIBLE | WS_TABSTOP,
        item_rect.left, item_rect.top, item_rect.right - item_rect.left,
        item_rect.bottom - item_rect.top, hwnd, 0, g_app_instance, 0);

    SendMessage(g_edit_hwnd, WM_SETFONT,
                (WPARAM)SendMessage(g_lv_hwnd, WM_GETFONT, 0, 0), 0);
...
LRESULT CALLBACK list_view_proc(HWND hwnd, UINT msg, WPARAM w_param,
...
INT_PTR CALLBACK general_cfg(const HWND hwnd, const UINT message,
                             const WPARAM w_param, const LPARAM l_param) {
  const auto lpnmhdr = reinterpret_cast<LPNMHDR>(l_param);
  ...
  case WM_INITDIALOG: {
    ...
    g_lv_hwnd = CreateWindowEx(
        WS_EX_CLIENTEDGE, WC_LISTVIEW, NULL,
        WS_TABSTOP | WS_VISIBLE | WS_CHILD | LVS_SINGLESEL | LVS_REPORT |
            LVS_SHOWSELALWAYS | LVS_ALIGNTOP,
        grid_rect.left, grid_rect.top, grid_rect.right - grid_rect.left,
        grid_rect.bottom - grid_rect.top, hwnd, (HMENU)IDC_SETTINGS_LV,
        g_app_instance, NULL);
    SetWindowSubclass(g_lv_hwnd, list_view_proc, 0, 0);

    HIMAGELIST image_list =
        ImageList_Create(16, 16, ILC_COLORDDB | ILC_MASK, 2, 0);
    ImageList_AddIcon(image_list,
                      LoadIcon(g_app_instance, MAKEINTRESOURCE(IDI_DENY)));
    ImageList_AddIcon(image_list,
                      LoadIcon(g_app_instance, MAKEINTRESOURCE(IDI_CHANGED)));
    ListView_SetImageList(g_lv_hwnd, image_list, LVSIL_SMALL);

    ListView_EnableGroupView(g_lv_hwnd, true);
    ListView_SetExtendedListViewStyle(
        g_lv_hwnd, LVS_EX_FULLROWSELECT | LVS_EX_DOUBLEBUFFER | LVS_EX_INFOTIP |
                       LVS_EX_LABELTIP);
  ...
  case WM_EDIT_END: {
    ...
    if (g_option_items[g_edit_option_item_index].type ==
        OptionsItem::Type::Number) {
      try {
  ...
  case WM_CONTEXTMENU: {
    ...
    AppendMenu(h_menu, MF_STRING | (readonly ? MF_DISABLED : MF_ENABLED), 1,
               L"Reset to default");
    ...
    const int offset =
        TrackPopupMenuEx(h_menu, TPM_RETURNCMD | TPM_NONOTIFY,
                         GET_X_LPARAM(l_param), GET_Y_LPARAM(l_param), hwnd, 0);

    if (offset < 0) {
      break;
    }

    void *default_equivalent =
        option_item.get_default_value_ptr(&g_default_config);
    ...
    if (offset == 2) {
      // Show a MessageBox with the tooltip and all possible values along with
      // the default value.
      ...
      FrontendService::show_dialog(str.c_str(), L"Information",
    ...
    if (offset == 5) {
      ...
      if (!can_all_be_changed) {
        FrontendService::show_dialog(
            L"Some settings can't be reset, as they are currently read-only. "
            L"Try again with emulation stopped.\nNo changes have been made to "
            L"the settings.",
            L"Reset all to default", FrontendService::DialogType::Warning,
            hwnd);
        goto destroy_menu;
      }

      const auto result = FrontendService::show_ask_dialog(
          L"Are you sure you want to reset all settings to default?",
  ...
  case WM_NOTIFY: {
    ...
    if (w_param == IDC_SETTINGS_LV) {
      ...
      case LVN_GETDISPINFO: {
        ...
        if (plvdi->item.mask & LVIF_IMAGE) {
          void *default_value_ptr =
              options_item.get_default_value_ptr(&g_prev_config);

          if (options_item.type == OptionsItem::Type::String) {
            plvdi->item.iImage =
                *(std::wstring *)default_value_ptr == *options_item.data_str
                    ? 50
                    : 1;
            ...
            plvdi->item.iImage =
                *(int32_t *)default_value_ptr == *options_item.data ? 50 : 1;
          }
        ...
        switch (plvdi->item.iSubItem) {
        case 0:
          StrNCpy(plvdi->item.pszText, options_item.name.c_str(),
                  plvdi->item.cchTextMax);
          break;
        case 1:
          if (g_hotkey_active_index.has_value() &&
              g_hotkey_active_index.value() == plvdi->item.iItem) {
            StrNCpy(plvdi->item.pszText, L"... (RMB to cancel)",
                    plvdi->item.cchTextMax);
            break;
          }
          StrNCpy(plvdi->item.pszText, options_item.get_value_name().c_str(),
                  plvdi->item.cchTextMax);
          break;
        }
        break;
      ...
      case LVN_GETINFOTIP: {
        ...
        StrCpyNW(getinfotip->pszText, option_item.tooltip.c_str(),
