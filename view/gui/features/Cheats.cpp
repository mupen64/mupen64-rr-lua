...
LRESULT CALLBACK WndProc(HWND hwnd, UINT Message, WPARAM wParam,
  ...
  case WM_COMMAND:
    ...
    case IDC_NEW_CHEAT: {
      auto script = Gameshark::Script::compile(
          L"D033AFA1 0020\n8133B1BC 4220\nD033AFA1 0020\n8133B17C "
          L"0300\nD033AFA1 0020\n8133B17E 0880");
      if (script.has_value()) {
    ...
    case IDC_CHECK_CHEAT_ENABLED: {
      ...
      script->set_resumed(IsDlgButtonChecked(hwnd, IDC_CHECK_CHEAT_ENABLED) ==
                          BST_CHECKED);
      goto rebuild_list;
    }
    ...
    case IDC_CHEAT_APPLY: {
      ...
      // Replace with script recompiled with new code, while keeping some
      // properties Old script goes out of ref
      ...
      if (!script.has_value()) {
        FrontendService::show_dialog(
            ...
            L"correct",
            L"Cheats", FrontendService::DialogType::Error);
        break;
      }
...
update_selection: {
  ...
  CheckDlgButton(hwnd, IDC_CHECK_CHEAT_ENABLED,
                 Gameshark::scripts[selected_index]->resumed() ? BST_CHECKED
                                                               : BST_UNCHECKED);
  SetDlgItemText(hwnd, IDC_EDIT_CHEAT,
                 Gameshark::scripts[selected_index]->code().c_str());
  Edit_SetText(GetDlgItem(hwnd, IDC_EDIT_CHEAT_NAME),
               Gameshark::scripts[selected_index]->name().c_str());
}
  return FALSE;
...
rebuild_list: {
  ...
  for (auto script : Gameshark::scripts) {
    auto name =
        !script->resumed() ? script->name() + L" (Disabled)" : script->name();
...
void show() {
  DialogBox(g_app_instance, MAKEINTRESOURCE(IDD_CHEATS), g_main_hwnd,
            (DLGPROC)WndProc);
}
} // namespace Cheats
