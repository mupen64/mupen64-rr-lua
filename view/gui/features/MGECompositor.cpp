...
namespace MGECompositor
{
        ...
        case WM_PAINT:
            {
                ...
                {
                    StretchDIBits(hdc, rect.top, rect.left, rect.right, rect.bottom, 0, 0, vbuf->bmp_info.bmiHeader.biWidth, vbuf->bmp_info.bmiHeader.biHeight, vbuf->buffer, &(vbuf->bmp_info), DIB_RGB_COLORS, SRCCOPY);
                }
    ...
    void create(HWND hwnd) { control_hwnd = CreateWindow(CONTROL_CLASS_NAME, L"", WS_CHILD | WS_VISIBLE, 0, 0, 1, 1, hwnd, nullptr, g_app_instance, nullptr); }
    ...
    void copy_video(void* buffer) { memcpy(buffer, internal_buffer.buffer, internal_buffer.width * internal_buffer.height * 3); }
