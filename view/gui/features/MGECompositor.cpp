...
LRESULT CALLBACK wndproc(HWND hwnd, UINT msg, WPARAM wparam, LPARAM lparam) {
  ...
  case WM_PAINT: {
    ...
    if (vbuf->dib) {
      ...
      StretchDIBits(hdc, rect.top, rect.left, rect.right, rect.bottom, 0, 0,
                    vbuf->bmp_info.bmiHeader.biWidth,
                    vbuf->bmp_info.bmiHeader.biHeight, vbuf->buffer,
                    &(vbuf->bmp_info), DIB_RGB_COLORS, SRCCOPY);
    }
...
void create(HWND hwnd) {
  control_hwnd = CreateWindow(CONTROL_CLASS_NAME, L"", WS_CHILD | WS_VISIBLE, 0,
                              0, 1, 1, hwnd, nullptr, g_app_instance, nullptr);
}
...
void init() {
  ...
  Messenger::subscribe(
      Messenger::Message::EmuLaunchedChanged, [](std::any data) {
        auto value = std::any_cast<bool>(data);
        ShowWindow(control_hwnd, (value && available()) ? SW_SHOW : SW_HIDE);
      });
}

bool available() { return ::get_video_size && read_video; }

void update_screen() {
  ::get_video_size(&internal_buffer.width, &internal_buffer.height);

  if (internal_buffer.width != internal_buffer.last_width ||
      internal_buffer.height != internal_buffer.last_height) {
    SetWindowLongPtr(control_hwnd, GWLP_USERDATA, (LONG_PTR)&internal_buffer);
    g_view_logger->info("MGE Compositor: Video size {}x{}",
    ...
    internal_buffer.dib =
        CreateDIBSection(internal_buffer.dc, &internal_buffer.bmp_info,
                         DIB_RGB_COLORS, &internal_buffer.buffer, nullptr, 0);
    SelectObject(internal_buffer.dc, internal_buffer.dib);

    ReleaseDC(control_hwnd, dc);

    MoveWindow(control_hwnd, 0, 0, internal_buffer.width,
               internal_buffer.height, true);
  }
...
void copy_video(void *buffer) {
  memcpy(buffer, internal_buffer.buffer,
         internal_buffer.width * internal_buffer.height * 3);
}
...
void load_screen(void *data) {
  g_main_window_dispatcher->invoke([=] {
    ...
    external_buffer.buffer =
        malloc(external_buffer.width * external_buffer.height * 3);
    memcpy(external_buffer.buffer, data,
           external_buffer.width * external_buffer.height * 3);

    MoveWindow(control_hwnd, 0, 0, external_buffer.width,
               external_buffer.height, true);
