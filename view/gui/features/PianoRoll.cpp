...
struct PianoRollState {
  // The input buffer for the piano roll, which is a copy of the inputs from the
  // core and is modified by the user. When editing operations end, this buffer
  // is provided to begin_warp_modify and thereby applied to the core, changing
  // the resulting emulator state.
...
// The clipboard buffer for piano roll copy/paste operations. Must be sorted
// ascendingly.
//
// Due to allowing sparse ("extended") selections, we might have gaps in the
// clipboard buffer as well as the selected indicies when pasting. When a
// gap-containing clipboard buffer is pasted into the piano roll, the selection
// acts as a mask.
//
// A----
// -----
...
//
// This also applies for the inverse (gapless clipboard buffer and
// g_piano_roll_state.selected_indicies with gaps).
//
...
// The value of the cell under the mouse at the time when the drag operation
// started
...
// Whether the drag operation should unset the affected buttons regardless of
// the initial value
...
// Stack index for the piano roll undo/redo stack. 0 = top, 1 = 2nd from top,
// etc...
...
/**
 * Gets whether inputs can be modified. Affects both the piano roll and the
 * joystick.
 */
bool can_modify_inputs() {
  return VCR::get_warp_modify_status() == e_warp_modify_status::none &&
         VCR::get_seek_completion().second == SIZE_MAX &&
         VCR::get_task() == e_task::recording && !VCR::is_seeking() &&
         !g_config.vcr_readonly && g_config.seek_savestate_interval > 0 &&
         emu_paused;
}
 ...
 */
bool can_seek() { return g_config.seek_savestate_interval > 0; }

/**
 * Refreshes the piano roll listview and the joystick, re-querying the current
...
void update_inputs() {
  ...
  // In playback mode, the input buffer can't change so we're safe to only pull
  // it once.
  if (VCR::get_task() == e_task::playback) {
    ...
    g_view_logger->info(
        "[PianoRoll] Pulled inputs from core for playback mode, count: {}",
...
void print_clipboard_dump() {
  ...
  for (auto item : g_clipboard) {
    item.has_value()
        ? g_view_logger->info("[PianoRoll] {:#06x}", item.value().Value)
...
/**
 * Ensures that the currently relevant item is visible in the piano roll
 * listview.
 */
void ensure_relevant_item_visible() {
  const int32_t i = ListView_GetNextItem(g_lv_hwnd, -1, LVNI_SELECTED);
  const auto current_sample = min(ListView_GetItemCount(g_lv_hwnd),
                                  VCR::get_seek_completion().first + 10);
  const auto playhead_sample = VCR::get_task() == e_task::recording
                                   ? current_sample - 1
                                   : current_sample;
...
void copy_inputs() {
  ...
  if (g_piano_roll_state.selected_indicies.size() == 1) {
    g_clipboard = {
        g_piano_roll_state.inputs[g_piano_roll_state.selected_indicies[0]]};
    return;
  }
  ...
  const size_t max =
      g_piano_roll_state
          .selected_indicies[g_piano_roll_state.selected_indicies.size() - 1];
  ...
  for (auto i = min; i <= max; ++i) {
    ...
    const bool gap =
        std::ranges::find(g_piano_roll_state.selected_indicies, i) ==
...
/**
 * Pushes the current piano roll state to the history. Should be called after
 * operations which change the piano roll state.
 */
void push_state_to_history() {
  ...
  g_piano_roll_state_index =
      min(g_piano_roll_state_index + 1, g_piano_roll_history.size() - 1);

  g_view_logger->info("[PianoRoll] Undo stack size: {}. Current index: {}.",
                      g_piano_roll_history.size(), g_piano_roll_state_index);
...
void apply_input_buffer(bool push_to_history = true) {
  if (!g_inputs_different) {
    g_view_logger->trace("[PianoRoll] Ignoring apply_input_buffer because "
                         "inputs didn't change.");
    return;
  }

  if (!can_modify_inputs()) {
    g_view_logger->warn("[PianoRoll] Tried to call apply_input_buffer, but "
                        "can_modify_inputs() == false.");
    return;
  }

  // This might be called from UI thread, thus grabbing the VCR lock.
  // Problem is that the VCR lock is already grabbed by the core thread because
  // current sample changed message is executed on core thread.
  AsyncExecutor::invoke_async([=] {
    ...
    g_piano_roll_dispatcher->invoke([=] {
      if (result == CoreResult::Ok) {
        ...
        // Since we do optimistic updates, we need to revert the changes we made
        // to the input buffer to avoid visual desync
        SetWindowRedraw(g_lv_hwnd, false);
        ...
        g_piano_roll_state.inputs = VCR::get_inputs();
        ListView_SetItemCount(g_lv_hwnd, g_piano_roll_state.inputs.size());
        g_view_logger->info("[PianoRoll] Pulled inputs from core for recording "
                            "mode due to warp modify failing, count: {}",
                            g_piano_roll_state.inputs.size());
...
/**
 * Sets the piano roll state to the specified value, updating everything
 * accordingly and also applying the input buffer. This is an expensive and slow
 * operation.
 */
void set_piano_roll_state(const PianoRollState piano_roll_state) {
  ...
  ListView_SetItemCountEx(g_lv_hwnd, g_piano_roll_state.inputs.size(),
                          LVSICF_NOSCROLL);
...
void paste_inputs(bool merge) {
  ...
  if (g_piano_roll_state.selected_indicies.size() > 1) {
    for (int i = 1; i < g_piano_roll_state.selected_indicies.size(); ++i) {
      if (g_piano_roll_state.selected_indicies[i] -
              g_piano_roll_state.selected_indicies[i - 1] >
          1) {
  ...
  g_view_logger->info("[PianoRoll] Clipboard/selection gaps: {}, {}",
                      clipboard_has_gaps, selection_has_gaps);

  SetWindowRedraw(g_lv_hwnd, false);

  if (g_piano_roll_state.selected_indicies.size() == 1) {
    // 1-sized selection indicates a bulk copy, where copy all the inputs over
    // (and ignore the clipboard gaps)
    ...
    for (auto item : g_clipboard) {
      if (item.has_value() && i < g_piano_roll_state.inputs.size()) {
        g_piano_roll_state.inputs[i] =
            merge ? BUTTONS{g_piano_roll_state.inputs[i].Value |
                            item.value().Value}
                  : item.value();
...
/**
 * Deletes all inputs in the current selection, removing them from the input
 * buffer.
 */
void delete_inputs_in_selection() {
  ...
  std::vector<size_t> selected_indicies(
      g_piano_roll_state.selected_indicies.begin(),
      g_piano_roll_state.selected_indicies.end());
  g_piano_roll_state.inputs =
      erase_indices(g_piano_roll_state.inputs, selected_indicies);
...
/**
 * Appends the specified amount of empty frames at the start of the current
 * selection.
 */
bool insert_frames(size_t count) {
  ...
  for (int i = 0; i < count; ++i) {
    g_piano_roll_state.inputs.insert(
        g_piano_roll_state.inputs.begin() +
            g_piano_roll_state.selected_indicies[0] + 1,
        {0});
  }
...
void update_groupbox_status_text() {
  ...
  if (g_piano_roll_state.selected_indicies.empty()) {
    ...
    SetDlgItemText(g_hwnd, IDC_STATIC,
                   std::format(L"Input - Frame {}",
                               g_piano_roll_state.selected_indicies[0])
                       .c_str());
  } else {
    SetDlgItemText(g_hwnd, IDC_STATIC,
                   std::format(L"Input - {} frames selected",
                               g_piano_roll_state.selected_indicies.size())
                       .c_str());
  }
}
...
void on_task_changed(std::any data) {
  g_piano_roll_dispatcher->invoke([=] {
    ...
    if (value != previous_value) {
      g_view_logger->info("[PianoRoll] Processing TaskChanged from {} to {}",
                          (int32_t)previous_value, (int32_t)value);
      update_inputs();
    }

    if (g_config.seek_savestate_interval == 0) {
      SetWindowText(g_status_hwnd,
                    L"Piano Roll read-only.\nSeek savestates must be enabled.");
    } else {
...
void on_unfreeze_completed(std::any) {
  g_piano_roll_dispatcher->invoke([=] {
    ...
    const auto item_count = VCR::get_task() == e_task::recording
                                ? min(VCR::get_seek_completion().first,
                                      g_piano_roll_state.inputs.size())
                                : g_piano_roll_state.inputs.size();

    g_view_logger->info(
        "[PianoRoll] Setting item count to {} (input count: {})...", item_count,
        g_piano_roll_state.inputs.size());
...
void on_emu_paused_changed(std::any) {
  // Redrawing during frame advance (paused on, then off next frame) causes ugly
  // flicker, so we'll just not do that
 ...
 */
LRESULT CALLBACK joystick_proc(HWND hwnd, UINT msg, WPARAM wParam,
                               LPARAM lParam) {
  switch (msg) {
  case WM_LBUTTONDOWN:
    if (!can_joystick_be_modified()) {
  ...
  case WM_PAINT: {
    ...
    g_view_logger->info(
        "[PianoRoll] Joystick repaint, can_joystick_be_modified: {}",
        can_joystick_be_modified());
...
mouse_move:
  ...
  int x = (pt.x * UINT8_MAX / (signed)(pic_rect.right - pic_rect.left) -
           INT8_MAX + 1);
  int y = -(pt.y * UINT8_MAX / (signed)(pic_rect.bottom - pic_rect.top) -
            INT8_MAX + 1);
 ...
 */
LRESULT CALLBACK list_view_proc(HWND hwnd, UINT msg, WPARAM wParam,
                                LPARAM lParam, UINT_PTR sId, DWORD_PTR) {
  switch (msg) {
  case WM_CONTEXTMENU: {
  ...
  case WM_RBUTTONDOWN: {
    ...
    if (lplvhtti.iItem < 0 ||
        lplvhtti.iItem >= g_piano_roll_state.inputs.size()) {
      g_view_logger->info("[PianoRoll] iItem out of range");
      goto def;
    }

    // Don't start a dragging operation if we're trying to modify read-only
    // inputs
    ...
    if (lplvhtti.iSubItem >= 4) {
      ...
      g_lv_drag_initial_value =
          !get_input_value_from_column_index(input, g_lv_drag_column);
...
handle_mouse_move:
  ...
  // Disable dragging if the corresponding mouse button was released. More
  // reliable to do this here instead of MOUSE_XBUTTONDOWN.
  ...
  // During a drag operation, we just mutate the input vector in memory and
  // update the listview without doing anything with the core. Only when the
  // drag ends do we actually apply the changes to the core via
  // begin_warp_modify

  const auto column = g_config.piano_roll_constrain_edit_to_column
                          ? g_lv_drag_column
                          : lplvhtti.iSubItem;
  ...
  set_input_value_from_column_index(&g_piano_roll_state.inputs[lplvhtti.iItem],
                                    column, new_value);
  ListView_Update(hwnd, lplvhtti.iItem);

  // If we are editing a row inside the selection, we want to apply the same
  // modify operation to the other selected rows.
  if (std::ranges::find(g_piano_roll_state.selected_indicies, lplvhtti.iItem) !=
    ...
    for (const auto &i : g_piano_roll_state.selected_indicies) {
      set_input_value_from_column_index(&g_piano_roll_state.inputs[i], column,
                                        new_value);
 ...
 */
LRESULT CALLBACK dialog_proc(HWND hwnd, UINT msg, WPARAM wParam,
                             LPARAM lParam) {
  switch (msg) {
  case WM_EXECUTE_DISPATCHER:
    ...
    break;
  case WM_INITDIALOG: {
    // We create all the child controls here because windows dialog scaling
    // would mess our stuff up when mixing dialog manager and manual creation
    g_hwnd = hwnd;
    g_joy_hwnd = CreateWindowEx(WS_EX_STATICEDGE, JOYSTICK_CLASS, L"",
                                WS_CHILD | WS_VISIBLE, 17, 30, 131, 131, g_hwnd,
                                nullptr, g_app_instance, nullptr);
    CreateWindowEx(0, WC_STATIC, L"History",
                   WS_CHILD | WS_VISIBLE | WS_GROUP | SS_LEFT | SS_CENTERIMAGE,
                   17, 166, 131, 15, g_hwnd, nullptr, g_app_instance, nullptr);
    g_hist_hwnd = CreateWindowEx(
        WS_EX_CLIENTEDGE, WC_LISTBOX, L"",
        WS_CHILD | WS_VISIBLE | WS_VSCROLL | LBS_NOINTEGRALHEIGHT | LBS_NOTIFY,
        17, 186, 131, 181, g_hwnd, nullptr, g_app_instance, nullptr);
    g_status_hwnd = CreateWindowEx(
        0, WC_STATIC, L"", WS_CHILD | WS_VISIBLE | WS_GROUP | SS_LEFT, 17, 370,
        131, 60, g_hwnd, nullptr, g_app_instance, nullptr);
    ...
    const auto lv_style = WS_TABSTOP | WS_VISIBLE | WS_CHILD | LVS_REPORT |
                          LVS_ALIGNTOP | LVS_NOSORTHEADER | LVS_SHOWSELALWAYS |
                          LVS_OWNERDATA;

    RECT grid_rect = get_window_rect_client_space(
        hwnd, GetDlgItem(hwnd, IDC_LIST_PIANO_ROLL));

    g_lv_hwnd = CreateWindowEx(
        WS_EX_CLIENTEDGE, WC_LISTVIEW, nullptr, lv_style, grid_rect.left,
        grid_rect.top, grid_rect.right - grid_rect.left,
        grid_rect.bottom - grid_rect.top, hwnd, (HMENU)IDC_PIANO_ROLL_LV,
        g_app_instance, nullptr);
    SetWindowSubclass(g_lv_hwnd, list_view_proc, 0, 0);

    ListView_SetExtendedListViewStyle(g_lv_hwnd, LVS_EX_GRIDLINES |
                                                     LVS_EX_FULLROWSELECT |
                                                     LVS_EX_DOUBLEBUFFER);

    HIMAGELIST image_list =
        ImageList_Create(16, 16, ILC_COLORDDB | ILC_MASK, 1, 0);
    ImageList_AddIcon(image_list,
                      LoadIcon(g_app_instance, MAKEINTRESOURCE(IDI_CURRENT)));
    ImageList_AddIcon(image_list,
                      LoadIcon(g_app_instance, MAKEINTRESOURCE(IDI_MARKER)));
    ...
    // HACK: Insert and then delete dummy column to have all columns
    // center-aligned
    ...
    for (int i = 4; i <= 15; ++i) {
      lv_column.pszText =
          const_cast<LPWSTR>(get_button_name_from_column_index(i));
    ...
    on_current_sample_changed(
        static_cast<int32_t>(VCR::get_seek_completion().first));
    update_groupbox_status_text();
    update_history_listbox();
  ...
  case WM_SIZE: {
    ...
    SetWindowPos(g_lv_hwnd, nullptr, 0, 0, rect.right - 10 - lv_rect.left,
                 rect.bottom - 10 - lv_rect.top, SWP_NOMOVE | SWP_NOZORDER);
    SetWindowPos(gp_hwnd, nullptr, 0, 0, gp_rect.right - gp_rect.left,
                 rect.bottom - 10 - gp_rect.top, SWP_NOMOVE | SWP_NOZORDER);
    break;
  }
  case WM_COMMAND:
  ...
  case WM_NOTIFY: {
    ...
    case LVN_GETDISPINFO: {
      ...
      if (plvdi->item.iItem < 0 ||
          plvdi->item.iItem >= g_piano_roll_state.inputs.size()) {
        g_view_logger->info("[PianoRoll] iItem out of range");
        break;
      }
      ...
      case 1:
        StrNCpy(plvdi->item.pszText, std::to_wstring(plvdi->item.iItem).c_str(),
                plvdi->item.cchTextMax);
        break;
      case 2:
        StrNCpy(plvdi->item.pszText, std::to_wstring(input.Y_AXIS).c_str(),
                plvdi->item.cchTextMax);
        break;
      case 3:
        StrNCpy(plvdi->item.pszText, std::to_wstring(input.X_AXIS).c_str(),
                plvdi->item.cchTextMax);
        break;
      default: {
        auto value =
            get_input_value_from_column_index(input, plvdi->item.iSubItem);
        auto name = get_button_name_from_column_index(plvdi->item.iSubItem);
        StrNCpy(plvdi->item.pszText, value ? name : L"",
                plvdi->item.cchTextMax);
        break;
      }
      }
...
void show() {
  ...
  std::thread([] {
    g_piano_roll_dispatcher =
        std::make_shared<Dispatcher>(GetCurrentThreadId(), []() {
          if (g_hwnd) {
    ...
    unsubscribe_funcs.push_back(Messenger::subscribe(
        Messenger::Message::CurrentSampleChanged, on_current_sample_changed));
    unsubscribe_funcs.push_back(Messenger::subscribe(
        Messenger::Message::UnfreezeCompleted, on_unfreeze_completed));
    unsubscribe_funcs.push_back(
        Messenger::subscribe(Messenger::Message::WarpModifyStatusChanged,
                             on_warp_modify_status_changed));
    unsubscribe_funcs.push_back(Messenger::subscribe(
        Messenger::Message::SeekCompleted, on_seek_completed));
    unsubscribe_funcs.push_back(Messenger::subscribe(
        Messenger::Message::SeekSavestateChanged, on_seek_savestate_changed));
    unsubscribe_funcs.push_back(Messenger::subscribe(
        Messenger::Message::EmuPausedChanged, on_emu_paused_changed));

    DialogBox(g_app_instance, MAKEINTRESOURCE(IDD_PIANO_ROLL), 0,
              (DLGPROC)dialog_proc);

    g_view_logger->info("[PianoRoll] Unsubscribing from {} messages...",
                        unsubscribe_funcs.size());
