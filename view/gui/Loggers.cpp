...
void Loggers::init() {
  HANDLE h_file = CreateFile(L"mupen.log", GENERIC_READ | GENERIC_WRITE,
  ...
  std::vector<spdlog::sink_ptr> shared_sinks = {
      std::make_shared<spdlog::sinks::wincolor_stdout_sink_mt>(),
      std::make_shared<spdlog::sinks::basic_file_sink_mt>("mupen.log")};
  std::vector<spdlog::sink_ptr> view_sinks = {
      std::make_shared<spdlog::sinks::wincolor_stdout_sink_mt>(),
      std::make_shared<spdlog::sinks::basic_file_sink_mt>("mupen.log")};
  g_core_logger = std::make_shared<spdlog::logger>("Core", begin(core_sinks),
                                                   end(core_sinks));
  g_shared_logger = std::make_shared<spdlog::logger>(
      "Shared", begin(shared_sinks), end(shared_sinks));
  g_view_logger = std::make_shared<spdlog::logger>("View", begin(view_sinks),
                                                   end(view_sinks));
#else
