...
void FrontendService::show_statusbar(const wchar_t* str) { Statusbar::post(str); }
...
bool FrontendService::get_prefers_no_render_skip() { return EncodingManager::is_capturing(); }
...
std::wstring FrontendService::find_available_rom(const std::function<bool(const t_rom_header&)>& predicate) { return Rombrowser::find_available_rom(predicate); }

void FrontendService::mge_get_video_size(int32_t* width, int32_t* height) { MGECompositor::get_video_size(width, height); }

void FrontendService::mge_copy_video(void* buffer) { MGECompositor::copy_video(buffer); }

void FrontendService::mge_load_screen(void* data) { MGECompositor::load_screen(data); }
