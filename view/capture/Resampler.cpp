...
int rsmp_get_resample_len(const int dst_freq, const int src_freq,
                          const int src_bitrate, int src_len) {
...
int rsmp_resample(short **dst, const int dst_freq, const short *src,
                  const int src_freq, const int src_bitrate,
                  const int src_len) {
  if (src_bitrate != 16) {
  ...
  speex_resampler_process_interleaved_int(speex_ctx, in_samps, &in_pos,
                                          out_samps, &out_pos);
