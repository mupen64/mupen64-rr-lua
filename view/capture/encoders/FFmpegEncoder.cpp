...
#define AUDIO_PIPE_NAME L"\\\\.\\pipe\\mupenaudio"
    ...
    m_video_pipe = CreateNamedPipe(VIDEO_PIPE_NAME, PIPE_ACCESS_OUTBOUND, PIPE_TYPE_BYTE | PIPE_WAIT, PIPE_UNLIMITED_INSTANCES, bufsize_video, bufsize_video, 0, nullptr);
    ...
    m_audio_pipe = CreateNamedPipe(AUDIO_PIPE_NAME, PIPE_ACCESS_OUTBOUND, PIPE_TYPE_BYTE | PIPE_WAIT, PIPE_UNLIMITED_INSTANCES, bufsize_audio, bufsize_audio, 0, nullptr);
    ...
    wsprintf(options, g_config.ffmpeg_final_options.data(), m_params.width, m_params.height, m_params.fps, VIDEO_PIPE_NAME, m_params.arate, AUDIO_PIPE_NAME, m_params.path.wstring().data());

    g_view_logger->info(L"[FFmpegEncoder] Starting encode with commandline:");
    g_view_logger->info(L"[FFmpegEncoder] {}", options);

    if (!CreateProcess(g_config.ffmpeg_path.c_str(), options, nullptr, nullptr, FALSE, NULL, nullptr, nullptr, &m_si, &m_pi))
    {

        FrontendService::show_dialog(std::format(L"Could not start ffmpeg process! Does ffmpeg exist on "
                                                 L"disk at '{}'?",
...
bool FFmpegEncoder::stop()
{
    ...
    {
        FrontendService::show_dialog(std::format(L"Capture stopped with {} video, {} audio elements "
    ...
    {
        FrontendService::show_dialog(std::format(L"{} frames were dropped during capture to avoid out-of-memory "
