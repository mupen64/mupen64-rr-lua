...
bool FFmpegEncoder::start(Params params) {
  ...
  g_view_logger->info(
      "[FFmpegEncoder] arate: {}, bufsize_video: {}, bufsize_audio: {}\n",
...
#define AUDIO_PIPE_NAME L"\\\\.\\pipe\\mupenaudio"
  ...
  m_video_pipe = CreateNamedPipe(
      VIDEO_PIPE_NAME, PIPE_ACCESS_OUTBOUND, PIPE_TYPE_BYTE | PIPE_WAIT,
  ...
  m_audio_pipe = CreateNamedPipe(
      AUDIO_PIPE_NAME, PIPE_ACCESS_OUTBOUND, PIPE_TYPE_BYTE | PIPE_WAIT,
  ...
  wsprintf(options, g_config.ffmpeg_final_options.data(), m_params.width,
           m_params.height, m_params.fps, VIDEO_PIPE_NAME, m_params.arate,
           AUDIO_PIPE_NAME, m_params.path.wstring().data());

  g_view_logger->info(L"[FFmpegEncoder] Starting encode with commandline:");
  g_view_logger->info(L"[FFmpegEncoder] {}", options);

  if (!CreateProcess(g_config.ffmpeg_path.c_str(), options, nullptr, nullptr,
                     FALSE, NULL, nullptr, nullptr, &m_si, &m_pi)) {

    FrontendService::show_dialog(
        std::format(L"Could not start ffmpeg process! Does ffmpeg exist on "
                    L"disk at '{}'?",
                    g_config.ffmpeg_path)
            .c_str(),
        L"FFmpeg Encoder", FrontendService::DialogType::Error);
  ...
  m_blank_buffer =
      static_cast<uint8_t *>(calloc(m_params.width * m_params.height * 3, 1));
  m_dropped_frames = 0;
...
bool FFmpegEncoder::stop() {
  ...
  if (!this->m_video_queue.empty() || !this->m_audio_queue.empty()) {
    FrontendService::show_dialog(
        std::format(L"Capture stopped with {} video, {} audio elements "
                    L"remaining in queue!\nThe capture might be corrupted.",
  ...
  if (m_dropped_frames > 0) {
    FrontendService::show_dialog(
        std::format(
            L"{} frames were dropped during capture to avoid out-of-memory "
...
static bool write_pipe_checked(const HANDLE pipe, const char *buffer,
                               const unsigned buffer_size,
                               const bool is_video) {
  DWORD written = 0;
  const auto result = WriteFile(pipe, buffer, buffer_size, &written, nullptr);
  if (written != buffer_size || !result) {
    // g_view_logger->error("[FFmpegEncoder] Error writing to {} pipe, error
    // code {}", is_video ? "video" : "audio", GetLastError());
    return false;
  }
...
bool FFmpegEncoder::append_video(uint8_t *image) {
  if (g_config.synchronization_mode == 1) {
    ...
    if (lag_count > 2) {
      ...
      append_audio_impl(m_silence_buffer,
                        static_cast<size_t>(round(samples_per_frame)));
    }
  }
  ...
  auto buf =
      static_cast<uint8_t *>(malloc(m_params.width * m_params.height * 3));

  // HACK: If we run out of memory, we start writing empty video frames
  // This can happen when encoding at high resolutions, as ffmpeg might not
  // start processing the pipe writes in time
  if (!buf) {
    g_view_logger->error(
        "[FFmpegEncoder] Out of memory, writing blank video frame");
    ++m_dropped_frames;
...
void FFmpegEncoder::write_audio_thread() {
  ...
  while (!this->m_stop_thread) {
    ...
    m_audio_cv.wait(lock,
                    [this] { return !m_audio_queue.empty() || m_stop_thread; });
...
void FFmpegEncoder::write_video_thread() {
  ...
  while (!this->m_stop_thread) {
    ...
    m_video_cv.wait(lock,
                    [this] { return !m_video_queue.empty() || m_stop_thread; });
    ...
    write_pipe_checked(m_video_pipe, (char *)pair.first,
                       m_params.width * m_params.height * 3, true);
    if (pair.second) {
      free(pair.first);
    }
  }
