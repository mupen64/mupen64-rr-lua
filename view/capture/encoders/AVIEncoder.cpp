    ...
    if (g_config.synchronization_mode == static_cast<int>(EncodingManager::Sync::Video) || g_config.synchronization_mode == static_cast<int>(EncodingManager::Sync::None))
    {
        ...
        if (desync > 1.0)
        {
            g_view_logger->info("[EncodingManager]: Correcting for A/V "
                                ...
                                desync);
            int len3 = (int)(m_params.arate / (long double)get_vis_per_second(ROM_HEADER.Country_code)) * (int)desync;
            len3 <<= 2;
...
bool AVIEncoder::write_sound(uint8_t* buf, int len, const int min_write_size, const int max_write_size, const BOOL force, uint8_t bitrate)
{
        ...
        if ((len2 % 8) == 0 || len > max_write_size)
        {
            ...
            if (len2 > 0)
            {
                if ((len2 % 4) != 0)
                {
                    ...
                    fprintf(stderr,
                            "[EncodingManager]: Warning: Possible stereo sound "
                            "error detected.\n");
                }
    ...
    sound_buf_pos += len;
    m_audio_frame += ((len / 4) / (long double)m_params.arate) * get_vis_per_second(ROM_HEADER.Country_code);
