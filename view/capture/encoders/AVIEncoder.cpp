...
bool AVIEncoder::start(Params params) {
  ...
  AVIFileInit();
  if (AVIFileOpen(&m_avi_file, params.path.wstring().c_str(),
  ...
  if (AVIMakeCompressedStream(&m_compressed_video_stream, m_video_stream,
                              m_avi_options, NULL) != AVIERR_OK) {
    stop();
    return false;
  }
  ...
  if (AVIStreamSetFormat(m_compressed_video_stream, 0, &m_info_hdr,
                         m_info_hdr.biSize +
                             m_info_hdr.biClrUsed * sizeof(RGBQUAD)) !=
      AVIERR_OK) {
    stop();
    return false;
  }
  ...
  if (AVIStreamSetFormat(m_sound_stream, 0, &m_sound_format,
...
bool AVIEncoder::append_video(uint8_t *image) {
  if (g_config.synchronization_mode !=
          static_cast<int>(EncodingManager::Sync::Audio) &&
      g_config.synchronization_mode !=
          static_cast<int>(EncodingManager::Sync::None)) {
    return true;
  }
  ...
  // AUDIO SYNC
  // This type of syncing assumes the audio is authoratative, and drops or
  // duplicates frames to keep the video as close to it as possible. Some games
  // stop updating the screen entirely at certain points, such as loading zones,
  // which will cause audio to drift away by default. This method of syncing
  // prevents this, at the cost of the video feed possibly freezing or jumping
  // (though in practice this rarely happens - usually a loading scene just
  // appears shorter or something).

  int audio_frames = (int)(m_audio_frame - m_video_frame + 0.1);
  // i've seen a few games only do ~0.98 frames of audio for a frame, let's
  ...
  if (g_config.synchronization_mode == (int)EncodingManager::Sync::Audio) {
    ...
    if (audio_frames == 0) {
      g_view_logger->warn("Dropped Frame! a/v: %Lg/%Lg", m_video_frame,
                          m_audio_frame);
    ...
    while (audio_frames > 0) {
      result = append_video_impl(image);
      g_view_logger->warn("Duped Frame! a/v: %Lg/%Lg", m_video_frame,
                          m_audio_frame);
      m_video_frame += 1.0;
      audio_frames--;
    }
...
bool AVIEncoder::append_audio(uint8_t *audio, size_t length, uint8_t bitrate) {
  ...
  if (g_config.synchronization_mode ==
          static_cast<int>(EncodingManager::Sync::Video) ||
      g_config.synchronization_mode ==
          static_cast<int>(EncodingManager::Sync::None)) {
    // VIDEO SYNC
    // This is the original syncing code, which adds silence to the audio track
    // to get it to line up with video. The N64 appears to have the ability to
    // arbitrarily disable its sound processing facilities and no audio samples
    // are generated. When this happens, the video track will drift away from
    // the audio. This can happen at load boundaries in some games, for example.
    //
    // The only new difference here is that the desync flag is checked for being
    // greater than 1.0 instead of 0. This is because the audio and video in
    // mupen tend to always be diverged just a little bit, but stay in sync over
    // time. Checking if desync is not 0 causes the audio stream to to get
    // thrashed which results in clicks and pops.

    double_t desync = m_video_frame - m_audio_frame;

    if (g_config.synchronization_mode ==
        (int)EncodingManager::Sync::None) // HACK
      desync = 0.0;

    if (desync > 1.0) {
      g_view_logger->info("[EncodingManager]: Correcting for A/V "
                          "desynchronization of %+Lf frames\n",
                          desync);
      int len3 = (int)(m_params.arate / (long double)get_vis_per_second(
      ...
      while (len3 > write_size) {
        write_sound(m_sound_buf_empty, write_size, m_params.arate, write_size,
                    FALSE, bitrate);
        len3 -= write_size;
      }
      write_sound(m_sound_buf_empty, len3, m_params.arate, write_size, FALSE,
                  bitrate);
    } else if (desync <= -10.0) {
      g_view_logger->info("[EncodingManager]: Waiting from A/V "
                          "desynchronization of %+Lf frames\n",
                          desync);
    }
  }
...
bool AVIEncoder::write_sound(uint8_t *buf, int len, const int min_write_size,
                             const int max_write_size, const BOOL force,
                             uint8_t bitrate) {
  if ((len <= 0 && !force) || len > max_write_size)
    return false;

  if (sound_buf_pos + len > min_write_size || force) {
    int len2 = rsmp_get_resample_len(RESAMPLED_FREQ, m_params.arate, bitrate,
                                     sound_buf_pos);
    if ((len2 % 8) == 0 || len > max_write_size) {
      static short *buf2 = nullptr;
      len2 = rsmp_resample(&buf2, RESAMPLED_FREQ,
                           reinterpret_cast<short *>(m_sound_buf),
                           m_params.arate, bitrate, sound_buf_pos);

      if (len2 > 0) {
        if ((len2 % 4) != 0) {
          g_view_logger->info("[EncodingManager]: Warning: Possible stereo "
                              "sound error detected.\n");
          fprintf(stderr, "[EncodingManager]: Warning: Possible stereo sound "
                          "error detected.\n");
        }

        const BOOL ok = (0 == AVIStreamWrite(m_sound_stream, m_sample,
                                             len2 / m_sound_format.nBlockAlign,
                                             buf2, len2, 0, NULL, NULL));
        m_sample += len2 / m_sound_format.nBlockAlign;
        m_avi_file_size += len2;

        if (!ok) {
          FrontendService::show_dialog(
              L"Audio output failure!\nA call to addAudioData() "
              L"(AVIStreamWrite) failed.\nPerhaps you ran out of memory?",
              L"AVI Encoder", FrontendService::DialogType::Error);
          return false;
        }
      }
      sound_buf_pos = 0;
  ...
  if (static_cast<unsigned int>(sound_buf_pos + len) >
      SOUND_BUF_SIZE * sizeof(char)) {
    FrontendService::show_dialog(
        L"Sound buffer overflow!\nCapture will be stopped.", L"AVI Encoder",
