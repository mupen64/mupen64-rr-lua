...
void readscreen_window() {
  g_main_window_dispatcher->invoke([] {
    ...
    GetDIBits(compat_dc, bitmap, 0, m_video_height, m_video_buf, &bmp_info,
              DIB_RGB_COLORS);
...
void readscreen_desktop() {
  g_main_window_dispatcher->invoke([] {
    ...
    BitBlt(compat_dc, 0, 0, m_video_width, m_video_height, dc, pt.x, pt.y,
           SRCCOPY);
...
void readscreen_hybrid() {
  ...
  // To avoid GDI weirdness with cross-thread resources, we do all GDI work on
  // UI thread.
  g_main_window_dispatcher->invoke([&] {
    // Since atupdatescreen might not have occured for a long time, we force it
    // now. This avoids "outdated" visuals, which are otherwise acceptable
    // during normal gameplay, being blitted to the video stream.
...

    {
      ...
      StretchDIBits(compat_dc, 0, 0, raw_video_width, raw_video_height, 0, 0,
                    raw_video_width, raw_video_height, m_video_buf, &bmp_info,
                    DIB_RGB_COLORS, SRCCOPY);
    }

    // First, composite the lua's dxgi surfaces
    for (auto &pair : g_hwnd_lua_map) {
      pair.second->presenter->blit(
          compat_dc, {0, 0, (LONG)pair.second->presenter->size().width,
                      (LONG)pair.second->presenter->size().height});
    }

    // Then, blit the GDI back DCs
    for (auto &pair : g_hwnd_lua_map) {
      TransparentBlt(compat_dc, 0, 0, pair.second->dc_size.width,
                     pair.second->dc_size.height, pair.second->gdi_back_dc, 0,
                     0, pair.second->dc_size.width, pair.second->dc_size.height,
                     lua_gdi_color_mask);
    }
...
void get_video_dimensions(int32_t *width, int32_t *height) {
  ...
  } else if (g_config.capture_mode == 1 || g_config.capture_mode == 2 ||
             g_config.capture_mode == 3) {
...
bool start_capture(std::filesystem::path path, EncoderType encoder_type,
  ...
  if (is_capturing()) {
    if (!stop_capture()) {
      g_view_logger->info("[EncodingManager]: Couldn't start capture because "
                          "the previous capture couldn't be stopped.");
      return false;
    }
  }
  ...
  if (!result) {
    FrontendService::show_dialog(
        L"Failed to start encoding.\r\nVerify that the encoding parameters are "
        L"valid and try again.",
        L"Capture", FrontendService::DialogType::Error);
    return false;
  }
  ...
  g_view_logger->info("[EncodingManager]: Starting capture at {} x {}...",
...
bool stop_capture() {
  ...
  if (!m_encoder->stop()) {
    FrontendService::show_dialog(L"Failed to stop encoding.", L"Capture",
...
void ai_len_changed() {
  ...
  const auto p = reinterpret_cast<short *>(
      (char *)rdram + (ai_register.ai_dram_addr & 0xFFFFFF));
  ...
  if (!m_encoder->append_audio(reinterpret_cast<uint8_t *>(buf), ai_len,
                               m_audio_bitrate)) {
    FrontendService::show_dialog(
...
void ai_dacrate_changed(std::any data) {
  ...
  if (m_capturing) {
    FrontendService::show_dialog(L"Audio frequency changed during "
                                 L"capture.\r\nThe capture will be stopped.",
                                 L"Capture",
                                 FrontendService::DialogType::Error);
    g_main_window_dispatcher->invoke([] { stop_capture(); });
    return;
  }
...
size_t get_video_frame() { return m_total_frames; }

std::filesystem::path get_current_path() { return m_current_path; }

bool is_capturing() { return m_capturing; }
