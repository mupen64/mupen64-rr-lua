...
inline uint32_t gradientRGB(uint32_t pixFront,
                            uint32_t pixBack) // blend front color with opacity M / N over opaque background:
                                              // http://en.wikipedia.org/wiki/Alpha_compositing#Alpha_blending
{
    ...
    return makePixel(calcColor(getRed(pixFront), getRed(pixBack)), calcColor(getGreen(pixFront), getGreen(pixBack)),
                     calcColor(getBlue(pixFront), getBlue(pixBack)));
}

template <unsigned int M, unsigned int N>
inline uint32_t gradientARGB(
    uint32_t pixFront,
    uint32_t pixBack) // find intermediate color between two colors with alpha channels (=> NO alpha blending!!!)
{
    static_assert(0 < M && M < N && N <= 1000, "");
    ...
    return makePixel(static_cast<unsigned char>(weightSum / N), calcColor(getRed(pixFront), getRed(pixBack)),
                     calcColor(getGreen(pixFront), getGreen(pixBack)), calcColor(getBlue(pixFront), getBlue(pixBack)));
}

// inline
// double fastSqrt(double n)
//{
//     __asm //speeds up xBRZ by about 9% compared to std::sqrt which internally uses the same assembler instructions
//     but adds some "fluff"
//     {
//         fld n
//         fsqrt
...
struct MatrixRotation
{
    static const size_t I_old =
        N - 1 -
        MatrixRotation<static_cast<RotationDegree>(rotDeg - 1), I, J, N>::J_old; // old coordinates before rotation!
...
inline double distYCbCr(uint32_t pix1, uint32_t pix2, double lumaWeight)
{
    ...
    const int r_diff =
        static_cast<int>(getRed(pix1)) - getRed(pix2); // we may delay division by 255 to after matrix multiplication
    const int g_diff = static_cast<int>(getGreen(pix1)) - getGreen(pix2); //
    const int b_diff =
        static_cast<int>(getBlue(pix1)) - getBlue(pix2); // substraction for int is noticeable faster than for double!
...
inline double distYCbCrBuffered(uint32_t pix1, uint32_t pix2)
{
    ...
    return diffToDist[(((r_diff + 0xFF) / 2) << 16) | // slightly reduce precision (division by 2) to squeeze value into
                                                      // single byte
                      (((g_diff + 0xFF) / 2) << 8) | ((b_diff + 0xFF) / 2)];
#else // not noticeably faster:
    const int r_diff_tmp = ((pix1 & 0xFF0000) + 0xFF0000 - (pix2 & 0xFF0000)) / 2;
    const int g_diff_tmp = ((pix1 & 0x00FF00) + 0x00FF00 - (pix2 & 0x00FF00)) /
                           2; // slightly reduce precision (division by 2) to squeeze value into single byte
    ...
    preProcessCorners(const Kernel_4x4 &ker, const xbrz::ScalerCfg &cfg) // result: F, G, J, K corners of "GradientType"
{
    ...
    if ((ker.f == ker.g && ker.j == ker.k) || (ker.f == ker.j && ker.g == ker.k)) return result;

    auto dist = [&](uint32_t pix1, uint32_t pix2) { return ColorDistance::dist(pix1, pix2, cfg.luminanceWeight); };

    const int weight = 4;
    double jg =
        dist(ker.i, ker.f) + dist(ker.f, ker.c) + dist(ker.n, ker.k) + dist(ker.k, ker.h) + weight * dist(ker.j, ker.g);
    double fk =
        dist(ker.e, ker.j) + dist(ker.j, ker.o) + dist(ker.b, ker.g) + dist(ker.g, ker.l) + weight * dist(ker.f, ker.k);

    if (jg < fk) // test sample: 70% of values max(jg, fk) / min(jg, fk) are between 1.1 and 3.7 with median being 1.8
    {
        const bool dominantGradient = cfg.dominantDirectionThreshold * jg < fk;
        if (ker.f != ker.g && ker.f != ker.j) result.blend_f = dominantGradient ? BLEND_DOMINANT : BLEND_NORMAL;
    ...
    else if (fk < jg)
    {
        ...
        if (ker.j != ker.f && ker.j != ker.k) result.blend_j = dominantGradient ? BLEND_DOMINANT : BLEND_NORMAL;
...
#define DEF_GETTER(x)                                                                                                  \
    template <RotationDegree rotDeg> uint32_t inline get_##x(const Kernel_3x3 &ker)                                    \
    {                                                                                                                  \
        return ker.x;                                                                                                  \
    }
...
template <RotationDegree rotDeg> inline unsigned char rotateBlendInfo(unsigned char b)
{
    return b;
}
...
FORCE_INLINE // perf: quite worth it!
    void
    blendPixel(const Kernel_3x3 &ker, uint32_t *target, int trgWidth,
...
#define i get_i<rotDeg>(ker)
    ...
    if (getBottomR(blend) >= BLEND_NORMAL)
    {
        ...
        const bool doLineBlend = [&]() -> bool {
            ...
            // make sure there is no second blending in an adjacent rotation for this pixel: handles insular pixels,
            // mario eyes
        ...
        if (doLineBlend)
        {
            const double fg = dist(f, g); // test sample: 70% of values max(fg, hc) / min(fg, hc) are between 1.1
                                          // and 3.7 with median being 1.9
...
void scaleImage(const uint32_t *src, uint32_t *trg, int srcWidth, int srcHeight, const xbrz::ScalerCfg &cfg, int yFirst,
                int yLast)
{
    ...
    //"use" space at the end of the image as temporary buffer for "on the fly preprocessing": we even could use larger
    //area of "sizeof(uint32_t) * srcWidth * (yLast - yFirst)" bytes without risk of accidental overwriting before
    //accessing
    const int bufferSize = srcWidth;
    unsigned char *preProcBuffer =
        reinterpret_cast<unsigned char *>(trg + yLast * Scaler::scale * trgWidth) - bufferSize;
    ...
    if (yFirst > 0)
    {
        ...
        for (int x = 0; x < srcWidth; ++x)
        {
            ...
            */
            ...
            if (x + 1 < bufferSize) setTopL(preProcBuffer[x + 1], res.blend_k);
        }
    }
    //------------------------------------------------------------------------------------
        ...
        for (int x = 0; x < srcWidth; ++x, out += Scaler::scale)
        {
            ...
            unsigned char blend_xy = 0; // for current (x, y) position
            {
                ...
                */
                ...
                setBottomR(blend_xy, res.blend_f); // all four corners of (x, y) have been determined at this point due
                                                   // to processing sequence!
                ...
                blend_xy1 = 0;
                setTopL(blend_xy1,
                        res.blend_k); // set 1st known corner for (x + 1, y + 1) and buffer for use on next column
...
template <class ColorGradient> struct Scaler3x : public ColorGradient
{
    ...
    template <class OutputMatrix> static void blendCorner(uint32_t col, OutputMatrix &out)
    {
        ...
        // alphaGrad<7, 256>(out.template ref<2, 1>(), col); //0.02826017254 -> negligible + avoid conflicts with other
        // rotations for this odd scale alphaGrad<7, 256>(out.template ref<1, 2>(), col); //0.02826017254
    }
};
...
template <class ColorGradient> struct Scaler5x : public ColorGradient
{
    ...
    template <class OutputMatrix> static void blendCorner(uint32_t col, OutputMatrix &out)
    {
        ...
        // alphaGrad<1, 64>(out.template ref<4, 2>(), col); //0.01676812367 -> negligible + avoid conflicts with other
        // rotations for this odd scale alphaGrad<1, 64>(out.template ref<2, 4>(), col); //0.01676812367
    }
};
                 ...
                 const xbrz::ScalerCfg &cfg, int yFirst, int yLast)
{
    ...
    if (!src || !trg || factor < 2 || factor > SCALE_FACTOR_MAX || srcWidth <= 0 || srcHeight <= 0) return;
    ...
    case ColorFormat::RGB:
        ...
        {
        case 2:
            return scaleImage<Scaler2x<ColorGradientRGB>, ColorDistanceRGB>(src, trg, srcWidth, srcHeight, cfg, yFirst,
                                                                            yLast);
        case 3:
            return scaleImage<Scaler3x<ColorGradientRGB>, ColorDistanceRGB>(src, trg, srcWidth, srcHeight, cfg, yFirst,
                                                                            yLast);
        case 4:
            return scaleImage<Scaler4x<ColorGradientRGB>, ColorDistanceRGB>(src, trg, srcWidth, srcHeight, cfg, yFirst,
                                                                            yLast);
        case 5:
            return scaleImage<Scaler5x<ColorGradientRGB>, ColorDistanceRGB>(src, trg, srcWidth, srcHeight, cfg, yFirst,
                                                                            yLast);
        case 6:
            return scaleImage<Scaler6x<ColorGradientRGB>, ColorDistanceRGB>(src, trg, srcWidth, srcHeight, cfg, yFirst,
                                                                            yLast);
        }
        break;
    ...
    case ColorFormat::ARGB:
        ...
        {
        case 2:
            return scaleImage<Scaler2x<ColorGradientARGB>, ColorDistanceARGB>(src, trg, srcWidth, srcHeight, cfg,
                                                                              yFirst, yLast);
        case 3:
            return scaleImage<Scaler3x<ColorGradientARGB>, ColorDistanceARGB>(src, trg, srcWidth, srcHeight, cfg,
                                                                              yFirst, yLast);
        case 4:
            return scaleImage<Scaler4x<ColorGradientARGB>, ColorDistanceARGB>(src, trg, srcWidth, srcHeight, cfg,
                                                                              yFirst, yLast);
        case 5:
            return scaleImage<Scaler5x<ColorGradientARGB>, ColorDistanceARGB>(src, trg, srcWidth, srcHeight, cfg,
                                                                              yFirst, yLast);
        case 6:
            return scaleImage<Scaler6x<ColorGradientARGB>, ColorDistanceARGB>(src, trg, srcWidth, srcHeight, cfg,
                                                                              yFirst, yLast);
        }
        break;
    ...
    case ColorFormat::ARGB_UNBUFFERED:
        ...
        {
        case 2:
            return scaleImage<Scaler2x<ColorGradientARGB>, ColorDistanceUnbufferedARGB>(src, trg, srcWidth, srcHeight,
                                                                                        cfg, yFirst, yLast);
        case 3:
            return scaleImage<Scaler3x<ColorGradientARGB>, ColorDistanceUnbufferedARGB>(src, trg, srcWidth, srcHeight,
                                                                                        cfg, yFirst, yLast);
        case 4:
            return scaleImage<Scaler4x<ColorGradientARGB>, ColorDistanceUnbufferedARGB>(src, trg, srcWidth, srcHeight,
                                                                                        cfg, yFirst, yLast);
        case 5:
            return scaleImage<Scaler5x<ColorGradientARGB>, ColorDistanceUnbufferedARGB>(src, trg, srcWidth, srcHeight,
                                                                                        cfg, yFirst, yLast);
        case 6:
            return scaleImage<Scaler6x<ColorGradientARGB>, ColorDistanceUnbufferedARGB>(src, trg, srcWidth, srcHeight,
                                                                                        cfg, yFirst, yLast);
        }
        break;
    }
                         ...
                         /**/ uint32_t *trg, int trgWidth, int trgHeight)
{
    ...
    bilinearScale(src, srcWidth, srcHeight, srcWidth * (int)sizeof(uint32_t), trg, trgWidth, trgHeight,
                  trgWidth * (int)sizeof(uint32_t), 0, trgHeight, [](uint32_t pix) { return pix; });
}
                                ...
                                /**/ uint32_t *trg, int trgWidth, int trgHeight)
{
    ...
    nearestNeighborScale(src, srcWidth, srcHeight, srcWidth * (int)sizeof(uint32_t), trg, trgWidth, trgHeight,
                         trgWidth * (int)sizeof(uint32_t), 0, trgHeight, [](uint32_t pix) { return pix; });
}
